export default {
  ...
  market: {
    ...
    detail: {
      ...
      sdkapi: {
        title: 'SDK API 兼容性提醒',
        missing: '该插件未声明 sdkapi，可能无法在当前 Tuff 版本中运行。',
        invalid: '该插件声明的 sdkapi 无效（{value}），可能无法安装。',
        unsupported: '该插件使用的 sdkapi {value} 不受当前客户端支持。',
        tooNew: '该插件要求 sdkapi {value}，当前客户端为 {version}，请先升级 Tuff。',
        legacy: '该插件基于较旧的 sdkapi {value}，可能缺少权限保护。',
      },
      reviews: {
        title: '社区评论',
        tag: '社区',
...
Tuff 可能随时更新本协议,继续提交表示接受变更。`,
        ...
        sdkapi: {
          title: 'SDK API 兼容性',
          missing: '未声明 sdkapi，发布前请设置为 {version}。',
          invalid: 'sdkapi 无效（{value}），请使用 {version}。',
          unsupported: 'sdkapi {value} 不在支持列表，请使用 {version}。',
          tooNew: 'sdkapi {value} 高于当前 {version}，请先升级 Tuff。',
          tooOld: 'sdkapi {value} 低于最低要求 {version}，请升级插件。',
        },
