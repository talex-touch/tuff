...
import type {
  ...
  NotificationChannelDisplayStatus,
  NotificationChannelRiskStatus,
  NotificationDeliveryStatus,
  PlatformGovernanceD1ReadinessCheck,
  PlatformGovernanceD1ReadinessStatus,
  PlatformGovernanceReportPriority,
  PlatformGovernanceReportStatus,
  ProviderQuotaActionQueuePriority,
  ProviderQuotaActionQueueReason,
  ProviderQuotaRiskReason,
  ProviderQuotaSmokeStatus,
  ProviderQuotaStatus,
  StatusTone,
  StorageAlertMetric,
  StorageChannelPressureStatus,
  StoragePolicyAlert,
  StoragePolicyEvaluationStatus,
  StorageSmokeEvidenceStatus,
} from "~/types/admin-governance";

export type GovernanceTranslate = (key: string, fallback: string) => string;

export function createGovernanceFormatters(
  tt: GovernanceTranslate,
  locale: () => string,
) {
  ...
  function formatHeatmapCellOpacity(value: number, peak: number): string {
    if (
      ...
      return "0.08";
    return String(
      Math.min(0.96, Math.max(0.18, Math.round((value / peak) * 100) / 100)),
    );
  }
  ...
  function storageEvaluationTone(
  ...
  function storageChannelPressureTone(
    status: StorageChannelPressureStatus,
  ): StatusTone {
  ...
  function storageChannelPressureLabel(
    ...
    if (status === "unmanaged")
      return tt(
        "dashboard.governance.analytics.storageChannelUnmanaged",
        "Unmanaged",
      );
    return storageEvaluationLabel(status);
  }

  function storageSmokeEvidenceTone(
  ...
  function providerQuotaSmokeTone(
    ...
    if (status === "consumed" || status === "allowed") return "success";
    return "info";
  }
  ...
  function providerQuotaRiskReasonLabel(
    reason: ProviderQuotaRiskReason,
  ): string {
    if (reason === "overage")
      return tt(
        "dashboard.governance.analytics.providerQuotaRiskOverage",
        "Over limit",
      );
    if (reason === "low-remaining")
      return tt(
        "dashboard.governance.analytics.providerQuotaRiskLowRemaining",
        "Low remaining",
      );
    if (reason === "projected-exhaustion")
      return tt(
        "dashboard.governance.analytics.providerQuotaRiskProjected",
        "Projected exhaustion",
      );
    if (reason === "blocked")
      return tt(
        "dashboard.governance.analytics.providerQuotaRiskBlocked",
        "Blocked",
      );
    return tt(
      "dashboard.governance.analytics.providerQuotaRiskWarning",
      "Warning threshold",
    );
  }
  ...
  function providerQuotaActionReasonLabel(
    ...
    if (reason === "token-overage")
      return tt(
        "dashboard.governance.analytics.providerQuotaActionTokenOverage",
        "Token overage",
      );
    if (reason === "request-overage")
      return tt(
        "dashboard.governance.analytics.providerQuotaActionRequestOverage",
        "Request overage",
      );
    if (reason === "token-exhausted")
      return tt(
        "dashboard.governance.analytics.providerQuotaActionTokenExhausted",
        "Tokens exhausted",
      );
    if (reason === "request-exhausted")
      return tt(
        "dashboard.governance.analytics.providerQuotaActionRequestExhausted",
        "Requests exhausted",
      );
    if (reason === "projected-exhaustion")
      return tt(
        "dashboard.governance.analytics.providerQuotaActionProjected",
        "Projected exhaustion",
      );
    if (reason === "quota-disabled")
      return tt(
        "dashboard.governance.analytics.providerQuotaActionDisabled",
        "Quota disabled",
      );
    if (reason === "missing-hard-limit")
      return tt(
        "dashboard.governance.analytics.providerQuotaActionMissingLimit",
        "Missing hard limit",
      );
    return tt(
      "dashboard.governance.analytics.providerQuotaActionWarning",
      "Warning threshold",
    );
  }
  ...
  function reportEvidenceTone(
    status: PlatformGovernanceReportEvidenceStatus,
  ): StatusTone {
    if (isProductionEvidenceSource(status)) return "success";
    if (isFallbackEvidenceSource(status)) return "warning";
    return "muted";
  }
  ...
  function storageAlertMetricLabel(metric: StorageAlertMetric): string {
    if (metric === "storedBytes")
      return tt(
        "dashboard.governance.storageAlerts.storedBytes",
        "Stored bytes",
      );
    if (metric === "trafficBytes")
      return tt(
        "dashboard.governance.storageAlerts.trafficBytes",
        "Traffic bytes",
      );
  ...
  function formatStorageAlertValue(
    ...
    return alert.metric === "operations"
      ? formatNumber(value)
      : formatBytes(value);
  }
  ...
  function notificationChannelTone(
    status: NotificationChannelDisplayStatus,
  ): StatusTone {
    if (status === "warning") return "warning";
  ...
  function notificationChannelLabel(
    ...
    if (status === "warning")
      return tt(
        "dashboard.governance.analytics.notificationChannelWarning",
        "Warning",
      );
    if (status === "disabled")
      return tt(
        "dashboard.governance.analytics.notificationChannelDisabled",
        "Disabled",
      );
  ...
  function notificationActionQueueStatusLabel(
    item: NotificationActionQueueItem,
  ): string {
    if (item.source === "delivery-health")
      return tt(
        "dashboard.governance.analytics.notificationDeliveryRisk",
        "Delivery risk",
      );
    return notificationChannelLabel(
      item.status as NotificationChannelRiskStatus,
    );
  }

  function notificationChannelCredentialLabel(channel: {
    ...
    if (!channel.credentialRequired)
      return tt(
        "dashboard.governance.analytics.notificationChannelCredentialNotRequired",
        "no credential required",
      );
    return (channel.hasCredentialRef ?? Boolean(channel.credentialRef))
      ? tt(
          "dashboard.governance.analytics.notificationChannelCredentialBound",
          "credentialRef bound",
        )
      : tt(
          "dashboard.governance.analytics.notificationChannelCredentialMissing",
          "credentialRef missing",
        );
  }

  function notificationChannelReadinessLabel(channel: {
    readiness: { productionReady: boolean; reasons: string[] };
  }): string {
    if (channel.readiness.productionReady)
      return tt(
        "dashboard.governance.analytics.notificationChannelProductionReady",
        "production ready",
      );
    return (
      channel.readiness.reasons.join(", ") ||
      tt(
        "dashboard.governance.analytics.notificationChannelReadinessPending",
        "readiness pending",
      )
    );
  }
  ...
  function formatD1MissingObjects(
    check: PlatformGovernanceD1ReadinessCheck,
  ): string {
  ...
  return {
    ...
    formatDelta,
    formatTrendWidth,
    formatHeatmapCellOpacity,
    formatWeekdayLabel,
    formatDate,
    formatShortDate,
    configTypeLabel,
    storageEvaluationTone,
    storageEvaluationLabel,
    storageChannelPressureTone,
    storageChannelPressureLabel,
    storageSmokeEvidenceTone,
    storageSmokeEvidenceLabel,
    providerQuotaTone,
    providerQuotaLabel,
    providerQuotaSmokeTone,
    providerQuotaSmokeLabel,
    providerQuotaRiskReasonLabel,
    providerQuotaActionTone,
    providerQuotaActionReasonLabel,
    reportStatusTone,
    reportStatusLabel,
    reportEvidenceTone,
    reportEvidenceLabel,
    reportPriorityTone,
    storageAlertMetricLabel,
    formatStorageAlertValue,
    formatStorageBudgetValue,
    formatProjectedDays,
    notificationDeliveryTone,
    notificationChannelTone,
    notificationChannelLabel,
    notificationActionQueueStatusLabel,
    notificationChannelCredentialLabel,
    notificationChannelReadinessLabel,
    formatRatio,
    d1ReadinessTone,
    d1ReadinessLabel,
    formatD1MissingObjects,
  };
}
