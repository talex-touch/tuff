...
describe('docs page performance boundaries', () => {
  ...
  it('warms component docs links on sidebar intent without enabling bulk NuxtLink prefetch', () => {
    ...
    expect(docsSidebar).toContain('COMPONENT_DOCS_FULL_BODY_PREFETCH_DELAY_MS = 240')
    expect(docsSidebar).toContain('COMPONENT_DOCS_FULL_BODY_PREFETCH_IDLE_TIMEOUT_MS = 1400')
    ...
    expect(docsSidebar).toMatch(/const prefetchFullDoc = \(\) => \{[\s\S]*requestDocsPage\(\{ path: normalized, locale, body: '1' \}\)/)
    expect(docsSidebar).toMatch(/setTimeout\(\(\) => \{[\s\S]*window\.requestIdleCallback\(prefetchFullDoc, \{ timeout: COMPONENT_DOCS_FULL_BODY_PREFETCH_IDLE_TIMEOUT_MS \}\)[\s\S]*prefetchFullDoc\(\)[\s\S]*COMPONENT_DOCS_FULL_BODY_PREFETCH_DELAY_MS/)
