...
describe('docs page performance boundaries', () => {
  ...
  it('caches split full-body docs on the client without deep AST reactivity', () => {
    ...
    expect(page).toMatch(/const shouldDeferDocBodySections = computed\(\(\) =>[\s\S]*import\.meta\.dev[\s\S]*import\.meta\.client[\s\S]*docScope\.value\.isComponent[\s\S]*docSyncStatus\.value !== 'verified'[\s\S]*deferredBodySplitIndex\.value > 0/)
