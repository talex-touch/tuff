...
describe('docs page performance boundaries', () => {
  ...
  it('caches split full-body docs on the client without deep AST reactivity', () => {
    ...
    expect(page).toMatch(/function startFullDocFetchForRoute\(\) \{[\s\S]*const fetchId = \+\+activeDocFetchId[\s\S]*void loadFullDocForRoute\(fetchId, docPath\.value, docsLocale\.value\)[\s\S]*\}/)
