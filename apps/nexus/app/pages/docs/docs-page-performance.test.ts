...
describe('docs page performance boundaries', () => {
  it('keys catch-all docs pages by route path to avoid stale content on client navigation', () => {
    ...
    expect(page).toContain('function shouldSplitDocsPageBody(path: string)')
    expect(page).toContain("return normalized.startsWith('/docs/') && normalized !== '/docs/index'")
    expect(page).toContain('const shouldSplitDocBody = computed(() => shouldSplitDocsPageBody(activeRoutePath.value))')
  })
  ...
  it('keeps the sidebase auth client plugin out of the public docs app shell', () => {
    expect(nuxtConfig).toContain('function isSidebaseAuthClientPlugin')
    expect(nuxtConfig).toContain('function isSidebaseAuthMiddleware')
    expect(nuxtConfig).toContain('function removeSidebaseAuthAppRuntime')
    expect(nuxtConfig).toContain("replace(/\\.js$/, '')")
    expect(nuxtConfig).toContain("normalized.endsWith('/@sidebase/nuxt-auth/dist/runtime/middleware/auth')")
    expect(nuxtConfig).toMatch(/function removeSidebaseAuthAppRuntime[\s\S]*isSidebaseAuthClientPlugin\(app\.plugins\[index\]\?\.src\)[\s\S]*isSidebaseAuthMiddleware\(middleware\?\.name, middleware\?\.path\)/)
    expect(nuxtConfig).toMatch(/'app:resolve'\(app\) \{[\s\S]*removeSidebaseAuthAppRuntime\(app\)/)
    expect(nuxtConfig).toMatch(/'app:templates'\(app\) \{[\s\S]*removeSidebaseAuthAppRuntime\(app\)/)

    expect(appRoot).toContain('const { status, getSession } = useNexusAuth()')
    expect(theHeader).toContain('const { status } = useNexusAuth()')
    expect(appRoot).not.toContain('useAuth()')
    expect(theHeader).not.toContain('useAuth()')

    expect(nexusAuth).toContain("const AUTH_BASE_URL = '/api/auth'")
    expect(nexusAuth).not.toContain('@sidebase/nuxt-auth')
    expect(nexusAuth).not.toContain('dist/runtime/plugin')
  })

  it('keeps auth runtime coverage on explicit auth and protected route matrix after removing sidebase client plugin', () => {
    expect(nuxtConfig).toContain("'@sidebase/nuxt-auth'")
    expect(nuxtConfig).toContain("baseURL: '/api/auth'")
    expect(nuxtConfig).toContain("type: 'authjs'")
    ...
    expect(teamJoinPage).toContain('requiresAuth: true')
    expect(teamJoinPage).toContain('fetchCurrentUserProfile')
    expect(adminBootstrapPage).toContain('requiresAuth: true')
    expect(adminBootstrapPage).toContain('const { signOut } = useNexusAuth()')

    expect(appRoot).toContain('const isProtectedRoute = computed(() => route.meta.requiresAuth === true)')
    expect(appRoot).toMatch(/if \(!protectedSessionRecheckDone\.value && !protectedSessionRecheckRunning\.value\) \{[\s\S]*void getSession\(\)[\s\S]*protectedSessionRecheckDone\.value = true/)
    expect(appRoot).toMatch(/router\.replace\(\{[\s\S]*path: '\/sign-in'[\s\S]*redirect_url: redirectTarget\.value/)

    expect(theHeader).toContain("const HeaderUserMenu = defineAsyncComponent(() => import('./HeaderUserMenu.vue'))")
    expect(theHeader).toContain('const isAuthenticated = computed(() => status.value === \'authenticated\')')
  })

  it('keeps docs, store, dashboard, and landing source boundaries split before production chunking', () => {
    expect(page).not.toContain("~/components/dashboard/")
    expect(page).not.toContain("~/components/store/")
    expect(page).not.toContain("~/components/tuff/landing/")
    expect(page).not.toContain("from '@talex-touch/tuffex/button'")
    expect(page).not.toContain("from '@talex-touch/tuffex/tabs'")

    expect(storePage).not.toContain("~/components/docs/")
    expect(storePage).not.toContain("~/components/tuff/landing/")
    expect(storePage).not.toContain("~/components/dashboard/")
    expect(storePage).toContain("~/components/plugin/PluginMetaHeader.vue")
    expect(storeLayout).not.toContain("~/components/docs/")
    expect(storeLayout).not.toContain("~/components/dashboard/")

    expect(dashboardPage).not.toContain("~/components/docs/")
    expect(dashboardPage).not.toContain("~/components/store/")
    expect(dashboardOverviewPage).not.toContain("~/components/docs/")
    expect(dashboardOverviewPage).not.toContain("~/components/store/")
    expect(providerRegistryPage).toContain('<ClientOnly>')
    expect(providerRegistryPage).toContain('<LazyDashboardProviderRegistryAdminPanel />')
    expect(providerRegistryPage).not.toContain("import ProviderRegistryAdminPanel")
    expect(governancePage).not.toContain("~/components/docs/")
    expect(governancePage).not.toContain("~/components/store/")

    expect(landingPage).toContain("defineAsyncComponent(() => import('~/components/tuff/TuffHome.vue'))")
    expect(landingPage).not.toContain("~/components/dashboard/")
    expect(landingPage).not.toContain("~/components/docs/")
    expect(landingPage).not.toContain("~/components/store/")
    expect(newLandingPage).toContain("defineAsyncComponent(() => import('~/components/tuff/landing/TuffLandingPlugins.vue'))")
    expect(newLandingPage).not.toContain("~/components/dashboard/")
    expect(newLandingPage).not.toContain("~/components/docs/")
    expect(newLandingPage).not.toContain("~/components/store/")
  })
  ...
  it('caches split full-body docs on the client without deep AST reactivity', () => {
    expect(page).toContain("import { cacheDocsFullBody, hasCachedDocsFullBody, isDocsPageRecordForRoute, readCachedDocsFullBody, requestDocsPage, resolveDocsFullBodyCacheKey } from '~/utils/docs-page-client-cache'")
    expect(page).toContain("import { useTypedFetch } from '~/utils/request'")
    expect(docsPageClientCache).toContain('DOCS_FULL_BODY_CACHE_LIMIT = 24')
    ...
    expect(page).toContain("DOCS_CURRENT_PAGE_FETCH_KEY_PREFIX = 'docs-current-page'")
    expect(page).not.toContain('DOCS_CURRENT_FULL_BODY_FETCH_KEY')
    expect(docsPageClientCache).toContain('const docsFullBodyCache = new Map<string, DocsPageRecord>()')
    expect(page).toContain('const shouldRequestMetadataOnlyDocBody = computed(() => shouldSplitDocBody.value)')
    expect(page).toContain("const currentDocsPageBodyMode = computed(() => (shouldRequestMetadataOnlyDocBody.value ? '0' : '1'))")
    expect(page).toMatch(/const currentDocsPageFetchKey = computed\(\(\) => `\$\{DOCS_CURRENT_PAGE_FETCH_KEY_PREFIX\}:\$\{docPath\.value\}:\$\{docsLocale\.value\}:\$\{currentDocsPageBodyMode\.value\}`\)/)
    ...
    expect(page).not.toContain('const docsFullBodyCache = new Map')
    expect(docsPageClientCache).toContain('const docsFullBodyCache = new Map<string, DocsPageRecord>()')
    expect(docsPageClientCache).toContain('export function resolveDocsFullBodyCacheKey(path: string, locale: DocsPageLocale)')
    expect(docsPageClientCache).toContain('export function readCachedDocsFullBody(cacheKey: string)')
    expect(docsPageClientCache).toContain('export function hasCachedDocsFullBody(cacheKey: string)')
    expect(docsPageClientCache).toContain('export function cacheDocsFullBody(value: DocsPageRecord)')
    expect(docsPageClientCache).toMatch(/return `doc-full:\$\{normalizeDocsPagePath\(path\)\}:\$\{locale\}`/)
    expect(docsPageClientCache).toContain('while (docsFullBodyCache.size > DOCS_FULL_BODY_CACHE_LIMIT)')
    expect(page).toMatch(/const fullDocCacheKey = computed\(\(\) => resolveDocsFullBodyCacheKey\(docPath\.value, docsLocale\.value\)\)/)
    ...
    expect(page).toContain('const { data: docPayload, status } = await useTypedFetch<Record<string, any> | null>(')
    expect(docsPageClientCache).toContain('export function isDocsPageRecordForRoute(')
    expect(page).toMatch(/const doc = shallowRef<Record<string, any> \| null>\([\s\S]*isDocsPageRecordForRoute\(docPayload\.value, docPath\.value, docsLocale\.value\)/)
    ...
    expect(page).toContain('key: currentDocsPageFetchKey')
    expect(page).toMatch(/key: currentDocsPageFetchKey,[\s\S]*immediate: import\.meta\.server \|\| !shouldSplitDocBody\.value,[\s\S]*watch: false/)
    expect(page).toContain('body: currentDocsPageBodyMode.value')
    expect(page).toContain("const currentDocsPageBodyMode = computed(() => (shouldRequestMetadataOnlyDocBody.value ? '0' : '1'))")
    expect(page).not.toContain('key: DOCS_CURRENT_PAGE_FETCH_KEY')
    ...
    expect(page).toMatch(/const cacheKey = resolveDocsFullBodyCacheKey\(path, locale\)[\s\S]*if \(hasCachedDocsFullBody\(cacheKey\)\) \{[\s\S]*settleFullDoc\(readCachedDocsFullBody\(cacheKey\) \?\? null\)/)
    expect(page).toMatch(/fullDocLoading\.value = true[\s\S]*requestDocsPage\(\{ path, locale, body: '1' \}\)/)
    expect(page).toMatch(/settleFullDoc\(cacheDocsFullBody\(nextFullDoc\)\)/)
    // Every exit from the body fetch resolves the view — a spinner must never outlive
    // the data that would replace it.
    expect(page).toContain('function settleFullDoc(value: Record<string, any> | null)')
    expect(page).toMatch(/finally \{[\s\S]*if \(!isStaleDocFetch\(fetchId, path, locale\)\) \{[\s\S]*fullDocLoading\.value = false[\s\S]*isLoading\.value = false[\s\S]*\}/)
    expect(page).toMatch(/const viewState = computed\(\(\) => \{[\s\S]*if \(doc\.value \|\| renderDoc\.value\)[\s\S]*return 'content'[\s\S]*if \(isLoading\.value\)[\s\S]*return 'loading'/)
    // The incoming instance drives its own load; the outgoing one no longer can.
    expect(page).toMatch(/if \(import\.meta\.client\)\s*\n\s*void loadActiveDocForRoute\(\)/)
    expect(page).toContain('function seedFullDocFromCurrentDoc()')
    expect(page).toMatch(/function seedFullDocFromCurrentDoc\(\) \{[\s\S]*if \(!shouldSplitDocBody\.value \|\| !doc\.value\?\.body\)[\s\S]*return false[\s\S]*settleFullDoc\(cacheDocsFullBody\(doc\.value as Record<string, any>\)\)[\s\S]*return true[\s\S]*\}/)
    expect(page).toContain('function scheduleFullDocFetchForRoute(fetchId: number, path: string, locale: \'en\' | \'zh\')')
    expect(page).toMatch(/function scheduleFullDocFetchForRoute\(fetchId: number, path: string, locale: 'en' \| 'zh'\) \{[\s\S]*clearFullDocFetchSchedule\(\)[\s\S]*const load = \(\) => \{[\s\S]*if \(isStaleDocFetch\(fetchId, path, locale\)\)[\s\S]*return[\s\S]*void loadFullDocForRoute\(fetchId, path, locale\)/)
    ...
    expect(page).toMatch(/const cachedFullDoc = splitBody \? readCachedDocsFullBody\(fullDocCacheKey\.value\) : undefined[\s\S]*if \(cachedFullDoc !== undefined\) \{[\s\S]*settleFullDoc\(cachedFullDoc\)[\s\S]*return/)
    expect(page).toContain('const hasResolvedDoc = isDocsPageRecordForRoute(doc.value, path, locale)')
    expect(page).toMatch(/if \(!hasResolvedDoc\) \{[\s\S]*doc\.value = null[\s\S]*fullDoc\.value = null[\s\S]*\}[\s\S]*isLoading\.value = !hasResolvedDoc/)
    expect(page).toMatch(/try \{[\s\S]*requestDocsPage\(\{ path, locale, body: splitBody \? '0' : '1' \}\)/)
    expect(page).toMatch(/function isStaleDocFetch\(fetchId: number, path: string, locale: 'en' \| 'zh'\) \{[\s\S]*return fetchId !== activeDocFetchId \|\| path !== docPath\.value \|\| locale !== docsLocale\.value/)
    expect(page).not.toContain('const fullDocRequest = splitBody && !hasCachedBody')
    expect(page).not.toContain('void fullDocRequest')
    expect(page).toMatch(/if \(!splitBody \|\| seedFullDocFromCurrentDoc\(\)\)[\s\S]*return[\s\S]*scheduleFullDocFetchForRoute\(fetchId, path, locale\)/)
    ...
    expect(page).toMatch(/<template v-if="renderDoc\?\.body">[\s\S]*<ContentRenderer[\s\S]*:value="primaryRenderDoc \?\? \{\}"/)
    expect(page).toContain('const deferredBodySplitIndex = computed(() => resolveDeferredBodySplitIndex(renderDoc.value?.body))')
    expect(page).toContain('const isDocPerformancePending = computed(() => {')
    expect(page).toMatch(/const isDocPerformancePending = computed\(\(\) => \{[\s\S]*docMeta\.value\.syncStatus\.trim\(\)\.toLowerCase\(\)[\s\S]*hasReviewedSyncStatus = raw === 'reviewed' \|\| normalizedSyncStatus === 'verified'[\s\S]*return !hasReviewedSyncStatus \|\| !isDocVerified\.value[\s\S]*\}/)
    expect(page).not.toMatch(/shouldDeferDocBodySections[\s\S]*docSyncStatus\.value !== 'verified'/)
    expect(page).toMatch(/const shouldDeferDocBodySections = computed\(\(\) =>[\s\S]*import\.meta\.dev[\s\S]*import\.meta\.client[\s\S]*docScope\.value\.isComponent[\s\S]*isDocPerformancePending\.value[\s\S]*deferredBodySplitIndex\.value > 0/)
    expect(page).toContain('const primaryRenderDoc = computed(() => {')
    expect(page).toContain('const deferredRenderDoc = computed(() => {')
    expect(page).toContain('function readBodyNodeTag(node: unknown)')
    expect(page).toContain('function isDeferredBoundaryNode(node: unknown)')
    expect(page).toContain("tag === 'tuff-demo-wrapper'")
    expect(page).toContain("tag === 'tuff-props-table'")
    expect(page).toMatch(/function resolveDeferredBodySplitIndex\(body: unknown\) \{[\s\S]*let secondH2SplitIndex = -1[\s\S]*h2Count === 2[\s\S]*secondH2SplitIndex = index[\s\S]*let deferredBoundaryCount = 0[\s\S]*deferredBoundaryCount < 2 && secondH2SplitIndex > 0[\s\S]*return Math\.min\(secondH2SplitIndex, boundarySplitIndex\)[\s\S]*return secondH2SplitIndex/)
    expect(page).toContain('let secondH2SplitIndex = -1')
    expect(page).toContain('let deferredBoundaryCount = 0')
    expect(page).toContain('if (deferredBoundaryCount < 2 && secondH2SplitIndex > 0)')
    expect(page).toContain('return Math.min(secondH2SplitIndex, boundarySplitIndex)')
    expect(page).toContain('if (Array.isArray(record.value))')
    expect(page).toContain('if (Array.isArray(node))')
    expect(page).toContain('return node[0] === tag')
    expect(page).toContain("Array.isArray(body.value)")
    expect(page).toContain('[childKey]: children')
    expect(page).toContain('function scheduleDeferredDocBodySections()')
    expect(page).toContain('DOCS_DEFERRED_BODY_IDLE_TIMEOUT_MS = 3200')
    expect(page).toContain("DOCS_DEFERRED_BODY_INTENT_EVENTS = ['scroll', 'wheel', 'keydown', 'touchstart'] as const")
    expect(page).toContain('function bindDeferredDocBodyIntentListeners()')
    expect(page).toContain('function handleDeferredDocBodyIntent()')
    expect(page).toMatch(/window\.addEventListener\(eventName, handleDeferredDocBodyIntent, \{ passive: true \}\)/)
    expect(page).toMatch(/setTimeout\(\(\) => \{[\s\S]*mountDeferredDocBodySections\(\)[\s\S]*\}, DOCS_DEFERRED_BODY_IDLE_TIMEOUT_MS\)/)
    expect(page).toMatch(/watch\([\s\S]*\(\) => route\.hash,[\s\S]*revealDeferredDocBodyForHash\(hash\)/)
    expect(page).toContain('ref="deferredDocBodySentinelRef"')
    expect(page).toContain('class="docs-deferred-body-shell"')
    expect(page).toMatch(/<ContentRenderer[\s\S]*v-if="shouldRenderDeferredDocBody && deferredRenderDoc\?\.body"[\s\S]*:value="deferredRenderDoc \?\? \{\}"/)
  ...
  it('dedupes docs page client requests shared by sidebar prefetch and route loads', () => {
    ...
    expect(docsPageClientCache).toContain('primeDocsPageRequestCache({ path, locale: locale as DocsPageLocale, body: \'1\' }, value)')
