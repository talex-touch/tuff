...
describe('docs page performance boundaries', () => {
  ...
  it('caches split full-body docs on the client without deep AST reactivity', () => {
    ...
    expect(page).toMatch(/function resolveDeferredBodySplitIndex\(body: unknown\) \{[\s\S]*if \(children\.length < 5\)[\s\S]*h2Count === 2[\s\S]*return index[\s\S]*for \(let index = 3; index < children\.length; index \+= 1\)[\s\S]*isDeferredBoundaryNode\(children\[index\]\)[\s\S]*isBodyHeadingNode\(children\[index - 1\], 'h3'\)[\s\S]*return index - 1[\s\S]*return index/)
