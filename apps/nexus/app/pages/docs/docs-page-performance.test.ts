...
describe('docs page performance boundaries', () => {
  ...
  it('caches split full-body docs on the client without deep AST reactivity', () => {
    ...
    expect(page).toContain('DOCS_FULL_BODY_FALLBACK_DELAY_MS = 220')
  ...
  it('warms pager docs targets into the split-body cache on user intent', () => {
    ...
    expect(page).toMatch(/const prefetchFullDoc = \(\) => \{[\s\S]*if \(hasCachedFullDoc\(cacheKey\)\)[\s\S]*return[\s\S]*requestDocsPage\(\{ path: normalized, locale, body: '1' \}\)[\s\S]*cacheFullDoc\(nextFullDoc\)/)
    expect(page).toMatch(/setTimeout\(\(\) => \{[\s\S]*window\.requestIdleCallback\(prefetchFullDoc, \{ timeout: DOCS_FULL_BODY_IDLE_TIMEOUT_MS \}\)[\s\S]*prefetchFullDoc\(\)[\s\S]*DOCS_FULL_BODY_FALLBACK_DELAY_MS/)
