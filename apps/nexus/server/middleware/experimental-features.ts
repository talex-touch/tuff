...
import { isRiskControlEnabled, isWatermarkEnabled } from '../utils/featureFlags'
...
function isRiskApiPath(path: string): boolean {
  if (RISK_API_PREFIXES.some(prefix => path.startsWith(prefix)))
    return true
  if (path.startsWith('/api/admin/telemetry/ip-blocks/unblock'))
    return true
...
export default defineEventHandler((event) => {
  ...
  if (!isRiskControlEnabled(event) && isRiskApiPath(path)) {
    throw featureDisabled()
  }

  if (!isWatermarkEnabled(event) && isWatermarkApiPath(path)) {
    throw featureDisabled()
  }
})
