...
export async function listProviders(event: H3Event, userId: string): Promise<IntelligenceProviderRecord[]> {
  ...
  const { results } = await db
    .prepare(`SELECT * FROM ${PROVIDERS_TABLE} WHERE user_id = ? ORDER BY priority ASC, created_at ASC`)
    .bind(userId)
    .all<IntelligenceProviderRow>()
...
export async function getProvider(
  ...
  const row = await db
    .prepare(`SELECT * FROM ${PROVIDERS_TABLE} WHERE id = ? AND user_id = ?`)
    .bind(providerId, userId)
    .first<IntelligenceProviderRow>()
...
export async function createProvider(
  ...
  `,
    ...
    .bind(
      ...
      data.name,
      data.enabled ? 1 : 0,
      data.apiKey ? encryptApiKey(event, data.apiKey) : null,
      data.baseUrl || null,
...
export async function updateProvider(
  ...
  if (data.apiKey !== undefined) {
    ...
    values.push(data.apiKey ? encryptApiKey(event, data.apiKey) : null)
  }
  ...
  if (data.rateLimit !== undefined) {
    ...
    values.push(data.rateLimit ? JSON.stringify(data.rateLimit) : null)
  }
  if (data.capabilities !== undefined) {
    sets.push('capabilities = ?')
    values.push(data.capabilities ? JSON.stringify(data.capabilities) : null)
  }
  ...
  if (sets.length === 0) return getProvider(event, userId, providerId)
  ...
  await db
    .prepare(`UPDATE ${PROVIDERS_TABLE} SET ${sets.join(', ')} WHERE id = ? AND user_id = ?`)
    .bind(...values)
    .run()
...
export async function deleteProvider(event: H3Event, userId: string, providerId: string): Promise<boolean> {
  ...
  const result = await db
    .prepare(`DELETE FROM ${PROVIDERS_TABLE} WHERE id = ? AND user_id = ?`)
    .bind(providerId, userId)
    .run()
...
export async function getProviderApiKey(event: H3Event, userId: string, providerId: string): Promise<string | null> {
  ...
  const row = await db
    .prepare(`SELECT api_key_encrypted FROM ${PROVIDERS_TABLE} WHERE id = ? AND user_id = ?`)
    .bind(providerId, userId)
    .first<{ api_key_encrypted: string | null }>()

  if (!row?.api_key_encrypted) return null

  return decryptApiKey(event, row.api_key_encrypted)
}
...
export async function getSettings(event: H3Event, userId: string): Promise<IntelligenceSettingsRecord> {
  ...
  const row = await db.prepare(`SELECT * FROM ${SETTINGS_TABLE} WHERE user_id = ?`).bind(userId).first<{
    user_id: string
...
export async function updateSettings(
  ...
  `,
    ...
    .bind(userId, strategy, audit ? 1 : 0, cache ? 1 : 0, expiration, now)
    .run()
