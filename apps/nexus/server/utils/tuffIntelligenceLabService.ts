...
async function resolveAgentPromptInstruction(
  ...
  try {
    const template = await resolvePromptTemplateFromRegistry(
      event,
      userId,
      capabilityId,
    );
...
async function buildSystemContextLines(
  ...
  const locale =
    normalizeLocaleCode(profile?.locale) || profile?.locale || "en";
...
async function buildSystemPrompt(
  ...
  const contextLines = await buildSystemContextLines(
    event,
    userId,
    options?.sessionId,
  );
...
function parsePlannedActions(raw: string): IntelligenceLabAction[] {
  ...
  const normalized = parsed
    ...
    .map((item, index): IntelligenceLabAction | null => {
      ...
      const typeCandidate = String(row.type || "")
        .trim()
        .toLowerCase();
      const type: IntelligenceLabActionType =
        typeCandidate === "tool" || typeCandidate === "agent"
          ? typeCandidate
          : "capability";
      ...
      const capabilityId =
        String(row.capabilityId || "text.chat").trim() || "text.chat";
      return {
        id: createId("action"),
        title,
        type: "capability",
...
function normalizeRiskLevel(
  value: unknown,
): TuffIntelligenceApprovalTicket["riskLevel"] | undefined {
...
function requiresApproval(
  level: TuffIntelligenceApprovalTicket["riskLevel"],
): boolean {
...
export function shouldContinueOnActionFailure(
  ...
  if (globalContinueOnError) return true;
  if (action.continueOnError !== true) return false;
  if (action.type !== "tool" || !action.toolId) return false;
...
async function explainActionNarrativesWithModel(
  ...
  try {
    ...
    throw error instanceof Error
      ? error
      : new Error("Action narratives failed.");
  }
}
...
async function summarizeActionResultWithModel(
  ...
  try {
    ...
    throw error instanceof Error
      ? error
      : new Error("Action result commentary failed.");
  }
}
...
export async function heartbeatIntelligenceLabSession(
  ...
  const session = await touchRuntimeSessionHeartbeat(event, {
    sessionId,
    userId,
  });
  if (!session) {
  ...
  for (const session of sessions) {
    ...
    items.push({
      ...
      pendingActions: Math.max(
        0,
        (checkpointState?.actions.length ?? 0) -
...
export async function planIntelligenceLab(
  ...
  const { result, context, fallbackCount, retryCount } = await invokeModel(
    event,
    userId,
    {
      providerId: payload.providerId,
...
export interface ExecuteIntelligenceLabHooks {
  ...
  shouldPause?: (context: {
    ...
    { paused: boolean; reason?: TuffIntelligencePauseReason } | boolean
  >;
}
...
export async function executeIntelligenceLab(
  ...
  try {
    ...
    const bootstrapPrompt = await buildSystemPrompt(
      event,
      userId,
    ...
    const bootstrapError =
      error instanceof Error ? error : new Error("Executor bootstrap failed.");
  ...
  for (const action of payload.actions) {
    const continueOnActionFailure = shouldContinueOnActionFailure(
      action,
      globalContinueOnError,
    );
    ...
    if (hooks?.shouldPause) {
      ...
      const paused =
        typeof pauseResult === "boolean" ? pauseResult : pauseResult.paused;
      const reason =
        typeof pauseResult === "boolean" ? undefined : pauseResult.reason;
      if (paused) {
        return {
          objective,
    ...
    if (action.type === "tool") {
      ...
      if (!toolId) {
        ...
        if (!continueOnActionFailure) break;
        continue;
      }
      ...
      try {
        const output = await executeTool(toolId, normalizedInput, {
          event,
          userId,
        });
        await pushResult({
    ...
    const capabilityId =
      action.type === "capability"
        ...
        : "text.chat";
    const defaultExecutionInstruction =
      action.type === "agent"
    ...
    const executionPrompt = await buildSystemPrompt(
      event,
      userId,
    ...
    try {
      ...
      const executionError =
        error instanceof Error
          ? error
          : new Error("Capability execution failed.");
      (executionError as { stage?: string; cause?: unknown }).stage =
        action.type === "agent" ? "agent" : "capability";
...
function toRuntimeStatus(
  ...
  if (status === "paused_disconnect") return "paused_disconnect";
...
function normalizeRuntimeCheckpointState(
  value: unknown,
  ...
  return {
    ...
    followUp:
      row.followUp && typeof row.followUp === "object"
        ? {
            summary: String(
              (row.followUp as Record<string, unknown>).summary || "",
            ),
            nextActions: Array.isArray(
              (row.followUp as Record<string, unknown>).nextActions,
            )
              ? (
              ...
              : [],
            revisitInHours: Number(
              (row.followUp as Record<string, unknown>).revisitInHours || 24,
            ),
          }
        : undefined,
    providerId: typeof row.providerId === "string" ? row.providerId : undefined,
    providerName:
      typeof row.providerName === "string" ? row.providerName : undefined,
...
function computeActionStats(results: IntelligenceLabExecutionResult[]): {
  ...
  const waitingApprovals = results.filter(
  ...
  try {
    ...
    const nextActions = Array.isArray(parsed?.nextActions)
      ? parsed.nextActions
          ...
          .slice(0, 5)
      : [];
    const revisitInHours =
      typeof parsed?.revisitInHours === "number" &&
      Number.isFinite(parsed.revisitInHours)
    ...
    throw error instanceof Error
      ? error
      : new Error("Follow-up generation failed.");
  }
}
...
export async function orchestrateIntelligenceLabStream(
  ...
  const heartbeatTimeoutMs = Math.max(
    10_000,
    payload.heartbeatTimeoutMs ?? DEFAULT_HEARTBEAT_TIMEOUT_MS,
  );
  const startedAt = now();
  ...
  const emit = async (
    eventItem: Omit<
      ...
      "timestamp" | "contractVersion" | "engine" | "runId" | "seq" | "phase"
    > & {
      timestamp?: number;
      phase?: string;
    },
  ...
  const formatModelErrorDetail = (error: unknown) => {
  ...
  const resolvePauseReason =
    async (): Promise<TuffIntelligencePauseReason | null> => {
  ...
  let narrativeResult: Awaited<
    ReturnType<typeof explainActionNarrativesWithModel>
  >;
  try {
  ...
  const completedActionIds = new Set(
    mergedResults.map((item) => item.actionId),
  );
      ...
      execution = await executeIntelligenceLab(
        ...
        },
        {
          onActionStart: async (action, context) => {
            ...
            if (!llmNarrative) {
              const narrativeError = new Error(
                `Action narrative missing for ${action.id}`,
              );
              (narrativeError as { stage?: string }).stage =
                "action-narratives";
          ...
          onStep: async (result, context) => {
            ...
            const currentAction =
              actions.find((item) => item.id === result.actionId) || null;
            let resultNarrative: Awaited<
              ReturnType<typeof summarizeActionResultWithModel>
            >;
            try {
              resultNarrative = await summarizeActionResultWithModel(
                event,
                userId,
                {
                  objective,
              ...
              const commentaryError =
                error instanceof Error
                  ? error
                  : new Error("Action result commentary failed.");
              (commentaryError as { stage?: string }).stage =
                "action-result-commentary";
              throw commentaryError;
            }
            const statusMessage =
              resultNarrative.rawModelOutput || resultNarrative.message;
            await emit(
              {
                type: "status",
  ...
  let followUpResult: Awaited<
    ReturnType<typeof generateIntelligenceLabFollowUp>
  >;
  try {
  ...
  const actionStats = computeActionStats(mergedResults);
  const status: IntelligenceLabRuntimeMetrics["status"] =
    actionStats.waitingApprovals > 0
      ? "waiting_approval"
  ...
  try {
    finalReply = await buildFinalAssistantReplyWithModel(
      event,
      userId,
      {
        objective,
  ...
  try {
    await createAudit(event, {
      ...
      errorMessage:
        status === "failed" ? "intelligence-agent-runtime failed" : null,
      traceId: activeTraceId || null,
      metadata: {
        source: "intelligence-agent-runtime",
        sessionId,
...
export async function approveIntelligenceLabTool(
  ...
  const toolId =
    typeof deferredAction.toolId === "string"
      ? deferredAction.toolId
      : ticket.toolId;
...
export function sanitizeLabActions(payload: unknown): IntelligenceLabAction[] {
  ...
  return payload
    .map((item) => {
      ...
      const type: IntelligenceLabActionType =
        typeCandidate === "tool" || typeCandidate === "agent"
          ...
          : "capability";
      const id =
        String(row.id || createId("action")).trim() || createId("action");
      const title =
        String(row.title || "Untitled action").trim() || "Untitled action";
      const input = asRecord(row.input);
      const riskLevel = normalizeRiskLevel(row.riskLevel);
      ...
      return {
        ...
        capabilityId:
          typeof row.capabilityId === "string" ? row.capabilityId : undefined,
        input,
        riskLevel,
    ...
    .filter((action) => {
      if (action.type === "tool") return Boolean(action.toolId);
      if (action.type === "agent") return Boolean(action.agentId);
      return true;
    });
}

export function sanitizeExecutionResults(
  payload: unknown,
): IntelligenceLabExecutionResult[] {
  if (!Array.isArray(payload)) return [];
  return payload.map((item) => {
    ...
    const status: IntelligenceLabExecutionResult["status"] =
      statusRaw === "failed" || statusRaw === "waiting_approval"
        ? statusRaw
        : "completed";
    const typeRaw = String(row.type || "capability")
      .trim()
      .toLowerCase();
    const type: IntelligenceLabActionType =
    ...
    return {
      ...
      approvalTicket: row.approvalTicket as
