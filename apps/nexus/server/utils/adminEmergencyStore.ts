...
function getPepper(event: H3Event): string {
  ...
  const pepper = selectRuntimeCredential(bindings, bindings?.ADMIN_CONTROL_PLANE_PEPPER, [
  ...
  return assertRuntimeCredential('ADMIN_CONTROL_PLANE_PEPPER', pepper, {
    localDevelopment,
    minimumLength: localDevelopment ? 1 : 16,
  })
}
...
export async function createAdminEmergencySession(
  ...
  `,
    ...
    .bind(input.sessionId, input.adminId, input.dfpHash, input.challenge, input.expiresAt, createdAt)
    .run()
...
export async function getAdminEmergencySession(
  event: H3Event,
  sessionId: string,
): Promise<AdminEmergencySession | null> {
  const db = requireDb(event)
  await ensureSchema(db)
  await expireOldSessions(db)
...
export async function updateEmergencySessionVerified(
  ...
  `,
    ...
    .bind(input.sessionId, input.adminId, input.verifyNonce, now)
    .run()
}
...
export async function createEmergencyJti(
  ...
  `,
    ...
    .bind(input.jti, input.sessionId, input.scope, issuedAt, input.expiresAt)
    .run()
}
...
export async function createAdminRecoveryCode(
  ...
  `,
    ...
    .bind(randomUUID(), input.adminId, codeHash, salt, nowIso(), input.expiresAt ?? null)
    .run()
}
...
export async function verifyAndConsumeAdminRecoveryCode(
  ...
  `,
    ...
    .bind(input.adminId, now)
    .all<{ id: string; code_hash: string; salt: string }>()
