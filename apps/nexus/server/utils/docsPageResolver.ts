/**
 * The docs page resolver behind both `/api/docs/page?path=…` (query-string front, kept for
 * development, admin tooling and as the runtime fallback) and the path-shaped
 * `/api/docs/page/<locale>/<mode>/<path>.json` front that the build prerenders into static
 * files. One resolver, one nitro cache key space: the same document answers both URLs.
 */
...
export function normalizeDocsPageLocale(value: unknown): 'en' | 'zh' {
...
export function shouldIncludeDocsPageBody(value: unknown) {
  if (typeof value !== 'string')
    return true
  const normalized = value.trim().toLowerCase()
...
export const resolveDocsPage = cacheDocsContent(async (
  event: H3Event,
  docPath: string,
  locale: 'en' | 'zh',
  includeBody: boolean,
