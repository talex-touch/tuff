...
function toLangChainMessages(messages: IntelligenceProviderAdapterPayload['messages']): BaseMessage[] {
...
function normalizeOpenAiBaseUrl(baseUrl: string): string {
  ...
  const lower = trimmed.toLowerCase()
  return OPENAI_VERSION_SUFFIXES.some(suffix => lower.endsWith(suffix)) ? trimmed : `${trimmed}/v1`
}
...
function createRequestError(message: string, endpoint: string, response: unknown): Error {
...
interface NormalizeResponseMeta {
  ...
  traceId: string
  startedAt: number
  endpoint: string; resultEndpoint: string; emptyMessage: string
}
...
export async function invokeOpenAiCompatibleProviderAdapter(
  ...
  return normalizeResponse(await withTimeout(runner.invoke(toLangChainMessages(messages)), context.timeoutMs), {
    payload,
    traceId,
    startedAt,
