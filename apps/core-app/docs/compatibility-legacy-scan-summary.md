...
- src/renderer/src/components/permission/PermissionStatusCard.vue — trigger: 检测到 legacy SDK 插件; alternative: 升级插件 SDK; risk: low
- src/renderer/src/modules/box/adapter/hooks/useClipboardChannel.ts — trigger: 旧 clipboard 事件/方法被调用; alternative: 使用 TuffTransport ClipboardEvents/getLatestClipboard; risk: medium
- src/renderer/src/modules/hooks/dropper-resolver.ts — trigger: 旧插件格式被触发; alternative: 升级到 .tpex; risk: medium
- src/renderer/src/views/box/tag/FileTag.vue — trigger: 使用 buffer 图标字段; alternative: 使用 iconPath; risk: low
- src/renderer/src/views/base/settings/SettingPermission.vue — trigger: 旧版 SDK 插件统计与提示; alternative: 升级插件 SDK; risk: low
- src/renderer/src/views/base/application/AppList.vue — trigger: 旧排序逻辑注释仍在; alternative: 使用新排序逻辑; risk: low
- src/main/modules/flow-bus/module.ts — trigger: 旧插件集成路径; alternative: TuffTransport handlers; risk: medium
- src/main/modules/plugin/widget/widget-compiler.ts — trigger: legacy compile 接口调用; alternative: processor-based compile; risk: medium
...
- src/main/modules/division-box/session.ts — trigger: @deprecated API 被调用; alternative: getUIView(); risk: medium
...
- src/renderer/src/composables/market/useVersionCompare.ts — trigger: 版本比较门控; alternative: 统一 semver 库; risk: low
