/**
 * Version comparison utilities for plugin store
 *
 * Provides semver-compatible comparison without external dependencies
 */

export interface VersionCompareResult {
  /** -1: installed < store (upgradable), 0: equal, 1: installed > store */
  comparison: -1 | 0 | 1
  /** Whether the store version is newer (upgrade available) */
  hasUpgrade: boolean
  /** Whether the installed version is newer than store */
  isNewer: boolean
...
/**
 * Check if store version is newer than installed version
 */
export function hasUpgradeAvailable(
  ...
  return compareVersions(installedVersion, storeVersion) === -1
}
...
export function getVersionCompareResult(
  ...
  const comparison = compareVersions(installedVersion, storeVersion)
  ...
  return {
    ...
    storeDisplay: formatVersion(storeVersion)
  }
}

/**
 * Composable for version comparison in store context
 */
