 ...
 */
export class NpmPackageProvider extends BaseStoreProvider {
  ...
  async list(options: StoreProviderListOptions = {}): Promise<StorePlugin[]> {
    ...
    try {
      ...
      // Fetch metadata for each package and convert to StorePlugin
  ...
  /**
   * Fetch package metadata and convert to StorePlugin
   */
  private async fetchPackageAsPlugin(packageName: string): Promise<StorePlugin | null> {
    ...
    const install: StoreInstallInstruction = {
      type: 'npm',
