...
function readMigrationMarker(markerPath: string): DevDataMigrationMarker | null {
  try {
    if (!fse.existsSync(markerPath)) return null
    return fse.readJSONSync(markerPath) as DevDataMigrationMarker
  } catch {
    return null
  }
}
...
export function migrateLegacyDevDataIfNeeded(
  ...
  const existingMarker = readMigrationMarker(markerPath)
  ...
  if (existingMarker?.status === 'failed') {
    try {
      fse.removeSync(markerPath)
    } catch {
      // Keep best-effort retry behavior even if marker cleanup fails.
    }
  }
