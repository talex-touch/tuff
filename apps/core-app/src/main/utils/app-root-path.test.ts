...
describe('app-root-path', () => {
  ...
  it('allows retry after a failed migration marker', () => {
    const root = createTempRoot()
    const userDataPath = path.join(root, 'user-data')
    const appPath = path.join(root, 'app-root')
    const sourceRoot = path.join(appPath, APP_FOLDER_NAME)
    ...
    const retryResult = migrateLegacyDevDataIfNeeded(appLike)

    expect(retryResult.status).toBe('migrated')
    expect(fse.pathExistsSync(path.join(targetRoot, 'legacy.txt'))).toBe(true)
    expect(fse.readJSONSync(markerPath).status).toBe('migrated')
  })
})
