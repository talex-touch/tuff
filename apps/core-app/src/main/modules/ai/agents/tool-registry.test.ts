...
describe('agent ToolRegistry Tuff bridge', () => {
  it('keeps legacy registerTool and executeTool behavior', async () => {
    ...
    await expect(registry.executeTool('legacy.echo', { text: 'hello' }, {
      taskId: 'task-1',
      agentId: 'agent-1',
    })).resolves.toEqual({
      success: true,
  ...
  it('converts AgentTool definitions into Tuff tools', async () => {
    ...
    const result = await tool.execute({ value: 3 }, {
      caller: 'agent-1',
      traceId: 'task-1',
    })

    expect(executor).toHaveBeenCalledWith({ value: 3 }, expect.objectContaining({
      taskId: 'task-1',
      agentId: 'agent-1',
    }))
  ...
  it('registers Tuff tools without replacing legacy APIs', async () => {
    const registry = new ToolRegistry()
    registry.registerTuffTool(defineTuffTool({
      id: 'tuff.lower',
      name: 'Lowercase',
    ...
    await expect(registry.executeTool('tuff.lower', { text: 'HELLO' }, {
      taskId: 'task-1',
      agentId: 'agent-1',
    })).resolves.toEqual({
      success: true,
  ...
  it('maps Tuff invocation results back to Agent ToolResult', () => {
    expect(tuffToolResultToAgentToolResult({
      ok: true,
      toolId: 'ok',
      output: 1,
    })).toEqual({
