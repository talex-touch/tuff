...
async function loadPrecompiledWidget(
  ...
  if (sourcePath && (await fse.pathExists(sourcePath))) {
    try {
      ...
      plugin.logger.warn(`[WidgetManager] Failed to verify widget source hash: ${sourcePath}`, error as Error)
    }
  } else {
  ...
  if (entry.metaPath) {
    ...
    if (await fse.pathExists(metaPath)) {
      try {
        ...
        plugin.logger.warn(`[WidgetManager] Failed to read precompiled widget meta: ${metaPath}`, error as Error)
      }
    }
  }
...
export class WidgetManager {
  ...
  private async emitCompiledCache(
    ...
    try {
      ...
      plugin.logger.error('[WidgetManager] Failed to send widget registration event', error as Error)
  ...
  async registerWidget(
    ...
    try {
      ...
      if (isDev && classifyWidgetCompileError(error) === 'WIDGET_COMPILER_SERVICE_UNAVAILABLE') {
        plugin.logger.warn('[WidgetManager] Widget compiler service stopped, retrying once in dev mode')
        try {
          compiled = await compileWidgetSource(source, context)
        } catch (retryError) {
