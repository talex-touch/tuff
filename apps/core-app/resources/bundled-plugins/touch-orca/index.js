...
function normalizeSnapshot(snapshot) {
  if (!snapshot || typeof snapshot !== 'object' || Array.isArray(snapshot))
    return null
  if (!['ready', 'degraded', 'unsupported'].includes(snapshot.status))
    return null
  ...
  for (const key of ['workspaces', 'terminals', 'tasks']) {
    if (!Number.isInteger(snapshot[key]) || snapshot[key] < 0 || snapshot[key] > 100000)
      return null
    result[key] = snapshot[key]
  }
  if (typeof snapshot.tasksAvailable !== 'boolean')
    return null
