...
Options:
  ...
  --allowed-repository <name>   Allowed repository (for example talex-touch/tuff)
...
if [[ -z "$payload_repository" ]]; then
  payload_repository="$(json_get "$PAYLOAD_JSON" "repository.full_name")"
fi
if [[ -z "$payload_repository" ]]; then
  payload_repository="$(json_get "$PAYLOAD_JSON" "github.repository")"
fi
if [[ -n "$ALLOWED_REPOSITORY" && -n "$payload_repository" && "$payload_repository" != "$ALLOWED_REPOSITORY" ]]; then
