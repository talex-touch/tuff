...
2. Sample plugin cleanup：`touch-music` 迁到 TuffEx `base.css` + 子路径组件导入，清理 Vite starter asset；`touch-translation` 同步 storage 注释事实。
