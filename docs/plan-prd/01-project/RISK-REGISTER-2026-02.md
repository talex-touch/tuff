...
|---|---|---|---|---|---|---|---|
| RISK-001 | P0 | 旧同步链路仍可写入（/api/sync/*），且存储明文 JSON | Sync 规则与合规风险，可能引入新功能偏离 v1 | `apps/nexus/server/api/sync/push.post.ts`、`apps/nexus/server/utils/syncStore.ts` | 将 `/api/sync/*` 收口为只读或迁移期禁写；新增写入统一走 `/api/v1/sync/*`；加入 CI/审计守卫 | 2.4.8 | Open |
