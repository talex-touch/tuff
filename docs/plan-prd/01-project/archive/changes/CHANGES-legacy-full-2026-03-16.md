...
### AI：恢复 GitHub -> 1Panel webhook 自动部署（含密钥校验）

**变更类型**: CI/CD 自动化 / 运维安全加固

**描述**:
- 恢复并标准化 AI 自动部署链路：`aiapp-image.yml` 推送 GHCR 成功后，自动 `POST` 1Panel webhook 触发重建。
- 新增 webhook 安全约束：
  - 请求必须携带 `X-AI-Token`（或 `Authorization: Bearer`）；
  - 服务端使用 `AIAPP_WEBHOOK_TOKEN` 严格匹配；
  - 可选仓库/分支白名单：`AIAPP_WEBHOOK_ALLOWED_REPOSITORY`、`AIAPP_WEBHOOK_ALLOWED_BRANCH`。
- 新增轻量 HTTP webhook 服务与状态页：
  - `GET /`：查看最近一次部署状态与输出；
  - `GET /health`：健康检查；
  - `POST /deploy`：接收 payload 并调用部署脚本。
- 兼容入口补齐：根目录 `scripts/` 新增 webhook 兼容脚本与 env 示例，继续转发到 `retired-ai-app/deploy` 真正实现。

**验证结果**:
- `bash -n "retired-ai-app/deploy/deploy-aiapp-1panel-webhook.sh"` ✅
- `python3 -m py_compile "retired-ai-app/deploy/aiapp-deploy-webhook-server.py"` ✅
- `bash -n "scripts/deploy-aiapp-1panel-webhook.sh"` ✅

**修改文件（关键）**:
- `.github/workflows/aiapp-image.yml`
- `.github/workflows/README.md`
- `retired-ai-app/deploy/deploy-aiapp-1panel-webhook.sh`
- `retired-ai-app/deploy/deploy-aiapp-1panel-webhook.env.example`
- `retired-ai-app/deploy/aiapp-deploy-webhook-server.py`
- `retired-ai-app/deploy/aiapp-deploy-webhook.service.example`
- `retired-ai-app/deploy/README.md`
- `retired-ai-app/deploy/README.zh-CN.md`
- `scripts/deploy-aiapp-1panel-webhook.sh`
- `scripts/deploy-aiapp-1panel-webhook.env.example`
...
**主要变更**:
1. `aiapp-system-resource` 的树构建新增兼容归一化：同时支持“嵌套 children”与“扁平行 parentId”两种存储形态。
...
**验证结果**:
- `pnpm -C "retired-ai-app" run test -- --runInBand` ✅（包含新增多模态消息形态断言）
...
- 六主文档与生态文档同步回填：
  ...
  - `AIAPP-INTELLIGENCE-API-CONTRACT` 更新为当前主路径（Node Server + Postgres/Redis + JWT Cookie + MinIO），Cloudflare/D1/R2 切为历史语境；
...
- CI run：
  ...
  - AI Image Publish: https://github.com/talex-touch/tuff/actions/runs/23125828532（completed/failure）
  - AI CI: https://github.com/talex-touch/tuff/actions/runs/23125828545（completed/success）
...
- 活跃文档对齐：
  ...
  - `AIAPP-NEXUS-OAUTH-CLI-TEST-PLAN` 重写为“已落地 vs 未启动”快照，明确 `tuff-ai-cli` 仍为 backlog；
...
- 抽取 `server/utils/aiapp-title.ts`，统一 `executor` 与 `title` 接口的标题生成逻辑与短 prompt。
- `aiapp-runtime` 默认系统提示压缩为短、稳、可执行导向文案。
...
- 已记录并核对发布事实：
  ...
  - CI 结果：
    ...
    - AI Image Publish: https://github.com/talex-touch/tuff/actions/runs/23106610203
...
### AI：M2/M3 接口迁移收口（含聊天应用豁免 + 支付 3 秒自动结算）

**变更类型**: API 兼容迁移 / 豁免策略落地 / 运维脚本补齐

**描述**:
- 基于 `g-wggu...ends` 与 `g-wggu...view` 口径，AI 侧完成 M2/M3 迁移收口：
...
- 渠道治理收口：
  - 新增 `ends -> aiapp` 渠道合并能力，按 `id` 去重、AI 优先、Ends 仅补缺。
  - 新增管理端触发接口：`POST /api/aiapp/admin/channels/merge-ends`。
  - 新增一次性脚本：`pnpm -C "retired-ai-app" run channels:merge:ends`。
  - 渠道敏感字段继续通过 `AIAPP_CONFIG_ENCRYPTION_KEY` 加密存储。
- 目录治理：
  - `.gitignore` 新增忽略：`apps/g-wggu5114-thisai-thisai-ends-/`、`apps/g-wggu5114-thisai-thisai-view-/`。

**验证结果**:
- 前端 endpoint 对照复核：从 `g-wggu...view` 抽取的 `endHttp` 调用（101 条）已全部在 `retired-ai-app/server/api` 匹配到明确路由，未发现 501 落兜底路径。
- `pnpm -C "retired-ai-app" run test` ✅
- `pnpm -C "retired-ai-app" run typecheck` ⚠️（存在大量存量错误；本次新增 `merge-ends` 与 channel 类型约束相关错误已清理）

**修改文件（关键）**:
- `.gitignore`
- `retired-ai-app/server/utils/aiapp-compat-store.ts`
- `retired-ai-app/server/utils/aiapp-compat-payment.ts`
- `retired-ai-app/server/utils/aiapp-compat-seeds.ts`
- `retired-ai-app/server/utils/aiapp-system-resource.ts`
- `retired-ai-app/server/utils/aiapp-compat-aigc.ts`
- `retired-ai-app/server/utils/aiapp-local-auth.ts`
- `retired-ai-app/server/utils/aiapp-admin-channel-config.ts`
- `retired-ai-app/server/utils/aiapp-channel-merge-ends.ts`
- `retired-ai-app/server/api/aiapp/admin/channels/merge-ends.post.ts`
- `retired-ai-app/scripts/merge-ends-channels.mjs`
- `retired-ai-app/package.json`
- `retired-ai-app/server/api/system/**`
- `retired-ai-app/server/api/aigc/**`
- `retired-ai-app/server/api/order/**`
- `retired-ai-app/server/api/coupon/**`
- `retired-ai-app/server/api/doc/**`
- `retired-ai-app/server/api/feedback/**`
- `retired-ai-app/server/api/marketing/banner/**`
- `retired-ai-app/server/api/subscribe/**`
- `retired-ai-app/server/api/livechat/**`
- `retired-ai-app/server/api/platform/**`
- `retired-ai-app/server/api/auth/**`
- `docs/plan-prd/TODO.md`
- `docs/plan-prd/01-project/CHANGES.md`
- `docs/INDEX.md`
...
### AI：附件空正文提示 + MinIO env 直连兜底

**变更类型**: 交互可读性修复 / 存储配置增强

**描述**:
- 聊天输入仅上传附件且未输入文本时，自动补充正文占位 `"(无正文内容)"`，避免消息气泡出现空正文。
- 新旧聊天入口统一该行为：`aiapp` 新会话页与旧 QuotaGPTView 输入框在“仅附件发送”场景都会补齐占位文本。
- 附件存储增加 MinIO 环境变量直连兜底：当后台存储配置未设置时，可直接通过 env 启用 `s3/minio` 写入。
- 支持 env 键：`AIAPP_ATTACHMENT_PROVIDER`、`AIAPP_ATTACHMENT_PUBLIC_BASE_URL`、`AIAPP_ATTACHMENT_SIGNING_SECRET`、`AIAPP_MINIO_*`。
- 签名密钥优先读取 `AIAPP_ATTACHMENT_SIGNING_SECRET`，为空时回退 `AIAPP_COOKIE_SECRET`。
- 当 provider 设为 `s3/minio` 但配置缺失或上传失败时，服务端自动回退到 `memory` 并输出日志，优先保证附件功能可用。
- 上传大小限制统一为 **10MB**（旧 `tools/upload` 与新 `aiapp` 会话附件上传接口都生效），前端同步拦截超限文件。
...
- `retired-ai-app/server/utils/aiapp-attachment-storage.ts`
- `retired-ai-app/server/utils/__tests__/aiapp-attachment-storage.test.ts`
- `retired-ai-app/.env.example`
- `retired-ai-app/deploy/deploy-aiapp-1panel.env.example`
- `retired-ai-app/deploy/README.md`
- `retired-ai-app/deploy/README.zh-CN.md`
- `docs/plan-prd/01-project/CHANGES.md`

### AI：邮箱登录自动注册 + 仅附件消息可发送
...
- 新 AI 会话链路支持“仅附件发送”（空文本 + 附件），并同步修复流式接口校验与运行时 turn 判定，避免附件消息被拒绝。
- 旧 QuotaGPTView 输入框发送条件优化：仅在“文件仍在上传中”时禁发；已失败文件不再阻塞文本/已上传文件发送。
- 登录弹窗移除邮箱注册 tab，邮箱登录按钮改为“登录 / 自动注册”并补充提示文案，降低使用歧义。

**修改文件**:
- `retired-ai-app/server/api/auth/email/login.post.ts`
- `retired-ai-app/server/api/aiapp/chat/sessions/[sessionId]/stream.post.ts`
...
- `retired-ai-app/app/components/input/ThInput.vue`
- `retired-ai-app/app/components/chore/Login.vue`
- `docs/plan-prd/01-project/CHANGES.md`
...
### AI：管理员引导密码改为强制 env 注入（最小 6 位）

**变更类型**: 安全基线收敛 / 配置约束增强

**描述**:
- `aiapp-bootstrap-admin` 不再使用默认管理员密码，`AIAPP_BOOTSTRAP_ADMIN_PASSWORD` 必须显式提供。
- 启动时管理员密码最小长度约束从 3 位提升到 6 位；不满足时跳过 bootstrap 并输出提示日志。
- 部署与示例配置同步：移除 `admin` 默认回退，模板改为 `replace-with-admin-password-min-6` 占位值。

**修改文件**:
- `retired-ai-app/server/plugins/aiapp-bootstrap-admin.ts`
- `retired-ai-app/.env.example`
- `retired-ai-app/deploy/deploy-aiapp-1panel.env.example`
- `retired-ai-app/deploy/deploy-aiapp-1panel.sh`
- `retired-ai-app/deploy/README.md`
- `retired-ai-app/deploy/README.zh-CN.md`
- `docs/plan-prd/01-project/CHANGES.md`

### AI：显式 env 加载优先级固化（`.env.local > .env.prod > .env.dev > .env`）

**变更类型**: 配置行为固化 / 本地开发一致性修复

**描述**:
- 在 `retired-ai-app/nuxt.config.ts` 增加显式 env 文件加载逻辑，不再依赖 Nuxt 默认加载顺序。
- 按固定顺序覆盖：先加载 `.env`，再 `.env.dev`，再 `.env.prod`，最后 `.env.local`，确保最终优先级为 `.env.local > .env.prod > .env.dev > .env`。
- 新增共享加载器 `retired-ai-app/shared/aiapp-env-loader.ts`，并在 Postgres DSN 解析前强制一次性应用优先级，避免 Nitro 运行时与 Nuxt 配置阶段出现读取不一致。
- 修复“本地已在 `.env.local` 配置但默认 `pnpm run dev` 仍报 Postgres 密码错误”的不确定行为。

**修改文件**:
- `retired-ai-app/nuxt.config.ts`
- `retired-ai-app/shared/aiapp-env-loader.ts`
- `retired-ai-app/server/utils/aiapp-node-pg-d1.ts`
- `docs/plan-prd/01-project/CHANGES.md`

### AI：渠道不可用诊断增强（503 可观测性修复）

**变更类型**: 运行时诊断增强 / 错误信息可观测性

**描述**:
- `resolveAIChannelSelection` 在无可用渠道时输出结构化日志（包含 `db` 目标、配置渠道列表、启用渠道列表、默认渠道、请求渠道）。
...
- `retired-ai-app/server/plugins/aiapp-channel-bootstrap-check.ts`
- `docs/plan-prd/01-project/CHANGES.md`

### AI：Postgres 兼容性修复（登录合并链路）
...
- `retired-ai-app/server/utils/aiapp-guest-merge.ts`
- `docs/plan-prd/01-project/CHANGES.md`

### AI：管理后台新增渠道设置页（复用存储页风格）

**变更类型**: 管理能力补齐 / 配置可视化

**描述**:
- 新增 ` /aiapp/admin/channels ` 页面，支持增删渠道、默认渠道选择、超时与工具配置。
...
**修改文件**:
- `retired-ai-app/app/pages/aiapp/admin/channels.vue`
- `retired-ai-app/app/pages/aiapp/admin/storage.vue`
- `retired-ai-app/app/components/aiapp/AISessionsPanel.vue`
- `retired-ai-app/server/utils/aiapp-admin-channel-config.ts`
- `retired-ai-app/server/utils/aiapp-config-crypto.ts`
- `docs/plan-prd/01-project/CHANGES.md`

### AI：旧 QuotaGPTView 流式 Markdown 实时性优化（executor 链路）
...
- `retired-ai-app/app/components/article/MilkContent.vue`
- `retired-ai-app/app/components/chat/ThChat.vue`
- `retired-ai-app/app/components/render/RenderContent.vue`
...
- 修复关键冲突：
  ...
  - `AI Runtime` 统一为 `Node Server + Postgres/Redis` 主路径；
...
### CI：修复 AI PNPM 版本冲突 + core-app 构建对 deepagents 的错误引入

**变更类型**: CI 稳定性修复 / 构建边界收敛

**描述**:
- 修复 `AI CI` 在 `pnpm/action-setup@v4` 阶段失败的问题：
...
### AI：环境变量收敛清单固化（Postgres + Redis + DB Config）

**变更类型**: 配置治理 / 部署简化 / 兼容收口

**描述**:
- 运行时环境变量进一步收敛到最小集合：
  - 必填：`AIAPP_POSTGRES_URL`、`AIAPP_REDIS_URL`、`AIAPP_JWT_ACCESS_SECRET`、`AIAPP_JWT_REFRESH_SECRET`、`AIAPP_COOKIE_SECRET`、`AIAPP_CONFIG_ENCRYPTION_KEY`、`AIAPP_BOOTSTRAP_ADMIN_PASSWORD`。
  - 可选：`AIAPP_BOOTSTRAP_ADMIN_EMAIL`、`AIAPP_EXECUTOR_DEBUG`、`NUXT_PUBLIC_NEXUS_ORIGIN`、`AIAPP_NEXUS_OAUTH_CLIENT_ID`、`AIAPP_NEXUS_OAUTH_CLIENT_SECRET`。
...
- 1Panel 部署脚本改为“自动探测优先”：
  - 不再依赖 `AIAPP_PROJECT_DIR/AIAPP_COMPOSE_FILE/AIAPP_SERVICE_NAME` 等定位 env；
  - 部署侧仅保留 `AIAPP_IMAGE_TAG` 与健康检查变量（`AIAPP_HEALTHCHECK_*` + `AIAPP_ROLLBACK_ON_FAILURE`）。
- 部署示例 env 同步精简；如需首次生成 compose，改为命令参数显式触发 `--bootstrap-compose`。

**修改文件（核心）**:
- `retired-ai-app/.env.example`
- `retired-ai-app/nuxt.config.ts`
- `retired-ai-app/server/utils/aiapp-session.ts`
- `retired-ai-app/deploy/deploy-aiapp-1panel.sh`
- `retired-ai-app/deploy/deploy-aiapp-1panel.env.example`
- `retired-ai-app/deploy/README.md`
- `retired-ai-app/deploy/README.zh-CN.md`
- `scripts/deploy-aiapp-1panel.env.example`

### AI：环境变量收敛 + Node Runtime 固化（Postgres/Redis + JWT 双 Token）

**变更类型**: 架构收敛 / 部署链路清理 / 认证升级

**描述**:
- `retired-ai-app` 运行时收敛为 Node Server，移除 Cloudflare 运行分支与 wrangler 相关配置；Nuxt Nitro 固定 `node-server`。
- 数据与会话依赖收敛为强制 `Postgres + Redis`：
  - 移除 `sqlite/D1` 路径与旧别名变量（`AIAPP_DB_DRIVER`、`AIAPP_DB_FILE`、`AIAPP_PG_DSN`、`DATABASE_URL`）。
...
- 管理员引导策略调整：
  - 默认账号 `admin@aiapp.local / admin`（可由 env 覆盖）。
  - 启动日志不再输出管理员明文密码。
- 渠道与附件配置改为数据库真源：
  - `channels` 从 env 迁移到 admin DB 配置，并增加 `AIAPP_CONFIG_ENCRYPTION_KEY` 对敏感字段加密。
  - 附件存储移除 R2/Cloudflare 分支，仅保留 `memory/s3(minio)`，并移除 `AIAPP_S3_*` 别名。
- 部署资产收敛到 1Panel 脚本：
...
- `retired-ai-app/server/utils/aiapp-channel.ts`
- `retired-ai-app/server/utils/aiapp-admin-channel-config.ts`
- `retired-ai-app/server/utils/aiapp-admin-storage-config.ts`
- `retired-ai-app/server/utils/aiapp-attachment-storage.ts`
- `retired-ai-app/server/utils/aiapp-store.ts`
- `retired-ai-app/nuxt.config.ts`
- `retired-ai-app/package.json`
- `retired-ai-app/deploy/*`
- `retired-ai-app/.env.example`

### AI：legacy responses SSE 兼容修复（`Cannot use 'in' operator`）

**变更类型**: 稳定性修复 / 协议兼容增强

**描述**:
- 修复 AI 在 `adapter=legacy` + `transport=responses` 渠道下，偶发抛出
...
- 全仓 direct `fetch/axios` 清零（network 套件内部除外）：
  ...
  - 迁移 `retired-ai-app`（`useAIChatPage` JSON/SSE 请求统一改为 `networkClient`）。
...
- ESLint 硬禁补齐（最小必要范围）：
  - `apps/nexus`、`retired-ai-app`、`packages/tuff-intelligence`、`packages/tuffex`、`packages/unplugin-export-plugin`、`plugins/touch-{translation,image,music}` 新增 `no-restricted-imports(axios)` + `no-restricted-syntax(fetch)`。
- 修复迁移引入的类型回归：
  - `packages/utils/common/utils/time.ts` 的 retrier 返回值泛型推导修正（`withTimeout<T>`）。
  - `retired-ai-app/app/composables/useAIChatPage.ts` 与 `packages/tuff-intelligence/src/adapters/deepagent-engine.ts` 补齐 `NetworkMethod` 映射。
...
- `pnpm -C "retired-ai-app" exec eslint app/composables/useAIChatPage.ts` ✅
- `pnpm -C "packages/tuff-intelligence" exec eslint src/adapters/deepagent-engine.ts` ✅
- `pnpm -C "packages/tuff-intelligence" exec tsc --noEmit` ✅
- `pnpm -C "packages/tuffex" exec eslint packages/components/src/icon/src/TxIcon.vue` ✅
- `pnpm -C "retired-ai-app" run typecheck` ⚠️（存在大量历史存量错误，当前改动相关错误已清理）
...
- `retired-ai-app/eslint.config.js`
...
### AI 登录简化：本地邮箱优先 + Nexus 并存（M1.4）

**变更类型**: 认证链路收敛 / 体验修复 / 数据一致性增强

**描述**:
- 新增本地邮箱认证能力（`/api/auth/email/register`、`/api/auth/email/login`、`/api/auth/logout`），统一写入 `aiapp_auth_session` Cookie，不引入邮箱验证码流程。
...
- 原短信/聊天应用二维码登录入口改为“即将上线（Nexus）”占位，避免调用当前未迁移接口导致误报。
- 新增访客数据并入：登录成功后基于 device guest id 自动迁移会话/历史/运行时记录到登录账号，并输出后端合并审计日志。

**验证**:
- `pnpm -C "retired-ai-app" run test` ✅
- `pnpm -C "retired-ai-app" run build` ✅
- `pnpm -C "retired-ai-app" run typecheck` ⚠️（存在大量存量 TS 错误，集中在 Quota 旧页面与 CMS 模块，与本次登录改造无直接耦合）
- `pnpm -C "retired-ai-app" run lint` ⚠️（存在存量 lint debt，已在 `TODO.md` 的 M0 收口条目持续跟踪）

**修改文件**:
- `retired-ai-app/server/utils/aiapp-local-auth.ts`
- `retired-ai-app/server/utils/aiapp-guest-merge.ts`
- `retired-ai-app/server/api/auth/email/register.post.ts`
- `retired-ai-app/server/api/auth/email/login.post.ts`
- `retired-ai-app/server/api/auth/logout.post.ts`
- `retired-ai-app/server/api/auth/status.get.ts`
- `retired-ai-app/server/api/account/profile.get.ts`
- `retired-ai-app/server/routes/auth/callback.get.ts`
- `retired-ai-app/app/components/chore/Login.vue`
- `retired-ai-app/app/app.vue`
- `retired-ai-app/app/composables/user.ts`
- `retired-ai-app/app/composables/api/base/v1/auth.ts`
- `retired-ai-app/app/composables/api/auth.ts`
- `retired-ai-app/server/utils/__tests__/aiapp-local-auth.test.ts`
- `docs/plan-prd/01-project/CHANGES.md`

### AI M1.3：渠道 Adapter 化（禁用自动协议降级，legacy 固定 responses）

**变更类型**: 架构收敛 / 兼容策略调整

**描述**:
- 根据渠道差异，后端从“自动降级”切换为“按渠道 Adapter 显式路由”：
  - 新增 `adapter` 语义（`legacy | openai`），在 `AIAPP_CHANNELS_JSON` 可按渠道配置。
  - `legacy` 渠道强制走 `responses`，不会再尝试 `chat.completions`。
  - `openai` 渠道按 `transport` 显式选择（`responses | chat.completions`），不再隐式 fallback。
- 默认渠道策略可配置：
  - 新增 `AIAPP_CHANNEL_DEFAULT_ADAPTER`（默认 `legacy`）。
  - 保留 `AIAPP_CHANNEL_DEFAULT_TRANSPORT`，但在 `legacy` 下会被收敛为 `responses`。
...
- 诊断增强：
  ...
  - `executor` 错误事件新增 `detail` 结构（`status_code/status_message/endpoint/model/phase/cause`），并在后端输出 `[aiapp-executor-error]` 结构化日志，便于排查 `Connection error` 与网关链路问题。
- 管理接口增强：
  - `GET /api/aiapp/admin/channels` 返回 `adapter` 字段，便于后台排查渠道配置。

**验证**:
- `pnpm -C "retired-ai-app" run test` ✅
- `pnpm -C "retired-ai-app" run build` ✅（本次改动文件通过构建）
...
- `retired-ai-app/server/api/aigc/executor.post.ts`
- `retired-ai-app/server/api/aiapp/chat/sessions/[sessionId]/stream.post.ts`
- `retired-ai-app/server/api/aiapp/admin/channels.get.ts`
- `docs/plan-prd/01-project/CHANGES.md`

### AI M1.2：Executor 超时治理（渠道超时配置化 + 自动降级 + 可诊断错误）
...
- 修复 `POST /api/aigc/executor` 在 `responses` 通道下频繁返回 `Request timed out.` 的问题根因：
  ...
  - 支持全局变量 `AIAPP_CHANNEL_TIMEOUT_MS`，并允许 `AIAPP_CHANNELS_JSON` 渠道项单独指定 `timeoutMs`（未指定时继承全局默认）。
- 自动协议降级增强：
  ...
  - 注：该策略已在同日 `AI M1.3` 切换为 Adapter 显式路由（默认不再自动降级）。
- SSE 诊断信息增强：
  - `session_bound` 事件新增 `timeout_ms` 字段，便于定位当前渠道阈值。
  - 错误消息对超时场景改为可读提示：包含超时阈值与渠道信息，不再只有笼统 `Request timed out.`。
- 管理接口增强：
  - `GET /api/aiapp/admin/channels` 增加 `timeoutMs` 脱敏可见字段。

**验证**:
- `pnpm -C "retired-ai-app" run test` ✅
- `pnpm -C "retired-ai-app" run build` ✅（本次改动文件通过类型与构建校验）
...
### AI M0：GHCR 镜像发布链路落地（master + packages 触发）

**变更类型**: 工程化增强 / 发布链路补齐

**描述**:
- 新增 `retired-ai-app/Dockerfile`，将 AI 构建链路固定为 `NITRO_PRESET=node-server`，产出可直接运行的容器镜像。
...
- 新增 `.github/workflows/aiapp-image.yml`：
  - 触发条件：`master` push 且命中 `retired-ai-app/**`、`packages/**`、锁文件或工作流自身改动。
  - 发布目标：`ghcr.io/<owner>/tuff-ai`。
  - 标签策略：`aiapp-<short_sha>`（不可变）+ `aiapp-latest`（环境跟随）。
...
- `aiapp-ci.yml` 下线 `deploy-1panel` 任务，避免与镜像发布链路重复触发；1Panel 触发统一收口到 `aiapp-image.yml`。
- 工作流文档同步更新 `.github/workflows/README.md`。

**验证**:
- `pnpm -C "retired-ai-app" run build` ✅（在本地以 `NITRO_PRESET=node-server` 验证构建可通过）

**修改文件**:
- `.dockerignore`
- `retired-ai-app/Dockerfile`
- `.github/workflows/aiapp-image.yml`
- `.github/workflows/aiapp-ci.yml`
- `.github/workflows/README.md`
- `docs/plan-prd/01-project/CHANGES.md`

### AI M0：1Panel 部署脚本（GHCR 拉取 + 健康检查回滚）

**变更类型**: 工程化增强 / 部署可靠性提升

**描述**:
- 新增 `retired-ai-app/deploy/deploy-aiapp-1panel.sh`，用于在 1Panel 服务器执行 AI 容器更新。
...
- 新增 `retired-ai-app/deploy/deploy-aiapp-1panel-webhook.sh`，支持 webhook payload 解析（`image/tag/sha`）与 `sha -> aiapp-<short_sha>` 自动映射，并转发到主部署脚本。
- 新增部署模板与双语教程：
  - `retired-ai-app/deploy/deploy-aiapp-1panel.env.example`
  - `retired-ai-app/deploy/deploy-aiapp-1panel-webhook.env.example`
  - `retired-ai-app/deploy/README.zh-CN.md`
  - `retired-ai-app/deploy/README.md`
- 根目录 `scripts/deploy-aiapp-1panel.sh` 与 `scripts/deploy-aiapp-1panel-webhook.sh` 保留为兼容入口，转发到 `retired-ai-app/deploy/`。

**验证**:
- `bash -n "retired-ai-app/deploy/deploy-aiapp-1panel.sh"` ✅
- `retired-ai-app/deploy/deploy-aiapp-1panel.sh --help` ✅
- `bash -n "retired-ai-app/deploy/deploy-aiapp-1panel-webhook.sh"` ✅
- `retired-ai-app/deploy/deploy-aiapp-1panel-webhook.sh --dry-run --payload-json '{"repository":"talex-touch/tuff","sha":"abcdef123456","branch":"master"}'` ✅
...
- `retired-ai-app/deploy/README.zh-CN.md`
- `retired-ai-app/deploy/README.md`
- `scripts/deploy-aiapp-1panel.sh`
- `scripts/deploy-aiapp-1panel.env.example`
- `scripts/deploy-aiapp-1panel-webhook.sh`
- `scripts/deploy-aiapp-1panel-webhook.env.example`
- `docs/plan-prd/01-project/CHANGES.md`
...
### AI M1：多渠道 + Completions 兼容 + 后端会话主导（第一批 API 拆分）

**变更类型**: 融合增强 / 兼容迁移 / 稳定性修复

**描述**:
- 新增渠道解析与协议兼容层（`retired-ai-app/server/utils/aiapp-channel.ts`）：
  - 支持 `AIAPP_CHANNELS_JSON` 与 `AIAPP_DEFAULT_CHANNEL_ID`（含 runtime/env 回退）。
...
- 聊天执行器 `POST /api/aigc/executor` 完成 M1 升级（`retired-ai-app/server/api/aigc/executor.post.ts`）：
...
- 新增后端会话映射真源（`retired-ai-app/server/utils/aiapp-quota-session.ts`）：
  - 表 `aiapp_quota_sessions(chat_id/user_id/runtime_session_id/channel_id/topic/created_at/updated_at)`。
  - 删除会话时统一清理：`quota history + session mapping + runtime session`，避免脏数据。
- 兼容接口语义收口：
  - `POST /api/aigc/conversations` 调整为“兼容补写/覆盖元数据”，不再作为主同步通道。
  - `GET /api/aiapp/admin/channels` 新增 env 渠道脱敏查看；`POST /api/aiapp/admin/channels` 在 M1 返回 501（保持 env 只读策略）。
...
- 延续 M0 既定口径：
  - 路由继续保持 `/ -> Quota`、`/aiapp/* -> 原 AI`。
  - 认证主链路继续使用 AI Cookie 会话/访客模式（Bearer Token 非主路径）。
  - 兼容 API 继续输出 `{ code, message, data }`。
- 前端错误态兼容增强（`retired-ai-app/app/composables/api/base/v1/aigc/completion/index.ts` + `ErrorCard.vue`）：
...
**验证**:
- `pnpm -C "retired-ai-app" run test` ✅
- `NODE_OPTIONS=--max-old-space-size=8192 pnpm -C "retired-ai-app" run build` ✅
- `pnpm -C "retired-ai-app" run typecheck` ⚠️（Quota 迁移存量类型错误仍在，未新增阻塞）
- `pnpm -C "retired-ai-app" run lint` ⚠️（Quota 迁移存量 lint 债务仍在，规则量级较大）

### AI M1.1：构建与部署瘦身（sourcemap 关闭 + 静态发布剔除 worker）

**变更类型**: 性能优化 / 部署可靠性增强

**描述**:
- `retired-ai-app/nuxt.config.ts` 生产构建默认关闭 client/server sourcemap（避免发布产物被 `.map` 主导）。
- `retired-ai-app/uno.config.ts` 调整 WebFonts 策略：仅当 `NUXT_WEB_FONTS=true` 时启用，默认不走远程字体抓取，降低构建网络依赖。
- `retired-ai-app/package.json` 的 `prepare:cf-static` 新增 `rm -rf dist/_worker.js`，静态发布时显式剔除 worker 产物，规避 Cloudflare Worker 体积门槛影响。

**验证**:
- `pnpm -C "retired-ai-app" run generate` ✅
- `pnpm -C "retired-ai-app" run prepare:cf-static` ✅
- `node retired-ai-app/scripts/report-dist-size.mjs` ✅（`dist/_nuxt` 从约 `51.36MB` 降至 `12.66MB`）

**修改文件**:
- `retired-ai-app/nuxt.config.ts`
- `retired-ai-app/uno.config.ts`
- `retired-ai-app/package.json`
- `docs/plan-prd/01-project/CHANGES.md`
...
- `retired-ai-app/server/utils/aiapp-channel.ts`
- `retired-ai-app/server/utils/aiapp-runtime.ts`
- `retired-ai-app/server/utils/aiapp-quota-session.ts`
- `retired-ai-app/server/utils/quota-conversation-snapshot.ts`
- `retired-ai-app/server/utils/quota-share-store.ts`
- `retired-ai-app/server/utils/quota-user-store.ts`
- `retired-ai-app/server/utils/quota-upload-store.ts`
- `retired-ai-app/server/api/aigc/executor.post.ts`
- `retired-ai-app/server/api/aigc/conversations.post.ts`
- `retired-ai-app/server/api/aigc/conversations/[id].delete.ts`
- `retired-ai-app/server/api/aiapp/chat/sessions/[sessionId]/stream.post.ts`
- `retired-ai-app/server/api/aiapp/admin/channels.get.ts`
- `retired-ai-app/server/api/aiapp/admin/channels.post.ts`
- `retired-ai-app/server/api/aigc/prompts/user.get.ts`
- `retired-ai-app/server/api/aigc/prompts/detail/[id]/index.get.ts`
- `retired-ai-app/server/api/aigc/conversation/share/[id]/index.get.ts`
- `retired-ai-app/server/api/aigc/conversation/share/[id]/index.post.ts`
- `retired-ai-app/server/api/aigc/conversation/share_chat/[id]/index.get.ts`
- `retired-ai-app/server/api/aigc/conversation/share_list.get.ts`
- `retired-ai-app/server/api/auth/renew_token.get.ts`
- `retired-ai-app/server/api/user-config.get.ts`
- `retired-ai-app/server/api/user-config.post.ts`
- `retired-ai-app/server/api/user-config/user/[uid]/index.get.ts`
- `retired-ai-app/server/api/dummy/*`
- `retired-ai-app/server/api/invitation/records.get.ts`
- `retired-ai-app/server/api/order/*`
- `retired-ai-app/server/api/tools/upload.post.ts`
- `retired-ai-app/server/api/tools/upload/content/[id]/index.get.ts`
- `packages/tuff-intelligence/src/adapters/deepagent-engine.ts`
- `docs/plan-prd/01-project/CHANGES.md`

## 2026-03-10

### AI M0: 聊天未渲染修复（SSE 分片解析 + Milkdown 时序 + 运行时缺包）
...
- 修复 dev 运行时缺包导致首页 500：
  - `retired-ai-app/package.json` 显式补齐 `@langchain/core`、`@langchain/langgraph`、`@langchain/openai`、`deepagents`。
- 统一 `retired-ai-app` 中 `@milkdown/*` 主干依赖版本并显式声明 `@milkdown/utils`，降低 token 版本漂移风险。
...
- `executor` 对上游 `530 status code` 增加可读错误提示，明确指向 `NUXT_AIAPP_BASE_URL` / `NUXT_AIAPP_API_KEY` 配置排查。

**验证**:
- `pnpm -C "retired-ai-app" exec eslint app/components/article/MilkContent.vue app/composables/api/base/v1/aigc/completion/index.ts` ✅
- `curl http://127.0.0.1:<dev-port>/api/auth/status` ✅
- `curl -N POST /api/aigc/executor` SSE 实测输出 `status_updated -> completion -> [DONE]` ✅
- `pnpm -C "retired-ai-app" run typecheck` ⚠️（仍有 Quota 迁移存量类型错误，与本次修复无关）
...
### AI M0: 前端冷启动加速 + CI/1Panel 部署链路补齐

**变更类型**: 性能优化 / 工程化增强

**描述**:
- 启动链路优化（`retired-ai-app`）：
...
- 体积分析能力补齐：
  - 新增 `pnpm -C "retired-ai-app" run analyze:size` 与 `scripts/report-dist-size.mjs`，输出 `dist/_nuxt` 总体积、扩展名占比、Top 20 大文件。
  - 实测重块集中在 `@antv/x6`、`abcjs`、`echarts`、`element-plus`、`Milkdown` 相关 chunk。
- CI/部署链路补齐：
  - 新增 `.github/workflows/aiapp-ci.yml`：
...
**验证**:
- `pnpm -C "retired-ai-app" run test` ✅
- `pnpm -C "retired-ai-app" run build` ✅（8GB 堆内存下通过）
- 本地冷启动基线对比（`/` 首次可访问）：
  - 优化前约 `132s ~ 218s`
  - 优化后约 `9s ~ 10s`
...
- `retired-ai-app/nuxt.config.ts`
- `retired-ai-app/uno.config.ts`
- `retired-ai-app/app/components/article/components/EditorCodeBlock.vue`
- `retired-ai-app/scripts/report-dist-size.mjs`
- `.github/workflows/aiapp-ci.yml`
- `.github/workflows/README.md`
- `docs/plan-prd/01-project/CHANGES.md`
- `docs/plan-prd/TODO.md`
...
### AI M0: Cloudflare 部署改为静态发布（规避 Worker 3MiB 限制）

**变更类型**: 部署策略调整 / 线上可用性修复

**描述**:
- Cloudflare Pages 免费额度下，`nuxt build` 产物中的 `_worker.js` 模块体积约 `13MB`，发布时报错 `Worker exceeded 3 MiB`。
- 为保证 M0 可上线，`retired-ai-app` 的 Cloudflare 脚本切换为静态发布链路：
...
- 静态发布补齐路由兜底：
  ...
  - 自动生成 `dist/index.html`、`dist/aiapp/index.html`、`dist/aiapp/admin/storage/index.html`（由 `200.html` 复制），并写入 `/* /index.html 200` 到 `_redirects`，避免 ` /`、`/aiapp`、`/aiapp/admin/storage` 线上白屏/404。
- 新增运行时 API 基地址注入：
  - `runtimeConfig.public.endsBaseUrl`（优先 `NUXT_PUBLIC_ENDS_URL`，回退 `NUXT_AIAPP_BASE_URL`）
...
**测试**:
- `pnpm -C "retired-ai-app" run generate` ✅
- `npx wrangler pages deploy .output/public --project-name tuff-ai --branch main` ✅

**修改文件**:
- `retired-ai-app/package.json`
- `retired-ai-app/nuxt.config.ts`
- `retired-ai-app/app/app.vue`
- `retired-ai-app/wrangler.toml`
- `docs/plan-prd/01-project/CHANGES.md`

## 2026-03-09

### AI M0: 聊天渲染链路修复（Milkdown `SchemaReady` 冲突）
...
**测试**:
- `pnpm -C "retired-ai-app" exec nuxi prepare` ✅
- `pnpm -C "retired-ai-app" run dev` ✅（本地构建通过，可进入聊天页面）

**修改文件**:
- `retired-ai-app/app/components/article/MilkdownRender.vue`
- `retired-ai-app/app/components/article/ThContent.vue`
- `retired-ai-app/app/components/chat/head/ChatLinkShare.vue`
- `docs/plan-prd/01-project/CHANGES.md`
- `docs/plan-prd/TODO.md`
- `docs/INDEX.md`

### AI: Cloudflare Dashboard Secrets 基线配置落地

**变更类型**: 运维配置 / 文档同步

**描述**:
- 已在 Cloudflare Pages 项目 `tuff-ai` 的 `preview` 与 `production` 环境完成首批 secrets 写入，确保附件链路具备统一默认基线：
  - `AIAPP_ATTACHMENT_PROVIDER=auto`
  - `AIAPP_ATTACHMENT_PUBLIC_BASE_URL`
  - `AIAPP_ATTACHMENT_SIGNING_SECRET`
  - `AIAPP_MINIO_REGION=us-east-1`
  - `AIAPP_MINIO_FORCE_PATH_STYLE=true`
- 待人工补齐环境相关敏感项（按环境值）：
  - OAuth：`AIAPP_NEXUS_OAUTH_CLIENT_ID`、`AIAPP_NEXUS_OAUTH_CLIENT_SECRET`、`AIAPP_NEXUS_INTERNAL_ORIGIN`（按需）
  - MinIO（启用时）：`AIAPP_MINIO_ENDPOINT`、`AIAPP_MINIO_BUCKET`、`AIAPP_MINIO_ACCESS_KEY`、`AIAPP_MINIO_SECRET_KEY`、`AIAPP_MINIO_PUBLIC_BASE_URL`（可选）
- 本次仅完成 Dashboard 配置，不含部署动作。

**测试**:
- `npx wrangler pages secret list --project-name tuff-ai --env preview` ✅
- `npx wrangler pages secret list --project-name tuff-ai --env production` ✅

**修改文件**:
- `docs/plan-prd/docs/AIAPP-INTELLIGENCE-API-CONTRACT.md`
- `docs/plan-prd/TODO.md`
- `docs/plan-prd/01-project/CHANGES.md`
- `docs/INDEX.md`

### AI: 开发端口固定 3300 并移除 dev.mjs 启动入口

**变更类型**: 开发体验清理 / 启动链路收敛

**描述**:
- `retired-ai-app` 的 `dev` 脚本改为直接执行 `nuxt dev`，不再通过 `scripts/dev.mjs` 包装启动。
...
**测试**:
- `pnpm aiapp:dev` ✅（本地启动成功：`http://127.0.0.1:3300/`）
- `pnpm -C "retired-ai-app" run test` ✅（`3` files, `14` tests passed）
...
### AI M0: Quota 前端并入 + Nuxt 兼容 API 落地

**变更类型**: 融合交付 / 兼容迁移

**描述**:
- `retired-ai-app` 完成 Quota 前端体系并入（`pages/layouts/components/composables/plugins/constants`），并切换路由语义：
  - `/` 使用 Quota 页面体系
  - `/aiapp` 使用原 AI 聊天页
  - `/aiapp/admin/storage` 使用原 AI 管理页
- 根壳保留 AI `app.vue`，新增 Quota 运行时桥接（`appOptions/globalOptions`、指令/插件注册），并移除 Quota 强制登录门禁，改为 AI 会话/访客态可用。
...
- 聊天链路复用 AI Runtime，兼容历史存储保留 `chat_id/topic/value/meta` 结构；删除会话时同步清理兼容记录与 AI 会话。
- 根 `.gitignore` 新增 `apps/quota-gpt-view/` 与 `apps/quota-gpt-ends/`，迁移参考目录不纳入提交。

**测试**:
- `pnpm -C "retired-ai-app" run test` ✅
- `pnpm -C "retired-ai-app" run typecheck` ⚠️（Quota 迁移存量类型问题，集中在 article/cms 等模块）
- `pnpm -C "retired-ai-app" run lint` ⚠️（Quota 迁移存量 lint 问题）
- `NODE_OPTIONS=--max-old-space-size=8192 pnpm -C "retired-ai-app" run build` ✅

**修改文件**:
- `retired-ai-app/app/app.vue`
- `retired-ai-app/app/pages/index.vue`
- `retired-ai-app/app/pages/aiapp/index.vue`
- `retired-ai-app/app/pages/aiapp/admin/storage.vue`
- `retired-ai-app/app/layouts/aiapp.vue`
- `retired-ai-app/server/api/aigc/*`
- `retired-ai-app/server/api/auth/status.get.ts`
- `retired-ai-app/server/api/account/*`
...
- `retired-ai-app/server/utils/quota-history-codec.ts`
- `retired-ai-app/nuxt.config.ts`
- `retired-ai-app/package.json`
- `retired-ai-app/uno.config.ts`
- `.gitignore`
...
- SSE 消费新增双超时兜底：
  - 空闲超时：`45s` 无事件自动中断并提示。
  - 总时长超时：`8min` 自动中断并提示。
- 超时参数改为可配置：支持通过 `NUXT_PUBLIC_AIAPP_STREAM_IDLE_TIMEOUT_MS` 与 `NUXT_PUBLIC_AIAPP_STREAM_MAX_DURATION_MS` 调整（含前端边界夹取）。
- 当用户进入会话触发 `fromSeq/follow` 且超时后，前端会自动调用 `POST /api/aiapp/chat/sessions/:sessionId/pause`（`reason=heartbeat_timeout`）终止会话，避免长期 loading 卡住。
- 会话列表删除按钮改为按行禁用（仅禁用正在运行中的会话），新建会话按钮不再因其它会话运行而锁死。

**测试**:
- `pnpm -C "retired-ai-app" run test`
- `pnpm -C "retired-ai-app" run typecheck`

**修改文件**:
- `retired-ai-app/app/composables/useAIChatPage.ts`
- `retired-ai-app/app/composables/aiapp-chat.types.ts`
- `retired-ai-app/app/components/aiapp/AISessionsPanel.vue`
...
### AI: Cloudflare 流式响应延迟优化（SSE / D1 热路径）
...
### AI OAuth: 清理 bridge legacy + API/CLI 调用收口

**变更类型**: 认证链路收敛 / 兼容清理

**描述**:
- AI 回调链路改为严格 OAuth：`GET /auth/callback` 仅接受 `code + state`，移除 `ticket` 回退消费逻辑。
- Nexus 侧下线全部 bridge 接口与实现：`/api/aiapp/auth/bridge-ticket`、`/api/aiapp/auth/bridge-consume`、`/api/aiapp/auth/bridge-start`。
- `authStore` 移除 `auth_aiapp_bridge_tickets` 相关表初始化、类型与读写函数，避免后续误接入 legacy。
- OAuth token 交换路径收敛为标准授权码模式：统一要求 `client_id + client_secret`，不再接受 `x-aiapp-oauth-secret`。
- Nexus OAuth 授权端点移除 `aiapp_web + redirect allowlist` 分支，统一按 OAuth 应用注册的 `redirectUris` 校验。
- AI 配置与类型同步收口：
  - `runtimeConfig.aiapp` 改为 `nexusOauthClientId` + `nexusOauthClientSecret`。
  - `.env.example` / Cloudflare env 类型改为 `AIAPP_NEXUS_OAUTH_CLIENT_ID` / `AIAPP_NEXUS_OAUTH_CLIENT_SECRET`。
- 文档补齐 API/CLI 调用与本地回调配置口径，统一“dev 本地 / prod 线上”的回跳策略说明。

**测试**:
- `pnpm -C "apps/nexus" exec vitest run server/api/aiapp/oauth/__tests__/authorize.get.test.ts server/api/aiapp/oauth/__tests__/token.post.test.ts server/api/dashboard/oauth/__tests__/clients.post.test.ts server/utils/__tests__/oauth-access.test.ts`
- `pnpm -C "apps/nexus" run typecheck`
- `pnpm -C "retired-ai-app" run test`
- `pnpm -C "retired-ai-app" run typecheck`

**修改文件**:
- `retired-ai-app/server/routes/auth/authorize.get.ts`
- `retired-ai-app/server/routes/auth/callback.get.ts`
- `retired-ai-app/nuxt.config.ts`
- `retired-ai-app/.env.example`
- `retired-ai-app/types/cloudflare-env.d.ts`
- `apps/nexus/server/api/aiapp/oauth/authorize.get.ts`
- `apps/nexus/server/api/aiapp/oauth/token.post.ts`
- `apps/nexus/server/api/aiapp/oauth/__tests__/authorize.get.test.ts`
- `apps/nexus/server/api/aiapp/oauth/__tests__/token.post.test.ts`
- `apps/nexus/server/utils/authStore.ts`
- `apps/nexus/nuxt.config.ts`
- `apps/nexus/server/api/aiapp/auth/bridge-ticket.post.ts`
- `apps/nexus/server/api/aiapp/auth/bridge-consume.post.ts`
- `apps/nexus/server/api/aiapp/auth/bridge-start.get.ts`
- `apps/nexus/server/api/aiapp/auth/__tests__/bridge-ticket.post.test.ts`
- `apps/nexus/server/api/aiapp/auth/__tests__/bridge-consume.post.test.ts`
- `docs/plan-prd/docs/AIAPP-NEXUS-OAUTH-CLI-TEST-PLAN.md`
- `docs/plan-prd/01-project/CHANGES.md`
- `docs/INDEX.md`
...
### AI: 高标准交付（附件识别发送 / 可插拔存储 / 刷新续接 / Markdown 分块渲染）

**变更类型**: 会话可靠性增强 / 多模态能力补齐 / 体验优化

**描述**:
- 附件上传链路升级为可插拔存储：
  - 新增 `AIAttachmentStorage` 抽象实现（`memory` + `R2`），`POST /uploads` 统一写入对象存储，不再只保留元数据。
  ...
  - 新增 Admin 存储配置后台：`/admin/storage` + `/api/aiapp/admin/storage-config`（D1 持久化），支持运行时动态改 MinIO 与 Base URL 配置。
...
- `pnpm -C "retired-ai-app" run test`
- `pnpm -C "retired-ai-app" run typecheck`
- `pnpm -C "retired-ai-app" run lint`
- `pnpm -C "packages/tuff-intelligence" run lint`
- `pnpm -C "packages/tuff-intelligence" exec tsc --noEmit`

**修改文件**:
- `retired-ai-app/server/utils/aiapp-attachment-storage.ts`
- `retired-ai-app/server/api/aiapp/chat/sessions/[sessionId]/uploads.post.ts`
- `retired-ai-app/server/api/aiapp/chat/sessions/[sessionId]/attachments/[attachmentId]/content.get.ts`
- `retired-ai-app/server/api/aiapp/chat/sessions/[sessionId]/messages.get.ts`
- `retired-ai-app/server/api/aiapp/chat/sessions/[sessionId]/index.delete.ts`
- `retired-ai-app/server/api/aiapp/chat/sessions/[sessionId]/stream.post.ts`
- `retired-ai-app/server/utils/__tests__/aiapp-attachment-storage.test.ts`
- `retired-ai-app/app/composables/aiapp-chat.types.ts`
- `retired-ai-app/app/composables/useAIChatPage.ts`
...
### Nexus: 通用 OAuth Client 申请（team admin / nexus admin）+ AI OAuth 多客户端兼容
...
- AI OAuth 端点升级为多客户端兼容：
  - `GET /api/aiapp/oauth/authorize` 保留 `aiapp_web` 旧行为，同时支持已注册 `client_id`（按注册 `redirect_uris` 校验）。
  - `POST /api/aiapp/oauth/token` 保留 legacy header secret 路径，同时支持非 `aiapp_web` 客户端通过 `client_secret` 换码。
- 本地联调变量补齐：
  - AI `.env.example` 增加 `AIAPP_NEXUS_INTERNAL_ORIGIN` 与 `AIAPP_NEXUS_OAUTH_SECRET`。
...
### AI: Dev 环境 Nexus 回跳默认改为本地（避免本地误跳线上）

**变更类型**: 认证链路修复 / 环境解析收敛

**描述**:
- 新增 `resolveAINexusOrigin` 专用解析逻辑，仅用于 AI -> Nexus 的授权与回调链路。
...
- 非开发环境保持线上默认 `https://tuff.tagzxia.com`，继续支持 `AIAPP_NEXUS_INTERNAL_ORIGIN` 覆盖内部回调地址。
- `retired-ai-app/nuxt.config.ts` 的 `nexusOrigin` 默认值同步为“dev 本地 / prod 线上”。
- 新增配置单测覆盖 dev/prod 优先级与 internal 回退顺序。

**修改文件**:
- `retired-ai-app/server/utils/aiapp-config.ts`
- `retired-ai-app/server/routes/auth/authorize.get.ts`
- `retired-ai-app/server/routes/auth/callback.get.ts`
- `retired-ai-app/server/utils/__tests__/aiapp-config.test.ts`
- `retired-ai-app/nuxt.config.ts`
- `docs/plan-prd/01-project/CHANGES.md`

### AI: 修复流式消息空格丢失与 final 重复拼接
...
### AI: 登录页授权 + 访客可用 + 历史 3 天有效期

**变更类型**: 登录体验调整 / 存储策略收敛

**描述**:
- AI 新增“先展示再授权”的登录页：`GET /auth/login` 不再自动跳转 Nexus，而是显示授权说明页，用户点击后走 `GET /auth/authorize` 发起授权。
- OAuth 授权链路：`/auth/authorize -> Nexus /api/aiapp/oauth/authorize -> /auth/callback`。
- 页面不再强制登录拦截：未登录也可直接使用聊天能力。
- 未登录用户历史改为绑定本地设备 ID（`aiapp_device_id` cookie），ID 丢失则访客历史不可恢复。
- 会话历史增加统一过期策略：按 `updated_at` 保留最近 3 天，超期会话及其消息/trace/checkpoint/附件自动清理。
- 侧边栏新增“授权登录 Nexus”入口，用户可在访客模式下按需升级登录。

**修改文件**:
- `retired-ai-app/server/routes/auth/login.get.ts`
- `retired-ai-app/server/routes/auth/authorize.get.ts`
- `retired-ai-app/server/middleware/require-aiapp-page-auth.ts`
- `retired-ai-app/server/utils/aiapp-device.ts`
- `retired-ai-app/server/utils/auth.ts`
- `retired-ai-app/server/utils/aiapp-history.ts`
- `retired-ai-app/server/utils/aiapp-store.ts`
- `retired-ai-app/app/components/aiapp/AISessionsPanel.vue`
- `retired-ai-app/server/utils/__tests__/auth.test.ts`
- `docs/plan-prd/01-project/CHANGES.md`

### AI × Nexus: OAuth 授权码兑换（兼容 bridge secret）

**变更类型**: 认证协议升级 / API 增强

**描述**:
- Nexus 新增 AI OAuth 端点：
  - `GET /api/aiapp/oauth/authorize`：校验 `client_id=aiapp_web`、`redirect_uri` allowlist，登录后签发一次性 `code` 回跳。
  - `POST /api/aiapp/oauth/token`：AI 服务端带 shared secret 兑换 `code`，返回 `userId`。
- AI 回调逻辑升级：
  - `/auth/callback` 优先处理 OAuth `code + state`，服务端换取用户身份后写入 `aiapp_auth_session`。
  - `state` 使用 `aiapp_oauth_state` cookie 校验，防止回调伪造。
- 兼容策略：
  - `/auth/callback` 保留 legacy `ticket` 回调兜底（迁移窗口兼容）。
  - token 端点 secret 默认兼容 `AIAPP_NEXUS_BRIDGE_SECRET`，可选独立 `AIAPP_NEXUS_OAUTH_SECRET`。
  - redirect allowlist 可通过 `AIAPP_OAUTH_REDIRECT_ALLOWLIST` 覆盖，未配置使用内置安全默认值。
...
- `retired-ai-app/server/routes/auth/authorize.get.ts`
- `retired-ai-app/server/routes/auth/callback.get.ts`
- `retired-ai-app/server/utils/aiapp-oauth.ts`
- `docs/plan-prd/01-project/CHANGES.md`

### AI: Legacy Auth 全量移除 + Cloudflare 变量最小化

**变更类型**: 认证收敛 / 部署配置治理

**描述**:
- AI 鉴权收敛为单路径：`requireAIAuth` 仅接受签名会话 cookie `aiapp_auth_session`。
- 移除所有 legacy 兼容入口：`x-aiapp-user-id`、`x-user-id`、`aiapp_user_id`、`Bearer`、localhost dev bypass。
- `GET /auth/callback` 停止写入 legacy `aiapp_user_id` cookie，仅写新会话 cookie。
- `retired-ai-app/wrangler.toml` 改为最小变量集合：
  - 固定 `NUXT_PUBLIC_NEXUS_ORIGIN=https://tuff.tagzxia.com`
  - 保留 `NUXT_AIAPP_BASE_URL`（其余走默认值）
  - `AIAPP_NEXUS_BRIDGE_SECRET` / `AIAPP_COOKIE_SECRET` / `NUXT_AIAPP_API_KEY` 统一走 Cloudflare secrets
- 明确保留并绑定 AI 独立存储：
  - D1：`tuff-ai-preview` / `tuff-ai-production`
  - R2：`tuff-ai-preview-attachments` / `tuff-ai-production-attachments`

**测试**:
- 更新 `retired-ai-app/server/utils/__tests__/auth.test.ts`：
  - `session-cookie` 成功路径；
  - 未登录 401；
...
### AI: Main 区域头部精简与 Trace 入口收敛

**变更类型**: UI 体验优化

**描述**:
- 移除 `AIChatWorkspace` 头部的会话标题与 session id 展示，主区聚焦消息与输入。
...
### AI: Nexus 登录桥接 + 用户历史隔离保持 + Markdown 渲染 + 独立 Cloudflare 配置

**变更类型**: 认证集成 / 体验改进 / 部署治理

**描述**:
- Nexus 新增 AI 登录桥接票据能力：
  - `POST /api/aiapp/auth/bridge-ticket`（需 Nexus 登录态）签发 60s 一次性票据。
  - `POST /api/aiapp/auth/bridge-consume`（共享密钥校验）消费票据并返回用户身份。
  - `GET /api/aiapp/auth/bridge-start` 作为浏览器登录入口：未登录时跳转 `sign-in`，登录后签发票据并重定向回 AI 回调地址。
- AI 新增登录桥接流程：
...
- `requireAIAuth` 在首版桥接阶段采用“新优先、旧兼容”；该兼容路径已在同日后续迭代移除，当前仅保留 `aiapp_auth_session`。
- 聊天渲染改为 Markdown：`AIChatWorkspace` 的 `TxChatList` 开启 `:markdown=\"true\"`，沿用 tuffex sanitize 默认策略。
- Cloudflare 独立部署能力：
  - 新增 `retired-ai-app/wrangler.toml`，与根 wrangler 配置解耦。
  - `retired-ai-app/package.json` 的 `preview:cf/deploy:cf` 强制使用 AI 专属 config 与 project-name（`tuff-ai`）。
  - Nuxt Cloudflare Dev 配置改为读取 `retired-ai-app/wrangler.toml`。
...
- `retired-ai-app/wrangler.toml`
- `retired-ai-app/.env.example`
- `retired-ai-app/vitest.config.ts`
- `docs/plan-prd/01-project/CHANGES.md`
- `docs/INDEX.md`

### AI: DeepAgent 真增量流式渲染修复 + Legacy 兼容梳理

**变更类型**: 流式体验修复 / 兼容治理

**描述**:
- 修复 AI “回答需等待全部生成后才一次性渲染”的问题：
...
### AI: SSE 可观测性增强 + DeepAgent 风格阶段事件 + 默认模型升级

**变更类型**: 功能增强 / 可靠性修复

**描述**:
- 修复 AI 多轮对话 trace `seq` 冲突问题（按会话 `lastSeq` 续号），避免第二轮写入唯一索引冲突。
- `POST /api/aiapp/chat/sessions/:sessionId/stream` 增强：
...
- DeepAgent 最小封装迁入 `@talex-touch/tuff-intelligence`：
  - 新增 `DeepAgentLangChainEngineAdapter`，AI 不再内联拼装 `/v1/responses` 请求。
  ...
  - 新增 `proxy: false` 网络执行策略：在 AI runtime 调用链中禁用 `HTTP_PROXY/HTTPS_PROXY/ALL_PROXY`，并强制注入 `NO_PROXY=localhost,127.0.0.1,::1`，用于排除本机 upstream 误走系统代理。
  - AI 上游配置改为本地 `.env` 驱动：
    - 移除 `nuxt.config.ts` 与 `aiapp-runtime.ts` 内置 API Key / endpoint 硬编码。
    - 新增 `retired-ai-app/.env.example`（`NUXT_AIAPP_*`）用于本地配置渠道、预算、重试与 proxy 策略。
  - 进一步精简环境变量：AI AI 渠道仅保留 `NUXT_AIAPP_BASE_URL` / `NUXT_AIAPP_API_KEY` / `NUXT_AIAPP_MODEL`（默认 baseUrl 为 `http://localhost:3000/openai`），删除 `endpoint/upstreamChat` 入口，减少配置歧义。
- AI 前端 Trace 抽屉增强：展示 `run.audit` 和结构化错误详情；流结束后按增量 trace 合并，避免覆盖流内审计事件。
...
### AI: 页面层优先复用 tuffex 组件

**变更类型**: UI 架构收敛 / 组件复用

**描述**:
- AI 聊天页从自定义原生控件切换为优先使用 `@talex-touch/tuffex` 组件，统一视觉与交互语义。
- 保持现有行为约束不变：
  - 页面整体不滚动；
  - 中间聊天区全高撑开；
  - 输入区固定底部；
...
2. `retired-ai-app` 引入 `@talex-touch/tuffex` 依赖，并在 Nuxt 配置中补齐样式、transpile 与 workspace source alias。
3. 会话列表状态徽标、消息列表、输入区、Trace 抽屉与空状态全部切换到 tuffex 组件实现。
4. 扩展 `TxChatComposer`，新增“附件+输入一体化”能力（附件列表、附件按钮、左侧工具栏插槽），AI 底部输入区改为统一 Composer，不再分离独立附件区。
5. `TxChatComposer` 新增 `toolbar` 具名插槽，AI 通过插槽实现类似 Web 搜索模式的底部操作条（附件入口 + 模式/搜索项 + 右侧语音圆按钮）。
...
**描述**:
- 修复 AI 聊天页“视觉怪异”问题：消息空态由 `blank-slate` 插画改为 `custom + card` 轻量空态，避免大插画在主聊天区造成视觉突兀。
...
### AI: 首页结构拆分（左侧/中间组件 + 页面 composable）

**变更类型**: 架构整理 / 可维护性提升

**描述**:
- 将 AI 首页从单文件大组件拆分为“页面组装层 + 左侧会话组件 + 中间聊天组件 + 逻辑 composable”，降低页面耦合并便于后续迭代。
- 保持现有行为与交互不变：双栏布局、整页不滚动、消息区独立滚动、底部输入固定、Trace 右侧抽屉。

**主要变更**:
1. 抽离左侧会话区组件 `AISessionsPanel`，承载会话列表渲染与交互事件。
2. 抽离中间聊天区组件 `AIChatWorkspace`，承载消息区、底部一体化输入、Trace 抽屉与附件选择器。
3. 新增 `useAIChatPage` composable，统一收敛会话状态、SSE 流处理、消息发送、附件上传、标题总结与回放逻辑。
4. 新增 `aiapp-chat.types.ts` 统一页面与组件共享类型，减少页面内联类型噪音。
...
### AI: 页面版式收敛（间距清理 + 响应式比例 + 内容居中）

**变更类型**: UI 布局优化 / 响应式适配

**描述**:
- 清理 AI 页面多余 `margin/padding`，统一左右栏与聊天区间距节奏，降低视觉拥挤与空洞并存问题。
- 左右栏比例调整为更接近 ChatGPT 的宽度关系：左侧固定范围收敛，右侧主区域获得更稳定的阅读空间。
- 右侧聊天区新增居中壳层并限制最大宽度，兼容大屏与中屏，避免内容“贴边拉长”。

**主要变更**:
1. `aiapp-page` 网格列调整为 `clamp(248px, 19vw, 292px) + 1fr`。
2. `AIChatWorkspace` 新增 `aiapp-chat__shell`，右侧内容采用 `margin-inline: auto` 居中并设置最大宽度限制。
...
### AI: 左侧列表极简化（隐藏时间 + 尾部状态点 + Hover 删除）
...
**新增接口**:
- `GET /api/aiapp/chat/sessions/notifications`
- `POST /api/aiapp/chat/sessions/:sessionId/notification`

**修改文件**:
- `packages/tuff-intelligence/src/store/store-adapter.ts`
- `packages/tuff-intelligence/src/store/d1-runtime-store.ts`
- `retired-ai-app/server/api/aiapp/chat/sessions/notifications.get.ts`
- `retired-ai-app/server/api/aiapp/chat/sessions/[sessionId]/notification.post.ts`
- `retired-ai-app/server/api/aiapp/chat/sessions/[sessionId]/stream.post.ts`
- `retired-ai-app/app/composables/aiapp-chat.types.ts`
- `retired-ai-app/app/composables/useAIChatPage.ts`
- `retired-ai-app/app/components/aiapp/AISessionsPanel.vue`
...
### AI: 主题变量收敛（tuffex token）+ 系统亮暗自动切换

**变更类型**: 视觉一致性优化 / 主题适配增强

**描述**:
- AI 首页与左右栏样式中硬编码颜色收敛为 `tuffex` CSS 变量（`--tx-*`），减少页面私有色值分叉。
...
### AI: 渠道配置极简化（仅 BASE_URL + API_KEY）与读取修复

**变更类型**: 配置收敛 / 缺陷修复

**描述**:
- 修复 `api key is not configured` 误报：`aiapp-runtime` 改为多层读取顺序（`runtimeConfig -> Cloudflare env -> process.env`），避免本地 `.env` 或 worker env 已配置但运行时读空。
- AI 配置入口收敛为仅两项：
  - `NUXT_AIAPP_BASE_URL`
  - `NUXT_AIAPP_API_KEY`
- 移除 proxy/no-proxy/model 等 env 入口，默认模型固定 `gpt-5.4`（代码常量）。
- 移除历史兼容键名与回退：`AIAPP_*`、`NUXT_AIAPP_UPSTREAM_RESPONSES_*` 不再生效。
- 移除 `Authorization` 头作为 API Key 的回退来源，统一只使用配置项 `NUXT_AIAPP_API_KEY`。
- `@langchain/openai` 升级至支持 Responses API 的版本，并在 DeepAgent `ChatOpenAI` 中启用 `useResponsesApi`，默认直接走 `/v1/responses`。
- 新增 `packages/tuff-intelligence/src/business/aiapp/*`，将 `planning / run.metrics / replay` 流式编排逻辑从 `retired-ai-app` 下沉到 intelligence 包。
- AI 心跳改为 SSE 内部事件（`stream.heartbeat`），前端移除额外 `/heartbeat` 请求链路。
- `appendTrace + seq 冲突重试 + emit` 下沉到 `business/aiapp/emitter.ts`，`stream.post.ts` 收敛为传输层编排。
- `toJsonSafe/toSafeRecord/stream error detail` 统一下沉到 `packages/tuff-intelligence/src/business/aiapp/utils.ts`，`stream.post.ts` 不再内联同类实现。
...
- 删除 `POST /api/aiapp/chat/sessions/:sessionId/heartbeat` 路由入口。
- `DeepAgent` 适配层移除 proxy 注入链路与 upstream chat fallback 分支，仅保留 relay baseUrl 路径，减少配置歧义。
- 新增根忽略规则：`retired-ai-app/.env`，避免本地密钥误提交。
...
- `retired-ai-app/.env.example`
- `packages/tuff-intelligence/src/adapters/deepagent-engine.ts`
- `packages/tuff-intelligence/src/business/aiapp/{stream.ts,types.ts,emitter.ts,utils.ts}`
- `retired-ai-app/app/pages/index.vue`
- `retired-ai-app/server/api/aiapp/chat/sessions/[sessionId]/stream.post.ts`
- `.gitignore`
- `docs/plan-prd/01-project/CHANGES.md`

### AI 下一阶段执行文档落地（测试优先 + OAuth + CLI + channel routing）

**变更类型**: 文档规划 / 执行顺序收敛

**描述**: 基于 AI 首版上线状态，新增下一阶段执行文档，明确后续按“测试优先”推进，并串联 Nexus OAuth 登录、`tuff-ai-cli` 与后端渠道可配置能力，作为后续实现统一入口。

**主要变更**:
1. 新增文档 `docs/plan-prd/docs/AIAPP-NEXUS-OAUTH-CLI-TEST-PLAN.md`。
...
### AI dev 启动稳定性修复（Nuxt Cloudflare + checker）

**变更类型**: 开发体验修复 / 配置收敛

**描述**: 修复 `retired-ai-app` 在 `nuxt dev` 下因 `vite-plugin-checker` 触发 `typescript/lib/typesMap.json` 拷贝失败导致的启动异常；同时修正 Cloudflare dev 的兼容日期配置入口。

**主要变更**:
1. `retired-ai-app/nuxt.config.ts` 将 `compatibilityDate` 提升到 Nuxt 顶层并更新到 `2026-03-08`。
2. `retired-ai-app/nuxt.config.ts` 关闭 dev 内置 `typescript.typeCheck`，改为通过 `pnpm aiapp:typecheck` 独立执行类型检查。

**修改文件**:
- `retired-ai-app/nuxt.config.ts`
- `docs/plan-prd/01-project/CHANGES.md`

### AI 本地联调通路补齐（Cloudflare 绑定 + 内置 Responses mock upstream）

**变更类型**: 本地联调能力增强 / 测试配置落地

**描述**: 为 AI 测试阶段补齐本地可用链路。`nuxt dev` 在 cloudflare-dev 模式下显式复用根 `wrangler.toml`（含 D1/R2 绑定与持久化目录）；同时内置固定 Responses upstream（用户提供的 mock 地址与 token），让 Chat 流程可直接联调。

**主要变更**:
1. `retired-ai-app/nuxt.config.ts` 新增 `nitro.cloudflareDev`（`configPath` + `persistDir` + `environment`），本地调试可读取 `DB` 绑定并持久化到 `.wrangler/state/v3`。
2. `retired-ai-app/server/utils/aiapp-runtime.ts` 增加 OpenAI Responses 请求路径（`/v1/responses`）与多格式响应文本提取逻辑。
3. AI runtime config 增加内置 `upstreamResponsesBaseUrl` / `upstreamResponsesApiKey` / `upstreamResponsesModel` 默认值，用于本地 mock 联调。
4. `retired-ai-app/server/utils/aiapp-store.ts` 附件桶读取增加 `R2` fallback（兼容根 wrangler 绑定名）。
5. `retired-ai-app/types/cloudflare-env.d.ts` 增加 `R2` 绑定类型声明。
...
### AI（Nuxt Edge）上线首版 + Intelligence Hard-Cut 收口

**变更类型**: 新增应用 / 架构收口 / 协议落地

**描述**: 新增 `retired-ai-app` 作为独立部署的 AI Chat-first 应用，并将 Agent Runtime/Protocol 核心统一收口到 `packages/tuff-intelligence`。AI 服务端采用 SSE + checkpoint/replay 模式，具备长会话断线恢复的基础能力。

**主要变更**:
1. 新建 `retired-ai-app`（Nuxt + `cloudflare-pages` preset），实现会话、消息、trace、heartbeat、pause、upload、stream API。
2. 新增 `POST /api/aiapp/chat/sessions/:sessionId/stream`，支持 `assistant.delta/final`、`run.metrics`、`session.paused`、`error`、`done` 与 `fromSeq` 补播。
3. 新增 AI 前端 V1 聊天页：会话列表、消息流、附件上传、停止、补播恢复、Trace 抽屉。
...
7. 根脚本补齐 `aiapp:dev`、`aiapp:build`、`aiapp:typecheck`、`aiapp:lint`。
...
### 修复: AI 图片发送语义统一为单条 user 多模态 content
...
2. Quota 与 AI Stream 两条后端链路统一内联阈值（单图 5MB / 总量 12MB），并优先使用 `dataUrl`。
3. 补充回归测试，覆盖：
...
### 修复: AI 旧 QuotaGPTView 对话链路消息完整性与图片输入兼容
...
- `retired-ai-app/server/utils/quota-history-codec.ts`
- `retired-ai-app/server/api/aigc/executor.post.ts`
- `retired-ai-app/server/utils/__tests__/quota-history-codec.test.ts`
...
### 新增: AI 双栏会话体验与 AI 标题总结

**变更类型**: 功能增强

**描述**: AI 聊天页升级为 ChatGPT 风格双栏交互，支持会话删除与 AI 自动命名标题。
...
4. **会话标题 AI 总结能力**:
   - 新增 `POST /api/aiapp/chat/sessions/:sessionId/title` 接口
   - 基于会话消息调用模型总结短标题并持久化到会话表
   - 无模型可用时回退为首条用户消息摘要

5. **存储能力扩展**:
   - `aiapp_chat_sessions` 新增 `title` 字段并兼容老表结构
   - Runtime Store 新增 `setSessionTitle` / `deleteSession` 方法

**修改文件**:
- `retired-ai-app/app/pages/index.vue`
- `retired-ai-app/server/api/aiapp/chat/sessions/[sessionId]/index.delete.ts`
- `retired-ai-app/server/api/aiapp/chat/sessions/[sessionId]/title.post.ts`
- `packages/tuff-intelligence/src/store/store-adapter.ts`
- `packages/tuff-intelligence/src/store/d1-runtime-store.ts`

**影响**:
- AI 对话管理更接近主流 AI 聊天产品交互
- 会话可读性与检索效率提升（标题化）
- 会话清理与 trace 查看路径更清晰
