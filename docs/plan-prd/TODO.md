...
| Nexus 插件发布管理 | Nexus 不新增 `/dashboard/plugins` 页面路由，发布入口继续收敛在 `/dashboard/assets`；Dashboard plugin API 已支持登录态和 `plugin:read/plugin:publish` API key；创建插件支持同请求初始化首版，失败回滚新建 plugin；CoreApp 已新增 `/store/publisher` 发布管理 tab 与 `auth:nexus-upload` 上传桥 | 补 App 发布管理的编辑元数据、删除/重提 rejected 版本、API key 用户自助创建权限与更完整 package policy/security scan；补真实 `.tpex` 上传端到端证据 | `TODO` / `README` / `INDEX` / `CHANGES` |
