...
| Beta 发布准备 | `2.4.10-beta.22` release notes 已补齐；`build-and-release` 支持 tag push 触发 beta pre-release；Widget/CLI 最新修复与兼容性复核记录已合入；OmniPanel Gate、Utils Package CI 与 macOS beta build 准备结果延续有效；PR lint 阶段已改为只检查 PR 修改的 JS/TS/Vue 文件，避免既有全仓 lint 债务阻塞无关 PR | 用户确认后再提交、推送并创建/推送 `v2.4.10-beta.22` tag 触发发布；`quality:release` 仍保留全仓 lint，未补齐 Windows 真机 evidence 前不得宣称正式 `2.4.10` gate 通过 | `TODO` / `README` / `INDEX` / `CHANGES` / `.github/workflows/README` |
...
| 2.5.0 AI 板块 | dev/2.5.0 已开始实现登录态 Nexus AI invoke 与 OmniPanel Writing Tools MVP；CoreBox AI Ask 最小 Stable 切片已接入 `text.chat` 与剪贴板图片 `vision.ocr -> text.chat`；CoreApp 默认 `tuff-nexus-default` provider 通过 `/api/v1/intelligence/invoke` 调 Nexus Intelligence Provider / Provider Registry mirror 与 secure-store 凭证，未登录显式失败并 fallback 到其它 provider；OmniPanel 已新增划词 AI 翻译/摘要/改写/解释/Review、结果预览面板、transient Desktop Context Capsule、copy/retry/replace clipboard 二次确认与 Nexus invoke smoke；`touch-translation` 已收口为 Intelligence-first 翻译路径并透传 traceId，typed `ttsSpeak` / `audio.tts` 作为 Beta 能力进入翻译结果朗读入口，并提供进程内短期缓存 | 继续拆分 Workflow `Use Model` 节点、完整 Review Queue 与 3 个 P0 模板；补 TTS provider/live smoke、队列/播放服务与持久缓存评估，不得扩大到全量多模态或 Scene runtime 全量编排 | `TODO` / `README` / `INDEX` / `CHANGES` / `Roadmap` / `Quality Baseline` |
