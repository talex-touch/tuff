...
| Beta 发布准备 | `2.4.10-beta.22` release notes 已补齐；`build-and-release` 支持 tag push 触发 beta pre-release；Widget/CLI 最新修复与兼容性复核记录已合入；OmniPanel Gate、Utils Package CI 与 macOS beta build 准备结果延续有效 | 用户确认后再提交、推送并创建/推送 `v2.4.10-beta.22` tag 触发发布；完整 `quality:pr` 仍被 `apps/pilot` 既有 lint 债务阻断；未补齐 Windows 真机 evidence 前不得宣称正式 `2.4.10` gate 通过 | `TODO` / `README` / `INDEX` / `CHANGES` / `.github/workflows/README` |
