...
- [ ] Transport retained event 第四实施切片：优先评估 `auth` retained aliases，要求先建立 shared canonical typed events 与 `legacy` alias registry，再做 main/renderer 双监听或双发迁移；不得改变 `auth:*` / `account:*` 现有 wire name 行为，完成后继续收紧 `transport-event-boundary.test.ts` retained raw definition 上限并补 `transport-domain-sdks.test.ts`。
