...
- [ ] Transport retained event 第九实施切片：继续评估 CoreBox shared `CoreBoxEvents` 主体拆分边界，优先拆 `core-box:get-ui-view-state/shortcut-triggered/ui-mode-exited/hide-input/show-input` 或低耦合 input/layout 小组；仍需保持现有 wire name 兼容窗口，禁止一次性扩大到完整 CoreBox 主路径 hard-cut。
