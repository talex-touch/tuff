...
| 2.5.0 AI 板块 | dev/2.5.0 已开始实现登录态 Nexus AI invoke 与 OmniPanel Writing Tools MVP；CoreBox AI Ask 最小 Stable 切片已接入 `text.chat`、剪贴板图片 `vision.ocr -> text.chat` 与 Intelligence Session 交接上下文，`touch-intelligence` 轮次会绑定稳定 `handoffSessionId` 供后续恢复/接续；CoreApp 默认 `tuff-nexus-default` provider 通过 `/api/v1/intelligence/invoke` 调 Nexus Intelligence Provider / Provider Registry mirror 与 secure-store 凭证，未登录显式失败并 fallback 到其它 provider；Nexus `/api/v1/intelligence/invoke` 首批已按模型返回 `usage.totalTokens` 扣减 AI credits，CoreApp 设置页与 Intelligence 首页复用 `/api/credits/summary` 展示个人剩余/已用/总额与团队池剩余；OmniPanel 已新增划词 AI 翻译/摘要/改写/解释/Review、结果预览面板、transient Desktop Context Capsule、copy/retry/replace clipboard 二次确认与 Nexus invoke smoke；Workflow `Use Model` 最小节点已接入 `model` step kind，走 Stable capability 白名单并记录 trace/provider/model/usage；Workflow 页面本地 Review Queue MVP 已可审阅步骤输出、复制、二次确认替换剪贴板与忽略；`touch-translation` 已收口为 Intelligence-first 翻译路径并透传 traceId，typed `ttsSpeak` / `audio.tts` 作为 Beta 能力进入翻译结果朗读入口，并提供进程内短期缓存 | 继续推进完整/持久化 Review Queue 与 3 个 P0 模板；补 TTS provider/live smoke、队列/播放服务与持久缓存评估；Provider Registry Phase 4 再接 success rate、quota、dynamic `pricingRef` 与模型倍率，不得扩大到全量多模态或 Scene runtime 全量编排 | `TODO` / `README` / `INDEX` / `CHANGES` / `Roadmap` / `Quality Baseline` |
...
- [x] 2.5.0 Nexus AI credits 首切：`/api/v1/intelligence/invoke` 成功调用且 `usage.totalTokens > 0` 后写入 `credit_ledger` 并更新 user/team credits usage；provider 调用失败或 `totalTokens=0` 不扣费，credits 不足返回 `402 / CREDITS_EXCEEDED`；CoreApp 设置页与 Intelligence 首页通过 `AuthEvents.nexus.request` 复用 `/api/credits/summary` 展示 credits 剩余、已用、总额与团队池剩余。
- [ ] 2.5.0 AI/workflow 后续：OmniPanel Writing Tools MVP、Desktop Context Capsule 最小版、Workflow `Use Model` 最小节点、Workflow 页面本地 Review Queue MVP、翻译插件 Intelligence-first 与 TTS Beta typed 朗读入口、进程内短期缓存已落地；继续推进完整/持久化 Review Queue、3 个 P0 模板，以及 TTS 队列/播放服务与持久缓存评估。
