...
- 执行约束：PR lint 已收敛为 changed-file lint，但 `quality:release` 仍保留全仓 lint；2026-05-20 自动化审计未发现新的 P0 fixed fake-success，剩余高信号治理点为 `touch-snipaste` shell capability、`touch-window-presets` 展示期权限请求、Browser Data source-level diagnostics、widget runtime sandbox、secret backend、裸 console 与 SRP 大文件；旧 compat registry / legacy allowlist / size allowlist 已退场，不再作为 live SoT；2026-05-18 已按维护决策删除独立 `.github/workflows/omnipanel-gate.yml`，OmniPanel scoped typecheck/lint/unit/build/smoke 不再作为 GitHub Actions 自动门禁；npm 公共子包发布因仓库 `NPM_TOKEN` 无法 publish `@talex-touch` scope 仍阻塞；`2.5.0` AI/Provider 与可见体验工作台优化只能以 dev 小切片推进，不得抢占 `2.4.11` 稳定化与债务退场主线。
...
| P0-STARTUP | CoreApp 启动异步化长尾跟进 | 进行中 | Dev 与 packaged hot/cold benchmark、CoreBox 核心状态、App Index workbench 已完成最近路径验证；2026-05-20 已补 CoreBox app launch handoff：App Provider background launch 先 await immediate hide，再异步执行 app，避免慢启动期间可见 CoreBox 卡死；后续只保留真实设备 benchmark、登录恢复、AI/OmniPanel/Workflow/Provider 可见体验长尾跟进，不再作为 `2.4.10` 发布口径。 |
...
| P1-APP-DATA | App Data Plugins 与 Everything 收口 | 进行中 | 已新增 `docs/plan-prd/03-features/search/APP-DATA-PLUGINS-AND-EVERYTHING-ROADMAP.md` 与 `docs/plan-prd/03-features/search/RAYCAST-UTOOLS-CAPABILITY-GAP-MATRIX.md`。近期目标是先建统一数据源/索引诊断基线，并按 Raycast/uTools 常用能力差距继续补 `touch-browser-data` 真实浏览器书签/历史、Everything 真机 evidence、App Launcher evidence、Quicklinks 统一模型与 Context Actions；Calculator 显式入口已通过 CoreBox `calc` / `calculator` / `calculate` / `计算` / `换算` 前缀接入 PreviewSDK；`touch-snippets` 首批 placeholders 已支持 `{{date}}` / `{{time}}` / `{{uuid}}` / `{{clipboard}}`，browser-tab、cursor、hot string/autopaste 后置；`touch-emoji-symbols` 首版已支持常用 emoji/symbol 搜索与复制，recent usage、扩展数据集和 Store evidence 后置；Nexus SDK 文档已新增插件开发任务流并把 quickstart/manifest/publish 口径收敛到 `main: "index.js"`、Prelude、`tuff validate/build/publish --dry-run` 与 CloudSync/CloudShare 边界；TuffEx 场景化 demo 已完成首切；Windows Everything 已完成 CLI 手动选择、诊断 evidence 字段与 File Index watch-root 路径过滤代码闭环，仍需收口 SDK/CLI 最终策略、Windows 真机 evidence 与性能基线；2026-05-20 已补 `touch-browser-data` source-level diagnostics、Linux Arc unsupported 口径、profileCount/read-failed reason 与平台感知空态；历史扫描、写索引、disable/clear/rebuild UI 与跨平台 evidence 后置；明确不包含更新系统 Nexus Hard-Cut。 |
