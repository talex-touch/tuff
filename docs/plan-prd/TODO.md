...
| 2.5.0 AI 板块 | dev/2.5.0 已开始实现登录态 Nexus AI invoke 切片；CoreApp 默认 `tuff-nexus-default` provider 通过 `/api/v1/intelligence/invoke` 调 Nexus Intelligence Provider / Provider Registry mirror 与 secure-store 凭证，未登录显式失败并 fallback 到其它 provider；CoreBox / OmniPanel 仍是主入口，Workflow 是主要执行载体，Pilot 是增强能力来源 | 继续拆分 OmniPanel Writing Tools、Workflow `Use Model` 节点、Review Queue、Desktop Context Capsule 与 3 个 P0 模板；不得扩大到全量多模态或 Scene runtime 全量编排 | `TODO` / `README` / `INDEX` / `CHANGES` / `Roadmap` / `Quality Baseline` |
