...
- 当前执行基线从根目录与 `apps/core-app/package.json` 读取；历史 release 事实保留在 `../01-project/CHANGES.md` 与对应 Evidence Matrix。本文件只记录仍有效的质量边界。
...
- 数据正确性门禁：`item_usage_stats` 必须只有一个增量 writer；任何 summary/rebuild 路径不得在无 watermark 的情况下重复加计实时统计。历史数据修复必须先备份、量化影响、提供回滚，并证明单次 execute 在重启/summary 后仍只计一次。
- 执行状态门禁：提交继续按 related-only 组织，但本文件不记录某次 dirty worktree、分支、HEAD 或手工版本。实时任务看 Trellis，当前两周顺序看 `../TODO.md`，完成声明看 Evidence Matrix / `CHANGES.md`。
- 文档稳定性口径：Roadmap 只描述阶段与能力边界，专题 TODO 只描述专题细节；任何入口不得建立第二套全局优先级或把临时本地状态写成长期事实。
