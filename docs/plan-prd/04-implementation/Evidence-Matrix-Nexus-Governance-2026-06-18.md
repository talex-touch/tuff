...
- Governance report 已按 source fail-closed 区分 `live` / `d1` / `r2` / `local-only` / `memory` / `open`：本地 storage smoke、browser notification delivery、dry-run / consumed quota smoke 不会晋级 production evidence。
...
| NX-GOV-08 | provider quota fail-closed | 真实 provider quota exhausted 时阻断 direct invoke / scene run；channel quota 必须在 dispatch 前拦截 | provider quota smoke evidence + blocked direct invoke/scene run evidence | partial |
...
- `provider-quota` 只有 `mode=consume`、`status=blocked`、`requestRecorded=true` 且 `evidenceSource=live` 时才显示 `live`；dry-run、allowed 或 consumed smoke 只能显示 `local-only`。
