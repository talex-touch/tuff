...
- 当前代码版本以 root / CoreApp `2.4.12-beta.8` 为口径；当前分支存在多主题未提交改动，后续仍按 related-only 拆分验证。
...
| 阶段 | 主线 | 要交付什么 | 完成标准 |
| --- | --- | --- | --- |
| R0 | 口径清理 | 移除 npm publish / 平台验证相关 blocker；把当前 `2.4.12-beta.8`、GitHub 自动发版口径写入规划 | 文档无 `包发布凭据`、平台验证作为待办；`git diff --check` 通过 |
