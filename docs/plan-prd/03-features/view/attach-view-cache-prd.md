...
- 高频视图打开耗时中位数 < 200ms，缓存命中率 ≥ 70%。
- 缓存策略可观测、可控、可回收，避免长期内存泄漏。
- 插件可通过 SDK 明确声明缓存意图并获得回调反馈。

### 1.2 质量约束

- 缓存管理必须具备硬性上限（数量与内存），超限触发 LRU 回收。
- 预加载不允许阻塞主线程，必须在 idle/低负载条件下执行。
- 状态落地遵循 Storage Rule：配置与统计以 SQLite 为 SoT。
- 缓存与 DivisionBox/多视图共存必须可预测，禁止双重加载。

### 1.3 回滚与兼容策略

- 允许通过 `appSetting.viewCache` 全量关闭缓存与预加载。
- 关闭后回退为“按需创建/销毁”的旧行为，不影响插件 API 兼容性。
- 发生异常时优先降级到 cold 模式，并记录一次性告警。
