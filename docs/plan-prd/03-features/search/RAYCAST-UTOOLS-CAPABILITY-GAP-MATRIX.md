...
| Translation / OCR / 图片输入 | Raycast Google Translate 扩展；uTools 快速翻译、聚合翻译、OCR | `touch-translation` 支持翻译、多源翻译、截图翻译；2.5.0 Stable 目标只承诺 CoreBox 文本 + 显式 OCR、provider routing 与固定失败路径 | OCR pipeline、截图输入、provider health、隐私策略和失败 reason 需要统一 | P1 | 把 OCR 作为 AI/Native capability 明确 health；翻译 provider secret 使用 secure store degraded 状态。 |
