...
- 待继续：scan worker、incremental DB persist、index worker flush trace、FTS 写入语义、scan_progress 真实读写和 integrity-triggered runtime reset 的 durable job history 继续从 FileProvider 内部拆到 runtime task/store 边界；File scan、watch route、watch-root recovery 与 reset 已具备 `jobId/queuedAt` 用户可见 task evidence，后续再提升为完整 durable job history。
