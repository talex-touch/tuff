...
- Runtime task model 已拆出 `SourceDiagnosticsService`、`WatchEventRouter`、`ScanScheduler`、`ReconcileScheduler`、`ReconcileEngine`、`IndexStoreAdapter`、`IndexingRootPolicy` 与 SDK `IndexedSourceRuntimeTaskJobFactory`，`IndexingRuntime` 只保留 registry 与编排入口。
- `ScanScheduler` 支持 source-level scan task 与 batch 写入 store adapter，并已新增 batch scan result 统计与 failure isolation，避免单个 source scan 失败拖垮整批扫描；`IndexingRuntime` 已为 single/batch scan 成功、失败与 skipped source 写入 `lastScan.jobId/queuedAt` diagnostics，且 `lastScan` / history entry 由 SDK `buildIndexedSourceScanTaskState()` 构建；`WatchEventRouter` 将 watch delta 写入 store adapter，并已新增 route result 统计与 failure isolation，避免单个 source watch handler 或 store delta 写入失败拖垮整次 watcher route，同时 `lastWatch` 也会为 applied delta、handler/store failure 与 skipped source 写入 `jobId/queuedAt`，且 `lastWatch` / history entry 由 SDK `buildIndexedSourceWatchTaskState()` 构建；`ReconcileScheduler` 已作为 `IndexingRuntime` 与 `ReconcileEngine` 之间的最小任务入口，负责 job id、queuedAt、reason/rootCount 记录，reconcile result/failure/skipped 的 `lastReconcile` / history entry 由 SDK `buildIndexedSourceReconcileTaskState()` 构建；reset result 的 `lastReset` / history entry 由 SDK `buildIndexedSourceResetTaskState()` 构建；scan/watch/reconcile/reset 四类维护任务因此复用同一个 SDK `IndexedSourceRuntimeTaskJobFactory` runtime task identity，为后续 durable job history / retry / debounce 做边界；`IndexingRuntime` 持有共享 SDK `IndexedSourceTaskRunGate`，并注入 `ScanScheduler` 与 `ReconcileScheduler`，reset 入口也复用同一个 gate：同 source reset 运行中会记录 `reset-already-running` result，不再重复清 shared SearchIndex 或 source-local reset；`IndexingRuntime.clear()` 会清理该内存态 gate，避免 runtime teardown 后残留 running state；当前仍保持 running 时拒绝的既有行为；`ReconcileEngine` 支持 source-level reconcile 入口与 unsupported 结果，并已新增 batch reconcile result 统计与 failure isolation。
...
- Browser Bookmarks 已新增 `BrowserBookmarksIndexedSource` skeleton，注册进 CoreApp runtime diagnostics，带 high privacy + official-plugin + browser-data/file-system admission；disabled health/evidence 现在明确表达 `touch-browser-data.browser-bookmarks` 官方 provider 需要用户同意，而不是泛化为 pending migration；官方 provider id、consent reason、runtime bridge reason 与 `SearchProviderDescriptor` 构造已收敛到同一 source contract，`browser-bookmarks-source-config` 只复用该 descriptor 计算 enablement；同时已抽出 CoreApp 纯 Chromium Bookmarks scanner。显式 enabled path 下已能产出 `IndexedSourceRecordBatch`、browser root/evidence、基于扫描结果的 ready/degraded health、reconcile 小全量 refresh deltas 与 Bookmarks watch refresh deltas，并提供 `resetIndex()` 供 runtime 记录 user-clear / health-repair 等 reset diagnostics；profile/browser diagnostics 到 roots/evidence 的转换已复用 `IndexedSourceProfileDiagnosticsService`，health/roots/evidence 同轮 diagnostics 读取也通过 `IndexedSourceSnapshotCacheService` 去重，避免重复读取 Bookmarks 文件。默认仍不扫描真实浏览器文件，且已补测试直接覆盖 disabled diagnostics、scan 与 watch handler 不读取 Chromium `Bookmarks`；runtime source 通过轻量 `browser-bookmarks-source-config` 动态读取统一 provider config，只有用户显式启用 `browser-bookmarks` 或官方 `touch-browser-data.browser-bookmarks` provider 后才进入 enabled path；enabled/disabled evidence 会标记为 `touch-browser-data` runtime bridge/provider lifecycle，并携带 linked/configured/enabled/disabled provider ids、enablement reason 与 provider metadata，同时继续暴露 persistent plugin-owned indexing 尚未完成。watch route 目前已补 source-level filter，并复用 SDK watch path basename helper 只对 `Bookmarks` 文件变化刷新，避免 profile root 下其他配置文件变化触发 refresh；持久化 rebuild、真实 OS watcher root 注册与用户级 clear 仍未完成。产品归属已调整为官方插件 provider：CoreApp skeleton 只作为 runtime 迁移与诊断样板，后续应迁出到 browser-data 官方插件并通过插件权限声明进入根结果。
- `CoreBoxEvents.search.indexingDiagnostics` 已通过 typed transport 暴露统一 diagnostics snapshot，供 Settings/CoreBox 后续消费；`settings.indexedSource` SDK 已新增通用 runtime maintenance 入口，映射 `AppEvents.indexedSource.diagnostics/reset/reconcile/scan`，让 Settings 和后续 source 管理 UI 通过 `sourceId` 触发 diagnostics、reset、reconcile 与 scan，而不是为 File/App/Browser 各写私有 IPC。Settings 维护按钮现在通过 `resolveIndexedSourceMaintenanceActions()` 过滤不可执行动作，disabled / unsupported / permission-required / admission-invalid source 不再展示为可直接维护。
...
- Diagnostics snapshot 已开始合并 runtime memory task state：每个 source 可带最近一次 `lastScan`、`lastWatch`、`lastReconcile`、`lastReset` 结果，包含成功统计、失败摘要、四类维护任务的 jobId/queuedAt，以及 reconcile reason/rootCount；同时新增 bounded in-memory `recentTasks`，按最近优先保留 scan/watch/reconcile/reset 的 kind/status/jobId/queuedAt/error/summary，作为 Settings/trace 的 source-level task history 过渡层。SDK 已提供 `IndexedSourceRuntimeTaskJobFactory`、`buildIndexedSourceScanTaskState()`、`appendIndexedSourceTaskHistory()`、`updateIndexedSourceTaskState()` 与 `DEFAULT_INDEXED_SOURCE_TASK_HISTORY_LIMIT`，CoreApp runtime 和后续官方插件 indexed source 应复用同一 job identity、newest-first / bounded 裁剪和 last* task state 更新规则；scan 已开始通过 SDK builder 生成 lastScan 与 history entry，避免各 source 重复手写 jobId/queuedAt、lastScan/lastWatch/lastReconcile/lastReset 与 history 组合逻辑。Diagnostics snapshot 也新增 source-level `progress` 字段，统一表达 stage、current/total、百分比、estimatedRemainingMs、estimatedCompletionAt、averageItemsPerSecond、speedSampleCount 与 estimateBasis；这些字段只用于观测，不作为持久 SoT，也不写入 JSON sync payload。
- Batch scan/reconcile、single-source scan/reconcile 与 watch route 已在 runtime 层通过 SDK helper `resolveIndexedSourceTaskEligibility()` 执行 source admission 与 health guard：`getIndexedSourceAdmissionIssues()` 非空、缺 scan/watch/reconcile capability、health 为 disabled/unsupported/permission-required/error、或 permissionState 为 denied/promptable 的 source 会被跳过并进入 skipped 统计，同时写入 `lastScan` / `lastWatch` / `lastReconcile` 的 `skipped:*` evidence。Root-based watch routing 已改用 SDK helper `resolveIndexedSourceWatchRootRoute()` 校验命中的 `IndexedSourceRoot.permissionState`，denied/promptable root 只会产生 `root-permission:*` skipped evidence，不会进入 source handler；路径归一化与平台大小写规则也不再是 CoreApp 私有逻辑。Browser Bookmarks 等 high privacy source 因此不会仅靠 adapter 自觉返回空，也不能通过 Settings 单源维护入口绕过批量 guard，而是在统一调度层被拦住。
...
- Advanced Settings 的 File Index 页面已开始展示 source recovery chip，消费 SDK `resolveIndexedSourceRecoveryRecommendation()`，把 contract issue、权限/禁用、progress failed/stalled、recent task failure/skipped 与 degraded/error health 转成统一恢复建议。该 chip 只做诊断提示，真正 scan/reconcile/reset 按钮仍由 `resolveIndexedSourceMaintenanceActions()` 控制可用性，避免 UI 建议绕过 runtime guard。
...
- Index worker / write worker flush retry、buffer、latest snapshot、idle/result/failure snapshot 与 evidence 映射已下沉到 `@talex-touch/utils/search` SDK primitive：`IndexedWriteFlushRetryService` 统一处理 pending backlog delay、backlog threshold、exponential retry、jitter 与 retry count；`IndexedWriteBufferService` 统一处理 pending/inflight enqueue、take、commit、rollback 与 size 统计，`IndexedEntryKeyedWriteBufferService` 则支持从 worker payload 自身解析 key 后 enqueue，避免每个 source 为 fileId/url/id 等 payload key 重写一层 buffer；`buildIndexedWriteFlushBatchMetrics()` 统一从 flush batch 生成 numeric metadata，FileProvider 的 `withContent` 只保留 File worker payload predicate；`IndexedWriteFlushSnapshotService` 只记录最近一次 flush snapshot 与 `checkedAt`，`buildIndexedWriteFlushIdleSnapshot()` 统一把 unavailable/no-pending/flush-in-progress 等 idle 状态映射为 trace snapshot，`buildIndexedWriteFlushResultSnapshot()` 统一把 executor result 映射为 trace snapshot，`buildIndexedWriteFlushFailureRetryMetadata()` 统一失败重试 delay/reason metadata shape，`buildIndexedWriteFlushFailureSnapshot()` / `getIndexedWriteFlushResultFromError()` 统一把 thrown error、attached flushResult、pending/inflight size 与 retry metadata 合成为 failed snapshot，`IndexedWriteFlushEvidenceService` 只把 snapshot 转为 `IndexedSourceEvidence` 的 ready/degraded 状态、itemCount 与 metadata，不承载 worker readiness、SQLite persist 或 SearchIndex/FTS 语义。CoreApp 仅保留兼容 re-export，FileProvider 只在 `FileProviderIndexFlushRetryService` 中注入 SQLite busy 判断，并把结果映射为现有 `sqlite-busy-retry` / `flush-failed` reason；`FileProviderIndexFlushBufferService` 现在只提供 file worker result 的 `fileId` key selector，File runtime 直接消费 SDK idle-snapshot/result-snapshot/failure-snapshot/evidence primitives，并把 `worker-not-ready`、SQLite busy 分类等 File 专属状态留在 adapter 边界，避免后续 source 的 worker flush 各写一套 backoff/buffer/snapshot/evidence。
- Source reset executor 已下沉到 `@talex-touch/utils/search` 的 `IndexedSourceResetExecutorService` SDK primitive，并新增 `buildIndexedSourceResetOperationReasonPrefix()` / `buildIndexedSourceResetOperationReason()` 纯 helper：统一生成 `remove-by-provider` 与 `scan-progress-reset` 这类 reset operation reason。SDK executor 支持构造级默认 `operationReasonNamespace` 与单次 reset 覆盖，FileProvider runtime reset 只在 adapter 边界声明一次 `file-index` namespace，仍保留真实 DB scan_progress 删除、SearchIndex worker cleanup、worker readiness 与 reset result 适配。
- Source integrity policy 已下沉到 `@talex-touch/utils/search` 的 `IndexedSourceIntegrityService` SDK primitive：统一处理 source rows 与 indexed rows 的比例判断、缺索引/部分索引时的 reset request、aligned 状态下的 orphan cleanup 调用、duration、SDK-standard snapshot、adapter-safe snapshot 映射、source-specific row field rename 与 diagnostics evidence 映射。FileProvider 现在只注入 FTS/files row count、runtime reset、`keyword_mappings` orphan cleanup 以及 File evidence id/label/reason/metadata 命名，`ftsRows` / `filesRows` / `orphanedKeywordsRemoved` 字段映射由 SDK helper 生成；真实 DB 查询、SearchIndex/FTS 表语义和 File 专属 keyword cleanup 仍留在 FileProvider 边界。
- Source progress store 已下沉到 `@talex-touch/utils/search` 的 `IndexedSourceProgressStoreService` SDK primitive：统一处理 completed root 读取后的 pending-root summary、completed-root store 不可用时全 root pending summary、空 delete/upsert 跳过、upsert readiness gate 与 upsert result。FileProvider 的 `scan_progress` 适配现在只保留 Drizzle select/delete、store availability、worker readiness 与 `upsertScanProgress` 注入；真实表结构和 SearchIndex worker 仍留在 CoreApp 边界。
...
- Scan strategy policy 已下沉到 `@talex-touch/utils/search` 的公共 `resolveIndexedScanStrategy()` SDK primitive：统一把 watch roots 与 completed path set 拆分成首次 full scan roots 与 existing-root reconciliation roots。FileProvider scan strategy service 仍负责读取 completed paths、让出 event loop、记录 timing/logging；后续路径型 source 不应重复实现同样的 completed-root 分流。
- Auto-scan preflight policy 已下沉到 `@talex-touch/utils/search` 的公共 `resolveIndexedAutoScanPreflight()` SDK primitive：统一 disabled、initializing、missing-context、no-paths、app-busy、search-active 与 interval skip reason 的优先级。FileProvider 仍负责读取 `scan_progress`、检查 `appTaskGate` / search activity、执行 `deviceIdleService` idle/battery gate；SDK 只消费快照并返回 source-agnostic 允许/跳过决策。
- Write flush executor/runtime scheduler/idle snapshot/failure snapshot/evidence 已下沉到 `@talex-touch/utils/search` 通用 primitive：`IndexedWriteFlushExecutorService` 统一处理 buffer take/rollback/commit、readiness gate、capacity wait、persist delegation、duration recording 与 source-agnostic result metadata；`buildIndexedWriteFlushBatchMetrics()` 统一处理 batch numeric metadata；`mapIndexedWriteFlushExecutorResult()` 统一处理 executor result 到 adapter status / numeric metadata 字段的纯映射；`IndexedWriteFlushRuntimeService` 统一处理 flush timer、unavailable/no-pending/flush-in-progress idle 记录、in-progress defer、失败 retry 调度与成功后的 drain remaining；`resolveIndexedWriteFlushRuntimeConfig()` 统一处理 base/backlog delay、flush defer、backpressure queue 与 retry backoff 默认值；`buildIndexedWriteFlushIdleSnapshot()` 统一处理 idle snapshot 拼装，`buildIndexedWriteFlushFailureRetryMetadata()` 统一处理 failed snapshot retry delay/reason metadata，`buildIndexedWriteFlushFailureSnapshot()` 统一处理 failure snapshot 拼装；`IndexedWriteFlushEvidenceService` 统一把 latest flush snapshot 映射为 source diagnostics evidence。CoreApp 仅保留兼容 re-export；FileProvider 的 `FileProviderIndexRuntimeService` / `FileProviderIndexFlushExecutorService` 现在只保留 SQLite busy 分类、worker-not-ready 日志、`persistAndIndex` 注入、File 状态映射与 evidence id/label 适配，DB persist / SearchIndex worker / FTS 语义仍留在既有边界。
- Path-record write planning 已下沉到 `@talex-touch/utils/search` 的公共 `IndexedWritePlanService` SDK primitive：统一处理 incoming path records 的 insert/update/unchanged 拆分、timestamp tolerance、normalized manual summary，并允许 source 注入 update-record shaping；`chunkIndexedWriteRecords()` 统一写入记录批次拆分和正整数 chunk size 归一化，避免 update/reconciliation adapter 各自复制 chunk helper；`takeIndexedWriteRecordChunk()` 统一动态 batch window 取片、offset 和 chunk size 归一化，避免 full-scan adapter 手写 slice 窗口；`buildIndexedWriteManualContext()` / `buildEmptyIndexedWriteManualSummary()` 统一手动触发路径归一化、manual total 与空结果 summary，避免 incremental adapter 自己拼 manual path set；`toIndexedWriteDate()` / `toIndexedWriteTimestamp()` 也提供统一 Date / number / string / invalid-value 归一化，避免各 adapter 复制 ctime/mtime fallback 逻辑；`mapIndexedWritePathUpdateRecord()` 统一 incremental update record 的 existing-row identity、name/extension/size、ctime/mtime、type/isDir shaping；`mapIndexedWriteFullScanUpsertRecords()` 统一 full-scan path record 的 name/extension/size/ctime/mtime/lastIndexedAt/isDir/type upsert payload shaping；`mapIndexedWriteReconciliationUpsertRecords()` 统一 reconciliation add upsert record 的 path/name/extension/size/ctime/mtime/lastIndexedAt/isDir/type shaping；`mapIndexedWriteReconciliationDiskPayload()` / `mapIndexedWriteReconciliationDbPayload()` 统一 reconciliation worker disk/db diff payload 的 path/name/extension/size/ctime/mtime/id shaping，并保持 invalid timestamp fallback 为 `0`；`resolveIndexedWriteReconciliationDiff()` 统一 worker fallback 时的 add/update/delete diff，保留 duplicate disk path 忽略、`mtime > db.mtime` 更新判定与 reconciliation root scoped deletion 语义。CoreApp 仅保留兼容 re-export；FileProvider 的 `FileProviderIncrementalWritePlannerService` 现在只保留 File row type aliases、diffing 配置与 manual summary 输出，`FileProviderIncrementalWriteService` 只保留 buildRecord、existing rows 读取、insert/update delegation 与日志，manual context 与空 manual summary 复用 SDK helper；`FileProviderFullScanRunService` 只保留目录扫描、progress、insert delegation 与 completed-path reporting；`FileProviderReconciliationDiffService` 只保留 worker-first 调用与失败日志，`FileProviderReconciliationRunService` 只保留目录扫描、DB row 读取、diff delegation 与写入 orchestration，`FileProviderReconciliationInsertService` 只保留 upsert delegation、side-effect dispatch、runtime batch/delta/progress emission 与 source-specific reason wiring，chunk splitting 和 reconciliation update record shaping 也分别复用 SDK helper 与同一 `toIndexedWriteDate()` helper 处理 `ctime`/`mtime`；`FileProviderFullScanInsertService` 只保留动态 batch size 读取、upsert delegation、sleep pacing、side-effect dispatch 与 runtime batch/progress emission，record window 切片复用 SDK helper；index worker scheduling 复用 timestamp helper 处理 worker payload 时间戳，但 `lastIndexedAt` 当前时间语义、invalid timestamp 当前时间 fallback 与大文件 background dispatch 仍由 CoreApp 写入适配层负责。该 primitive 只面向 File、Obsidian、VSCode 这类路径型 source，Browser Bookmarks/History 等非路径 source 不应强套这套 diff 模型。
- Index worker file payload shaping 已下沉到 `@talex-touch/utils/search` 的公共 `mapIndexedWriteWorkerFilePayload()` SDK helper：统一把 id/path/name/displayName/extension/size/ctime/mtime 映射成 worker file payload，并让 source adapter 注入 invalid timestamp fallback。FileProvider index worker scheduling 现在只保留数据库上下文检查、immediate/deferred large-file split、`:background-content` reason suffix、worker dispatch 与 failure logging；SearchIndex/FTS、真实 worker、DB persist 与 large-file 策略不进入 SDK。
- File source record mapping 已下沉到 `@talex-touch/utils/search` 的公共 `mapIndexedFileSourceRecord()` SDK helper：统一把 path/name/displayName/extension/size/mtime/type/isDir 这类路径型 source row 映射为 `IndexedSourceRecord` 的 recordId、stableKey、title、path、mtime、size 与 metadata，并通过 `toIndexedSourceRecordTimestamp()` 保持 invalid/empty timestamp 输出 `undefined` 的既有 runtime emission 语义。FileProvider 现在只注入 source id，DB persist、SearchIndex/FTS、worker scheduling 与 runtime callback 仍留在 CoreApp adapter 边界；Obsidian、VSCode 等后续路径型 source 可复用该 helper，Browser Bookmarks/History 等非路径 source 不应强套该 record row shape。
...
- Write runtime emission 已下沉到 `@talex-touch/utils/search` 的公共 `IndexedWriteRuntimeEmitterService` SDK primitive：统一把已写入 records 映射为 `IndexedSourceRecordBatch`、add/change delta、delete delta path 与 progress snapshot，也可在没有 emit sink 的 scan/watch handler 中直接 build record batch 或 add/change/delete delta；emitter 支持构造级 `defaultDeltaReason` 与调用级 reason 覆盖，避免每个 source adapter 在重复 emit 路径里手写同一 reason；不依赖 SQLite、SearchIndex、File row 或 worker 类型。FileProvider full scan insert、reconciliation insert/update/delete、stale cleanup delete、App scan batch、App/File watch delta、Browser Bookmarks scan/reconcile/watch refresh 与 Quicklinks scan/reconcile/watch skeleton 现在复用该 emitter，自己只保留 File/App/Bookmark/Quicklink row mapper、upsert/update/delete persistence 与 source-specific reason；Obsidian、VSCode 后续补漏写入不应各自手写 record batch / delta / progress 输出协议。
...
- 已开始：WatchEventRouter 已补 source handler / store delta failure isolation 与 route result 统计，并将 applied delta、handler/store failure 与 skipped source 写入 `lastWatch.jobId/queuedAt` diagnostics；`lastWatch` 与 recent task history 构建已下沉到 SDK `buildIndexedSourceWatchTaskState()`，保持 runtime 只负责路由统计与 job identity。
- 已开始：Reconcile result/failure/skipped 的 `lastReconcile` 与 recent task history 构建已下沉到 SDK `buildIndexedSourceReconcileTaskState()`，CoreApp 仍只负责 reconcile 统计、request reason/rootCount 和 job identity 注入。
- 已开始：Reset result 的 `lastReset` 与 recent task history 构建已下沉到 SDK `buildIndexedSourceResetTaskState()`，CoreApp 仍只负责 reset store boundary、source-local reset 调用与 job identity 注入。
...
- 已开始：IndexingRuntime 已将最近 scan/watch/reconcile/reset result 合并进 diagnostics snapshot，作为 Settings/trace source-level task evidence；四类任务都带 runtime `jobId/queuedAt`，并进入 bounded `recentTasks` 历史。生产初始化通过 `SqliteIndexingTaskStateStore` 持久化到 `indexed_source_task_state`；2026-06-24 focused slice 已补 `durationMs`、`reason`、`trigger`、`attempt`、`errorCode`、`errorMessage` 等低敏审计字段与 Settings task chip 展示，后续仍需 packaged Settings evidence 和 durable scheduler 长尾。
- 已开始：Settings diagnostics JSON evidence gate 已补 `settings:indexing-diagnostics:verify`，可读取 CoreBox indexing diagnostics payload 或 packaged probe envelope，复用 Settings recent task chip helper 校验 `duration/trigger/reason/attempt/code` 已在 chip values/summary 可见；该 gate 不替代真实 packaged Settings 截图或录屏。
- 已开始：Packaged Settings diagnostics evidence 采集入口已补 `visible:experience:indexing-diagnostics-probe`，可 attach 到已运行 packaged profile 或启动 isolated userData，读取 typed `app:indexed-source:diagnostics`、打开 Settings File Index source detail，并落盘 diagnostics / verifier / DOM / screenshot artifacts；attach mode 对真实 profile 保持只读，不执行 scan/reset/reconcile/FTS rebuild/schema migration。isolated launch 可显式使用 `--seedRecentTaskEvidence` 注入低敏 seeded recent task，或使用 `--runMaintenanceAction scan|reconcile|reset` 通过 typed maintenance IPC 产生受控 runtime recent task；`--fixtureRoot` 会约束 FileProvider base roots、启动前检查 packaged bundle marker，并在 roots 未生效时 fail gate，避免把默认 Home roots 误收为受控 scan/reconcile evidence。这些模式证明 packaged UI / chip / verifier / typed maintenance 链路，不能替代自然真实 profile evidence。
...
- 已开始：`AppIndexedSource.scan()` 从 AppProvider 维护任务返回 `IndexedSourceRecordBatch`，AppProvider 的 scan batch 已复用公共 `IndexedWriteRuntimeEmitterService.buildBatch()`，runtime `ScanScheduler` 可通过 `SearchIndexStoreAdapter` 写入现有 SearchIndexService。
...
- 已开始：SDK 补了 Quicklinks、Browser Bookmarks、Browser History 与 System Settings 的 descriptor template factory，固定 owner、permission scope、storage、privacy、capabilities 与 clear/rebuild 语义，作为新增 source 的 admission 样板；Quicklinks 已开始注册 low-privacy runtime skeleton，但 Browser Data/System Settings 仍不可只靠模板宣称具备完整索引、watch、clear/rebuild 或 UI 管理能力。
- 已开始：新增 `QuicklinksIndexedSource` runtime skeleton 并注册到 CoreApp runtime。它复用 `createQuicklinksIndexedSourceDescriptor()` 与 `IndexedWriteRuntimeEmitterService`，支持可注入 quicklink snapshot 的 scan batch、reconcile/watch change deltas、source-level health/evidence、reset/open/clear handler contract；默认空源只报告 `quicklinks-empty` degraded diagnostics，不读取插件 storage，也不伪装已有内容。CoreApp 已新增 `quicklinks-source-config`，把 `quicklinks`、`touch-browser-bookmarks.quicklinks` 与 `touch-dev-toolbox.dev-toolbox` 作为 linked provider ids 统一映射到 source enablement；Quicklinks 作为 low-privacy source 默认保持 enabled，但显式禁用 `quicklinks` provider 会让 source 报告 `quicklinks-provider-disabled`，显式启用任一 linked official provider 会恢复 enabled。真实官方插件持久 feed、用户级 clear/rebuild UI 与 Settings evidence 仍待继续。
...
- 已开始：File watch delta 已从 path-only 事件推进到 add/change 返回 `record`、delete 返回 `stableKey/path`；watch path normalize 与平台默认 watch depth 已抽到通用 `normalizeIndexedWatchPath()` / `getIndexedWatchDepthForPath()`，watch roots 合并、ownership 与 pending permission root 过滤已抽到通用 `resolveIndexedWatchRootSet()` / `isIndexedWatchPathOwned()` / `filterIndexedWatchPendingPermissionPaths()`，auto scan preflight skip reason、new/stale root 与 lastScannedAt 计算已抽到通用 `resolveIndexedAutoScanPreflight()` / `resolveIndexedScanEligibility()`，full-scan/reconciliation root 分流已抽到通用 `resolveIndexedScanStrategy()`，`FileProviderPathService`、`FileProviderWatchService` 与 `FileProviderScanStrategyService` 现在只保留 CoreApp 平台类型、真实 watcher、配置持久化、scan_progress/completed path 读取、idle/battery gate、yield 与 logging 适配；watch delta 的 path 合并、delete 覆盖、prepare-flush gate 与串行 flush 调度已抽到通用 `IndexingWatchDeltaQueueService`，`FileProviderIncrementalQueueService` 现在只保留 File 语义适配和 `manual` 标记合并；增量 delete 的 path normalize、existing-row lookup、DB delete、SearchIndex remove 与成功日志编排已抽到通用 `IndexedWriteDeleteExecutorService`，reconciliation 删除也开始复用同一 executor 的 resolved-record 删除入口；reconciliation worker reconcile 与 main-thread fallback diff 已拆到 `FileProviderReconciliationDiffService`，统一产出 added/updated/deleted 计算结果；existing-root reconciliation 的 DB row read、directory scan、diff orchestration、delete/update/insert delegation、progress、stats 与 completed-path reporting 已拆到 `FileProviderReconciliationRunService`；reconciliation delete/update 的 source-level delta emission 与 changed/deleted result reporting 已拆到 `FileProviderReconciliationDeleteService` / `FileProviderReconciliationUpdateService`，其中 update 已复用通用 `IndexedWriteRuntimeEmitterService` 输出 change delta，FileProvider 只注入现有 delete/update executor 与 record mapper；stale watch-root cleanup 已拆到 `FileProviderCleanupDeleteService`，复用同一 delete executor 形状并注入 files-table delete、embedding cleanup、`scan_progress` cleanup、SearchIndex removal 与 cleanup progress；full scan root scanning、scan progress、event-loop yield、file-row payload mapping、insert delegation 与 completed-path reporting 已拆到 `FileProviderFullScanRunService`，其中 upsert record shaping 已复用通用 `mapIndexedWriteFullScanUpsertRecords()`；full scan insert/upsert 的 AIMD batch、idle pacing、side-effect、record batch、progress 与 added 统计已拆到 `FileProviderFullScanInsertService`，其中 record batch / progress 已复用通用 `IndexedWriteRuntimeEmitterService`；reconciliation add 的 chunk/upsert、side-effect、record batch、delta 与 progress 编排已拆到 `FileProviderReconciliationInsertService`，其中 record batch / add delta / progress 已复用同一 emitter，`upsertSearchIndexFiles` 与 source delta reason 语义保持注入；增量 add/change 的 record build、existing lookup、insert/update 执行与 manual summary 编排已拆到 `FileProviderIncrementalWriteService`；insert/update/unchanged/manual summary 规划已抽到通用 `IndexedWritePlanService`，`FileProviderIncrementalWritePlannerService` 只保留 File row update record 适配；增量 insert/upsert 的 persist callback、side-effect dispatch 与成功日志已抽到通用 `IndexedWriteInsertExecutorService`；分块 update 的 idle/capacity wait、逐条更新、刷新 updated rows、side-effect dispatch 与进度日志已抽到通用 `IndexedWriteUpdateExecutorService`；insert/update 后的 keyword/icon extension 处理与 content indexing 调度已抽到通用 `IndexedWriteSideEffectService`，`FileProviderWriteSideEffectService` 现在只保留 File 命名适配；index worker context gate、chunk dispatch、deferred dispatch 与 failure isolation 已抽到通用 `IndexedWorkerSchedulerService`，`FileProviderIndexSchedulerService` 现在只保留 file row 到 worker payload 的映射和 large-file background-content 策略；index worker result 到 `PersistEntry` 的 progress/fileUpdate/indexItem 映射已抽到通用 `IndexedWorkerPersistEntryMapperService`，`FileProviderIndexPersistEntryMapperService` 现在只保留 File worker result 适配；worker status summary、短 TTL cache、并发 status load 去重与失败不缓存已抽到通用 `IndexedWorkerStatusSnapshotService`，`FileProviderWorkerStatusService` 只保留 File worker status loader 适配；index worker flush 的 backlog delay、sqlite-busy retry、exponential backoff、jitter、失败 retry reason 与 retryable/fallback 分类决策已抽到通用 `IndexedWriteFlushRetryService`，`FileProviderIndexFlushRetryService` 只保留 SQLite busy classifier 与 FileProvider reason 命名；index worker flush 执行、worker readiness gate、DB backpressure、persistAndIndex、commit/rollback 与耗时记录已抽到通用 `IndexedWriteFlushExecutorService`，flush batch numeric metadata 已抽到通用 `buildIndexedWriteFlushBatchMetrics()`，FileProvider 只保留 `withContent` predicate；executor result 的 adapter status / numeric metadata 映射已抽到通用 `mapIndexedWriteFlushExecutorResult()`，executor result 到 trace snapshot 的映射已抽到通用 `buildIndexedWriteFlushResultSnapshot()`；flush timer、in-progress guard、idle snapshot、失败 retry scheduling、成功后的 drain remaining 与 runtime config 默认值已抽到通用 `IndexedWriteFlushRuntimeService` / `resolveIndexedWriteFlushRuntimeConfig()`，`FileProviderIndexRuntimeService` 只保留 FileProvider 返回语义、状态映射、字段命名适配与日志适配；最近 flush snapshot 已抽到通用 `IndexedWriteFlushSnapshotService`；pending/inflight buffer 的 enqueue、take、commit、rollback 与 size 统计已抽到通用 `IndexedWriteBufferService`，payload 自带 key 的 worker buffer 已抽到通用 `IndexedEntryKeyedWriteBufferService`，`FileProviderIndexFlushBufferService` 只保留 file worker result 的 `fileId` key selector；`file-provider:index-flush` evidence 暴露 flushed / worker-not-ready / failed、pending/inflight、retry reason、error 与 duration；真实 watcher 注册、DB insert/update/delete SQL、内部文件表 persist、flush trace wiring 与 FTS 写入/删除语义暂时保留在 FileProvider/SearchIndex worker 边界。
...
- 已开始：新增只读 SQLite/FTS migration preflight：`pnpm -C "apps/core-app" run search:index-migration:preflight -- --db <sqlite.db> --output <report.json>` 会用 `PRAGMA query_only = ON` 输出并落盘 required tables、`scan_progress` source-scope、FTS shadow table、provider parity、keyword/meta orphan、meta coverage 与 `migrationDryRun` plan。该 gate 仅用于迁移审批前 evidence，包含预计影响 rows、rollback 与 verification，不执行 schema/data migration、不清理 FTS、不改变 `scan_progress`。
- 已开始：`scan_progress` 读写边界已具备 source-scoped schema 兼容层。`FileProviderScanProgressService`、runtime reset cleanup 与 SearchIndex worker upsert 会先检测 `scan_progress` 是否存在 `source_id`；若存在则按 `file-provider` + path 读/删/写，若不存在则保持旧 `path primary key` 行为。该兼容层不执行 schema migration，但为后续 `(source_id, path)` durable migration 降低 hard-cut 风险。
- 已开始：`scan_progress` source-scoped schema 增加受控 migration helper 与 CLI。`pnpm -C "apps/core-app" run search:scan-progress-migration -- --db <sqlite.db>` 默认只读输出 approval/schema/data rewrite/rollback/verification 与 blocker；只有显式传 `--execute --confirm-source-scope-migration` 才会调用 `runScanProgressSourceScopeMigration()`，用 `BEGIN IMMEDIATE`、staging table 与 `scan_progress_path_only_backup` 把 path-only rows backfill 到 `file-provider` scope，并创建 `PRIMARY KEY(source_id, path)` + path index。生产启动路径现在也复用同一 helper：正常 DB migrations 后若 plan ready 则迁移，若遇到 unsafe rows / backup / staging blocker 则保留旧表并继续 old/new schema 兼容。
- 已开始：新增生产迁移 readiness auditor：`pnpm -C "apps/core-app" run search:production-migration-readiness -- --output <report.json>` 只读检查 schema/resource migrations/journal/runtime FTS ownership。`0024_search_index_runtime_store.sql` 已接入 `indexed_source_task_state` 与 durable-shape `search_index` FTS5 resource migration，`0025_scan_progress_source_scope.sql` 已接入 source-scoped `scan_progress` resource shape；当前真实仓库 source-read-only readiness 为 ready。该 gate 用于补齐高风险迁移设计 evidence，不执行 schema/data migration。
- 已开始：新增 SQLite/FTS ownership 副本仿真 CLI：`pnpm -C "apps/core-app" run search:fts-ownership-simulate -- --db <sqlite.db> --output <fts-simulation.json>` 使用 `VACUUM INTO` 生成 simulation DB，只在副本上应用候选 durable FTS DDL，并输出 `search_index` required columns、shadow tables、`search_index_meta`、keyword indexes、legacy `file_fts` retain-unchanged policy 与 row-discard/full-reindex impact。该 gate 可验证 FTS 迁移草案接近真实执行，但不替代 production resource migration 接入或真实 execute approval。
- 已开始：新增 `scan_progress` source-scope 副本仿真 CLI：`pnpm -C "apps/core-app" run search:scan-progress-simulate -- --db <sqlite.db> --output <simulation.json>` 使用 `VACUUM INTO` 生成 simulation DB，只在副本上执行受控 helper，并输出源库前后 snapshot 不变、copy 迁移后 `PRIMARY KEY(source_id, path)`、backup rows 与 gate blockers。该 gate 可验证迁移草案接近真实执行，但不替代生产 DB migration 接入或真实 execute approval。
- 已开始：packaged Settings diagnostics probe 的 source detail 选择已修正为按目标 source 行匹配，isolated packaged `dist/mac-arm64/tuff.app` 可采 Settings/File Index detail DOM 与截图；isolated profile 缺 recent task history 时 `settings:indexing-diagnostics:verify` 会正确失败。`--seedRecentTaskEvidence` 受控 isolated run 已证明 Settings File Index detail 能展示 duration / trigger / reason / attempt / code；`--runMaintenanceAction reset` 受控 isolated run 已证明 typed packaged runtime reset 会产生 recent task 并展示 duration / trigger / reason；重包后的 `--runMaintenanceAction scan|reconcile --fixtureRoot <tmp>` 受控 isolated run 已通过 bundle marker preflight、fixture roots gate 与 verifier，证明 packaged runtime scan/reconcile recent task 可进入 Settings detail。2026-06-30 plain Node CDP isolated durable scan evidence 进一步证明同一 isolated packaged DB 中 `indexed_source_task_state=1`、`scan_progress=1`、`search_index=35`，Settings verifier gate passed，preflight/simulation gate passed；该证据仍是 isolated profile，不替代自然真实 profile evidence。
- 待继续：真实 SQLite/FTS/`scan_progress` migration/preflight evidence 与真实 packaged Settings 截图/录屏 artifact 仍需按高风险清单单独确认；当前 `visible:experience:indexing-diagnostics-probe` npm/tsx cold-start 路径在本机仍可能触发 packaged Electron `SIGABRT`，macOS crash report 显示 abort 发生在 AppKit/HIServices `_RegisterApplication` / `NSApplication.sharedApplication`，早于 CoreApp main-process JS 日志，需要修复后再作为稳定复采入口；legacy `file_fts` 在 R3 本批保留不改，不迁移/删除/重建；`indexed_source_task_state`、`search_index` FTS5 与 source-scoped `scan_progress` resource migration 已接入，scan worker、incremental DB persist、index worker flush trace、integrity-triggered reset、Settings diagnostics JSON verifier、packaged probe 入口与 isolated maintenance reset/scan/reconcile fixture/durable scan evidence 已具备 runtime task/store focused evidence，durable scheduler 长尾后续继续补齐。
...
- 已开始：新增 `BrowserBookmarksIndexedSource` runtime skeleton，提供 descriptor/admission/health/evidence/reconcile skipped 结果，不改变 `touch-browser-data` 插件即时搜索行为；默认 disabled diagnostics 已对齐官方 provider consent required。
...
- 已开始：`BrowserBookmarksIndexedSource` 在显式 enabled/test path 下可 yield `IndexedSourceRecordBatch` 并输出 browser root/evidence，health 会基于扫描结果进入 ready/degraded；scan batch、reconcile 小全量 `IndexedSourceDelta` 与 Bookmarks 文件 watch refresh deltas 已复用公共 `IndexedWriteRuntimeEmitterService`，只保留 Chromium scanner、bookmark mapper 与 high-privacy enablement 在 CoreApp 边界；watch handler 现在通过 `shouldHandleWatchEvent()` 只接受 `Bookmarks` 文件事件，其他 profile 文件会被 runtime 记录为 source-watch-filtered；`resetIndex()` 会返回 source-level reset diagnostics，search-index 清理由 runtime store boundary 统一执行。默认 runtime 注册仍保持 disabled/provider consent required，且 enabled 判断已拆到 `browser-bookmarks-source-config` 从统一 provider config 动态读取；只有 `browser-bookmarks` 或官方 `touch-browser-data.browser-bookmarks` 被显式启用时才会扫描真实浏览器文件，避免 high privacy source 未经用户同意读取；lifecycle evidence 会标记 `touch-browser-data` runtime bridge/provider lifecycle，并解释 enablement reason 与 linked/configured/enabled/disabled provider ids，但 persistent plugin-owned indexing 仍是后续工作。
- 已开始：`touch-browser-data` 已声明官方 `touch-browser-data.browser-bookmarks` push provider，带 `root-results` / `browser-data` scope 和用户同意要求；同时已声明 metadata-only `browser-bookmarks` indexed source lifecycle intent，用于先固定 official-plugin / high privacy / disabled-by-default / fs.read+fs.index 边界。CoreApp Browser Bookmarks focused test 已读取插件 manifest 并钉住 provider id、indexedSourceId、permission scopes、defaultState、requiresUserConsent 与 metadata-only indexed source intent 必须和官方 provider descriptor 对齐。它仍是即时只读插件入口，尚未升级为 runtime-registered indexed source。
...
1. Quicklinks source：已开始 runtime skeleton，并完成官方 quicklink provider `indexedSourceId` 链接、source enablement helper 与 disabled diagnostics；下一步接官方插件持久 feed、用户级 clear/rebuild 与真实 Settings evidence。
...
当前 SDK 已提供 Quicklinks / Browser Bookmarks / Browser History / System Settings descriptor template；Quicklinks 已有低隐私 runtime skeleton，后续接入必须继续补真实 source lifecycle、health/evidence、Settings enablement 与验收 evidence；其它 source 仍必须先复用模板，再补实际 lifecycle，不能只靠模板宣称 source 已完成。
...
下一刀继续收口三件事：一是把 Quicklinks skeleton 接到官方插件持久 feed 与用户级 clear/rebuild，完成低隐私 source 的首个官方插件样板；二是把 Browser Bookmarks 的 indexed enable/disable、授权提示、clear/rebuild 与 Bookmarks 文件 watch 小全量刷新迁到官方 browser-data 插件 provider，并让 metadata-only `manifest.indexedSources` 升级为真实 runtime registration；三是继续把 FileProvider 内部 worker 写入、incremental queue、scan_progress 与 FTS reset 迁向统一 task/store 边界。
