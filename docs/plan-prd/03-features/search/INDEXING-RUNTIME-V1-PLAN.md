...
- 已开始：`scan_progress` source-scoped schema 增加受控 migration helper。`planScanProgressSourceScopeMigration()` 只读输出 approval/schema/data rewrite/rollback/verification 与 blocker，`runScanProgressSourceScopeMigration()` 仅在显式调用时用 `BEGIN IMMEDIATE`、staging table 与 `scan_progress_path_only_backup` 把 path-only rows backfill 到 `file-provider` scope，并创建 `PRIMARY KEY(source_id, path)` + path index。该 helper 尚未接入生产 DB migrations，也未执行真实 profile 迁移。
- 待继续：SQLite/FTS durable ownership、source-scoped `scan_progress` 生产 migration 接入与 packaged Settings evidence 仍需按高风险清单单独确认；scan worker、incremental DB persist、index worker flush trace 与 integrity-triggered reset 已具备 runtime task/store focused evidence，durable scheduler 长尾后续继续补齐。
