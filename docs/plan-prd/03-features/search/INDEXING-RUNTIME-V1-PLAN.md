...
- Index worker / write worker flush retry、buffer、latest snapshot 与 evidence 映射已下沉到 `@talex-touch/utils/search` SDK primitive：`IndexedWriteFlushRetryService` 统一处理 pending backlog delay、backlog threshold、exponential retry、jitter 与 retry count；`IndexedWriteBufferService` 统一处理 pending/inflight enqueue、take、commit、rollback 与 size 统计，`IndexedEntryKeyedWriteBufferService` 则支持从 worker payload 自身解析 key 后 enqueue，避免每个 source 为 fileId/url/id 等 payload key 重写一层 buffer；`IndexedWriteFlushSnapshotService` 只记录最近一次 flush snapshot 与 `checkedAt`，`IndexedWriteFlushEvidenceService` 只把 snapshot 转为 `IndexedSourceEvidence` 的 ready/degraded 状态、itemCount 与 metadata，不承载 worker readiness、SQLite persist 或 SearchIndex/FTS 语义。CoreApp 仅保留兼容 re-export，FileProvider 只在 `FileProviderIndexFlushRetryService` 中注入 SQLite busy 判断，并把结果映射为现有 `sqlite-busy-retry` / `flush-failed` reason；`FileProviderIndexFlushBufferService` 现在只提供 file worker result 的 `fileId` key selector，File runtime 直接消费 SDK snapshot/evidence primitives 并把 `worker-not-ready` 等 File 专属状态交给 source evidence 映射，避免后续 source 的 worker flush 各写一套 backoff/buffer/snapshot/evidence。
...
- Write flush executor/runtime scheduler/evidence 已下沉到 `@talex-touch/utils/search` 通用 primitive：`IndexedWriteFlushExecutorService` 统一处理 buffer take/rollback/commit、readiness gate、capacity wait、persist delegation、duration recording 与 source-agnostic result metadata；`mapIndexedWriteFlushExecutorResult()` 统一处理 executor result 到 adapter status / numeric metadata 字段的纯映射；`IndexedWriteFlushRuntimeService` 统一处理 flush timer、unavailable/no-pending/flush-in-progress idle 记录、in-progress defer、失败 retry 调度与成功后的 drain remaining；`IndexedWriteFlushEvidenceService` 统一把 latest flush snapshot 映射为 source diagnostics evidence。CoreApp 仅保留兼容 re-export；FileProvider 的 `FileProviderIndexRuntimeService` / `FileProviderIndexFlushExecutorService` 现在只保留 SQLite busy metadata、worker-not-ready 日志、`persistAndIndex` 注入、File 状态映射与 evidence id/label 适配，DB persist / SearchIndex worker / FTS 语义仍留在既有边界。
