...
- 已开始：新增 `QuicklinksIndexedSource` runtime skeleton 并注册到 CoreApp runtime。它复用 `createQuicklinksIndexedSourceDescriptor()` 与 `IndexedWriteRuntimeEmitterService`，支持可注入 quicklink snapshot 的 scan batch、reconcile/watch change deltas、source-level health/evidence、reset/open/clear handler contract；默认空源只报告 `quicklinks-empty` degraded diagnostics，不读取插件 storage，也不伪装已有内容。真实官方插件持久数据、provider enable/order 到 source enablement 的完整闭环、用户级 clear/rebuild UI 仍待继续。
...
1. Quicklinks source：已开始 runtime skeleton；下一步接官方插件持久 feed、provider enable/order 关联、用户级 clear/rebuild 与真实 Settings evidence。
...
下一刀继续收口三件事：一是把 Quicklinks skeleton 接到官方插件持久 feed、provider enable/order 与用户级 clear/rebuild，完成低隐私 source 的首个官方插件样板；二是把 Browser Bookmarks 的 indexed enable/disable、授权提示、clear/rebuild 与 Bookmarks 文件 watch 小全量刷新迁到官方 browser-data 插件 provider，并让 metadata-only `manifest.indexedSources` 升级为真实 runtime registration；三是继续把 FileProvider 内部 worker 写入、incremental queue、scan_progress 与 FTS reset 迁向统一 task/store 边界。
