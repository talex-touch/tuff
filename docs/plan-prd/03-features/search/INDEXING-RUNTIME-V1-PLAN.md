...
- Index worker / write worker flush retry 与 buffer 已下沉到 `@talex-touch/utils/search` SDK primitive：`IndexedWriteFlushRetryService` 统一处理 pending backlog delay、backlog threshold、exponential retry、jitter 与 retry count；`IndexedWriteBufferService` 统一处理 pending/inflight enqueue、take、commit、rollback 与 size 统计。CoreApp 仅保留兼容 re-export，FileProvider 只在 `FileProviderIndexFlushRetryService` 中注入 SQLite busy 判断，并把结果映射为现有 `sqlite-busy-retry` / `flush-failed` reason；`FileProviderIndexFlushBufferService` 只保留 fileId 适配，避免后续 source 的 worker flush 各写一套 backoff/buffer。
