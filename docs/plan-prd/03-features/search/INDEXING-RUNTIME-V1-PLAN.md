...
- File indexed source 已把 FileProvider 私有 indexing status 适配为 `IndexedSourceDiagnostics.progress`，让统一 diagnostics snapshot 能直接携带 File source 的 stage、current/total、百分比、ETA、预计完成时间、吞吐、样本数与估算来源；后续 Settings 应优先消费 source-level progress，而不是继续只读 FileProvider 私有 progress payload。
...
- 已开始：File indexed source 已提供 `getProgress()`，runtime diagnostics 会透传为 `IndexedSourceDiagnostics.progress`；这一步只统一 transport/diagnostics 合同，Settings 的统一 source progress 渲染仍是后续切片。
