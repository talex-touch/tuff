...
- `QuickOpsSessionManager` 已从 legacy provider 中拆出到 `apps/core-app/src/main/modules/box-tool/addon/quick-ops/quick-ops-session-manager.ts`，集中管理 power blocker、倒计时、番茄钟、秒表、清洁屏幕 overlay、会话清理与 session change 订阅；CoreApp 当前仅通过 `QuickOpsRuntimeHost` 暴露 runtime host boundary。
