...
- `plugins/touch-quickops` 官方迁移插件已新增：manifest 显式声明 `search.root-results` 与 root provider，并已收紧为显式 QuickOps / system / network / file / disk / diagnostics / session control 触发词；当前已承接 QuickOps CoreBox root-result 入口和结果渲染，包括 capability、sessions、auditRecent、system info、tuff diagnostics、disk space、directory usage / deep directory usage、network status、local ip、port status、DNS query、file hash、file Base64、recent download、common directory、path format、format text、battery status、system proxy，以及低风险状态控制入口 `stop-all-sessions`、`stop/pause/resume timer`、`stop/pause/resume pomodoro`、`stop clean screen`、`stop keep-awake`、`stop system-awake`、`pause/resume/lap/reset stopwatch`。插件 manifest 现在接受 `text` 与 `files` 输入，文件 Hash/Base64/path format 会在没有显式路径时使用首个有效 Files input。只读结果通过 `globalThis.quickOps` 过渡 facade 取数；低风险状态控制通过注入的 `flow.dispatch()` 发送到已有 QuickOps Flow target；插件侧 Flow/AI adapter contract 现在输出脱敏 `flowAdapterTrace`，确认型动作只展示需要 App UI `confirmationToken` 的提示，高风险 `kill port` 类请求 fail-closed 且没有 dispatch payload。插件不执行确认型或高风险动作、不自行生成 token、不绕过 Flow confirmation，也不 fallback 到私有 IPC。CoreApp 保留 QuickOps runtime host、host capability、typed bridge、Flow confirmation 和 evidence gate；CoreApp 不再保留 QuickOps natural-language resolver，CoreApp 托盘不再作为 QuickOps 状态入口，CoreApp 文件搜索 provider 不再承载 QuickOps Hash/Base64 execute 动作；后续应继续把业务 runtime、session manager、settings 入口、插件侧状态/诊断面板与可扩展编排逐步迁入官方插件或插件专属 runtime boundary。
