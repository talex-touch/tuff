...
- Nexus QuickOps 用户文档与开发者文档已补：`apps/nexus/content/docs/guide/features/quickops.*.mdc` 覆盖 CoreBox 使用入口、常用命令、Ops 场景速查、官方插件 `quickops settings` 设置摘要入口、本地策略、安全边界、插件扩展边界、何时写插件和未完成项；`apps/nexus/content/docs/dev/api/quickops.*.mdc` 覆盖插件 `quickOps` 只读 facade、transport domain SDK、扩展路径选择、Flow 扩展模式、插件自有 Ops checklist、内置 QuickOps 贡献落点、可写 settings host capability 待设计边界和扩展建议；Guide / Dev / API / Plugin Context 入口已接入对应链接与全局 `quickOps` 上下文说明。该文档切片不替代真实平台 evidence、packaged quit evidence、真实 AI UI 自然语言编排、高风险执行治理或企业集中策略 / 审计 / 组织级锁定。
- `plugins/touch-quickops` 官方迁移插件已新增：manifest 显式声明 `search.root-results` 与 root provider，并已收紧为显式 QuickOps / system / network / file / disk / diagnostics / settings / session control 触发词；当前已承接 QuickOps CoreBox root-result 入口和结果渲染，包括 capability、sessions、auditRecent、system info、tuff diagnostics、disk space、directory usage / deep directory usage、network status、local ip、port status、DNS query、file hash、file Base64、recent download、common directory、path format、format text、battery status、system proxy、`quickops settings` 只读设置摘要，以及低风险状态控制入口 `stop-all-sessions`、`stop/pause/resume timer`、`stop/pause/resume pomodoro`、`stop clean screen`、`stop keep-awake`、`stop system-awake`、`pause/resume/lap/reset stopwatch`。插件 manifest 现在接受 `text` 与 `files` 输入，文件 Hash/Base64/path format 会在没有显式路径时使用首个有效 Files input。只读结果通过 `globalThis.quickOps` 过渡 facade 取数；设置摘要只读取 capability / diagnostics，不写入 host policy；低风险状态控制通过注入的 `flow.dispatch()` 发送到已有 QuickOps Flow target；插件侧 Flow/AI adapter contract 现在输出脱敏 `flowAdapterTrace`，确认型动作只展示需要 App UI `confirmationToken` 的提示，高风险 `kill port` 类请求 fail-closed 且没有 dispatch payload。插件不执行确认型或高风险动作、不自行生成 token、不绕过 Flow confirmation，也不 fallback 到私有 IPC。CoreApp 保留 QuickOps runtime host、host capability、typed bridge、Flow confirmation、settings read path 和 evidence gate；CoreApp 不再保留 QuickOps natural-language resolver，CoreApp 托盘不再作为 QuickOps 状态入口，CoreApp Tools 设置页不再作为 QuickOps 配置入口，CoreApp 文件搜索 provider 不再承载 QuickOps Hash/Base64 execute 动作；后续应继续把可写 settings host capability、业务 runtime、session manager、插件侧状态/诊断面板与可扩展编排逐步迁入官方插件或插件专属 runtime boundary。
