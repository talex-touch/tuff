...
**业务目标**：Nexus 语音通道的能力边界、模型清单、限流与灰度策略可以**不改客户端版本**地调整；新用户登录后，**在云端包完成同步并激活的前提下**，其第一次语音输入即使用云端当前生效的语音路由；若包尚未激活，则保留内置 Nexus 路由；协议细节（端点、字段、分帧）不以明文出现在客户端产物里。
...
运行时还必须拒绝 `endpoint.baseUrl` 与 `getRuntimeNexusBaseUrl()` 具有不同 origin 的 `nexus-pack` 描述符；该跨 origin 门禁在采集开始前执行，相关拒绝路径由运行时测试覆盖。
