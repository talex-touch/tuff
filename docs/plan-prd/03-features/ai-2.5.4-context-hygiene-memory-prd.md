...
> 更新时间：2026-07-11
> 状态：P0/P1 Closure Implemented / P2-P3 Follow-up Open
> 目标版本：2.5.4
...
2026-07-11 P0/P1 closure：`.trellis/tasks/07-10-r9-2-context-hygiene-p0-p1-closure/` 下 6 个子任务实现 `6/6`。host-owned `contextInvoke/contextStream` 与单一 assembler 已让受治理 summary/recent turns/Memory/retrieval 按预算进入 Provider messages；Memory governance、搜索/来源审计、编辑后重评估、原子 replace+tombstone 与 CompressionSnapshot CAS/no-delete 已闭合。Workflow 每 run 独立 session，OmniPanel/Assistant 使用 `new + light`，CoreBox 保留 new/continue/stateless，并补 active widget 单次派发/latest-request-wins。分级 evidence manifest 已通过隐私扫描，isolated packaged Electron 覆盖 CoreBox `new / retrieval` 与 Assistant `new / light`；OmniPanel/Workflow packaged 与 `real-profile` 明确保留 open。workspace/project memory 在稳定 `scopeRef` migration 前继续 fail-closed；本轮未执行 SQLite schema/data migration，自动长期记忆仍关闭。

2026-07-11 continuation follow-up：显式 continue 指向 archived/expired/idle session 时不再复用旧主键，而是创建新 session 与 metadata-only checkpoint；只允许策略校验后的最新 CompressionSnapshot 或 legacy summary 进入新 ContextPackage。blocked/missing summary 保持 excluded/unavailable 且当前输入可继续；`ContextContinuationSummary` 已进入 utils/tuff-intelligence typed contract，并由官方插件 widget 显示边界 reason。旧 raw turns/Memory 不跨 session，packaged/real-profile evidence 仍 open。

2026-07-12 tombstone explain follow-up：Intelligence Audit 对 `memory-tombstoned` exclusion 单独计数，并在 inline summary 与 explain drawer 使用中英文 reason/notice；unknown reason 保持安全机器字符串。UI 只消费 package log 中的 sourceType/sourceId/reason/tokenEstimate，不读取或展示已删除 MemoryItem、prompt 或 turn 原文。focused UI contract 已通过，packaged/real-profile evidence 仍 open。

2026-07-13 OmniPanel packaged context follow-up：visible OmniPanel built-in AI action 已通过受控 Local/Ollama-compatible Provider 取得 `owner=omni-panel`、`new / light` session/package log、单次 Provider dispatch 与 Ready result。metadata-only artifact/manifest privacy scan passed；当前四入口 packaged context evidence 已闭合，real-profile 与 gated `scopeRef` migration 继续开放。

2026-07-13 multilingual token-estimate follow-up：ContextHygiene 与 2.5.3 LocalKnowledgeEngine 已移除各自的 UTF-16 `length / 4` 副本，统一使用 code-point-aware 保守估算；连续 ASCII 近似保持 `ceil(n / 4)`，CJK 等非 ASCII 至少各计 1、emoji 更高。历史 turn/chunk 读取时取 `max(stored estimate, current content estimate)`，因此旧低值不能绕过 ContextPackage/knowledge hard budget，同时不触发 SQLite row rewrite。该值仅用于宿主预算治理，不替代 Provider tokenizer，也不用于声称精确计费 token。

2026-07-13 ContextPackage optional-budget follow-up：预算溢出例外已从 `items.length === 0` 改为显式绑定正常 `current_input`。正常当前输入即使单项超过预算仍保留；continuation/compression/legacy summary、recent turn、Memory 与 retrieval 始终执行严格聚合预算。secret/private 当前输入被策略排除后，不会把“首个 item”例外转移给 oversized summary；拒绝项只记录 metadata-only `token-budget-pruned`，不写入正文。focused ContextHygiene/context-execution 2 files / 63 tests、targeted ESLint 与 CoreApp node typecheck passed；本项不宣称包含 mandatory current input 时 ContextPackage 总值一定低于预算。

2026-07-13 runtime budget normalization follow-up：共享 `normalizeContextTokenBudget()` 已封住 `Math.floor/Math.max` 的非有限值传播。ContextHygiene 对省略、非 number、`NaN` 与正负无穷回退到既有默认 1,600；LocalKnowledgeEngine 对同类非法值 fail-closed 到 1，且不接受数字字符串 coercion。有限 number 仍向下取整并以 1 为下限；typed SDK/API、有效预算上限策略、quota 与 schema 不变。focused estimator/knowledge/ContextHygiene/context-execution 4 files / 93 tests、targeted ESLint 与 CoreApp node typecheck passed。

2026-07-13 degraded fallback privacy follow-up：host-owned `contextInvoke/contextStream` 在 ContextHygiene/SQLite prepare 失败时仍保留安全 current-input-only 可用性降级，但现在会先复用本地 secret policy。命中 `api_key/token/secret/password`、私钥等既有规则的 raw input 直接以稳定 `CONTEXT_CURRENT_INPUT_POLICY_BLOCKED` fail-closed，Provider invoke/stream 均不执行，错误、日志与 metadata 不含原始 secret；安全输入仍返回 `context_prepare_failed` summary 并执行原有 fallback。focused ContextHygiene/context-execution 2 files / 68 tests、targeted ESLint 与 CoreApp node typecheck passed；本项不新增 secret pattern、远程 DLP 或数据库可用性承诺。

2026-07-13 degraded budget-metadata follow-up：`context_prepare_failed` current-only fallback 的 `IntelligenceContextExecutionSummary.tokenBudget` 与 Provider options metadata 现也复用 `normalizeContextTokenBudget()`；非 number、`NaN` 与正负无穷统一回退 1,600，有限小数向下取整。由此正常包、knowledge build 与数据库降级 summary 不再存在第二套非有限值路径。focused ContextExecution 1 file / 10 tests、targeted ESLint 与 CoreApp node typecheck passed。

2026-07-13 privacy precedence follow-up：`PrepareContextTurnInput.privacyLevel` 不再能够把 host 检出的 secret 降级为 `normal` 或 `private`。secret classifier 现在先于 caller hint：命中既有规则的 turn 一律标记 `secret`、以 canonical redacted marker 写入 SQLite、从 ContextPackage 排除并记录 `secret-policy-blocked`；安全内容仍可显式声明 `private` 并保留 `private-policy-blocked` 语义。turn/package log/metadata 均不含 raw secret。focused ContextHygiene/context-execution 2 files / 71 tests、targeted ESLint 与 CoreApp node typecheck passed；本项不扩充 secret pattern 或改变安全 caller 的显式隐私控制。

2026-07-13 credential classifier follow-up：共享 ContextHygiene secret policy 已补齐设计文档列出的两类明确凭证：大小写不敏感的 `Bearer` scheme + 至少 16 个标准 token 字符，以及具有 JSON-like base64url header/payload 和非平凡签名的三段 JWT。两者统一触发 MemoryPolicy reject、turn secret/redaction/ContextPackage exclusion 与数据库降级 invoke/stream fail-closed；`Bearer token`、`Bearer <token>` 和 `foo.bar.baz` 保持 safe，避免文档占位符误报。focused ContextHygiene/context-execution 2 files / 78 tests、targeted ESLint 与 CoreApp node typecheck passed；本项不宣称通用 DLP 或覆盖所有 vendor credential。
