...
统一 core/renderer 的 Release 资产命名与校验规则，extensions bundle 由 store system 负责分发与更新（UpdateSystem 不下载/安装），确保 UpdateSystem 与 GithubUpdateProvider 可在无需隐式猜测的情况下解析平台/架构/sha256/签名信息，并支持向后兼容旧资产。
...
当前 GitHub Actions release workflow 还会生成平台前缀资产名，例如：

```
windows-latest-beta-tuff-2.4.11-SNAPSHOT.6-setup.exe
macos-latest-beta-tuff.app.zip
ubuntu-latest-beta-tuff-2.4.11-beta.6.deb
```

- `version`：发布版本号（包含渠道后缀，如 `2.4.7-beta.11`）；Windows beta 的 builder 文件名可使用 `SNAPSHOT.N`，runtime / manifest release version 仍保持原 beta semver。
- `platform`：`win32` | `darwin` | `linux`
- `arch`：`x64` | `arm64`
- `ext`：`exe` | `dmg` | `AppImage` | `deb` | `zip`
- `-setup`：仅允许用于安装包（例如 Windows `setup.exe`），解析时不作为平台/架构判断依据
- 平台前缀资产是当前 workflow 兼容格式；新命名优先继续向 canonical `tuff-core-*` 收敛，但 validator 必须能识别两者以支持现有 release 资产。
...
- 平台无关，必须通过 manifest 的 `coreRange` 指定兼容的 core 版本范围
- 由 store system 使用，UpdateSystem 会忽略该资产
...
Release 资产中必须包含 `tuff-release-manifest.json`，用于声明所有更新资产的校验与兼容信息。该 manifest 是 GitHub Release 资产，不应伪装成 Nexus 的某个 `platform/arch` 下载资产；Nexus 平台资产矩阵只保存可下载安装包。
...
- `artifacts[]`：
  - `component`：`core` | `renderer` | `extensions`（extensions 仅供 store system，UpdateSystem 忽略）
...
- **Nexus sync**：同步到 Nexus 时必须跟随 GitHub manifest asset redirect，跳过 `latest*.yml` / `builder-debug.yml` 等 updater/debug metadata，只把安装包写入平台下载矩阵，并用 manifest 回填 sha256。
