...
> 目标版本: 2.5.0  
> 适用范围: `apps/core-app`、`apps/pilot`、`apps/nexus`、`packages/tuff-intelligence`、`plugins/touch-intelligence`

## 1. 背景与问题

Tuff 已经具备较完整的 Intelligence 底座：CoreApp 有 Provider、Capability、Prompt、Audit、Quota、Agent 与 Workflow；CoreBox 有 `touch-intelligence` 插件；Pilot 已形成独立 Chat-first 应用；Nexus Provider Registry / Scene 已进入通用供应商模型。

当前问题不是“缺少 AI 代码”，而是用户可感知入口分散、稳定能力边界不清、配置安全口径需要锁定：

- CoreBox AI 入口只提供轻量问答结果项，尚未形成桌面 AI 主链路。
- Pilot 能力完整但独立部署，与 CoreApp 桌面入口关系需要定义。
...
工程目标：复用现有 `intelligence.invoke()`、`@talex-touch/tuff-intelligence`、Provider/Capability 配置与审计能力，不重写 Pilot，不把 Nexus Scene runtime 编排塞进 2.5.0。

North Star：

- 用户有一个明确的桌面 AI 主入口，而不是在 CoreBox、Pilot、Assistant、设置页之间寻找能力。
...
| 方向 | 当前现状 | 2.5.0 预期 |
| --- | --- | --- |
| AI 入口 | CoreBox 有 `touch-intelligence` 插件；Pilot 是独立 Chat-first 应用；Assistant 仍偏实验 | 明确 CoreBox / OmniPanel 为桌面主入口，Pilot 作为高级 Chat / DeepAgent 能力来源 |
...
- Pilot 联动：作为高级 Chat / DeepAgent 入口或能力来源，不替代 CoreApp 桌面主入口。
...
- 不重写 Pilot，不把 `apps/pilot` 合并进 CoreApp。
...
需要长对话、工具卡、Websearch、DeepAgent 的用户进入 Pilot。2.5.0 只定义 CoreApp 到 Pilot 的产品关系与入口策略，不要求改造 Pilot runtime。
...
| user prompt | CoreBox / Pilot | 默认不保存完整原文 |
...
- Pilot 联动失败不阻塞 CoreApp Stable 文本能力。
- Nexus Scene runtime 未完成全量产品化时，2.5.0 Stable 只承载 `/api/v1/intelligence/invoke` 登录态薄接口；Scene 编排继续作为 2.5.x 后续。
