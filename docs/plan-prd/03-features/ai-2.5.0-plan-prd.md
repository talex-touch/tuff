...
- 登录态 Nexus AI Invoke：官方 app 登录后，CoreApp 默认 `tuff-nexus-default` provider 通过 `/api/v1/intelligence/invoke` 调用 Nexus Intelligence Provider / Provider Registry 配置；未登录时显式失败并由本地 SDK fallback 到其它可用 provider。
...
- Intelligence TTS：`audio.tts` 作为 Beta 能力进入统一 Intelligence 链路，首批用于翻译结果朗读；插件侧优先调用 typed `intelligence.ttsSpeak()`，复用 provider/quota/audit/fallback 与进程内短期缓存，不允许插件私有直连模型。
...
- `image.generate`、`image.edit`、`audio.stt`、`audio.transcribe`、`video.generate`；`audio.tts` 已提升为 Beta，但不作为 Stable 承诺。
- Nexus Scene runtime 全量 orchestration 产品化。2.5.0 只落地登录态 Intelligence invoke 薄接口，不把 Scene 编排作为 Stable 主卖点。
...
- 统一能力入口继续是 `intelligence.invoke(capabilityId, payload, options)`；TTS 朗读使用 typed `intelligence.ttsSpeak(payload)` 封装 `audio.tts`、data URL 归一与进程内短期缓存。
- 2.5.0 Stable 能力只承诺：
  - `text.chat`
  - `text.translate`
  - `text.summarize`
...
- 2.5.0 Beta 能力新增：
  - `audio.tts`
- 翻译插件合同：
  - `touch-translation` 默认通过 `text.translate` 调用 Intelligence，并将 `traceId / provider / model` 透传到 widget。
  - 翻译结果朗读通过 typed `intelligence.ttsSpeak()` 调用 Intelligence；插件只消费可播放音频结果，不接触 provider secret，也不自行拼 provider 请求。
  - 仅启用外部翻译源时才要求 `network.internet`；仅使用 Tuff Intelligence 翻译/朗读时只要求 `intelligence.basic`。
- Provider 配置合同：
...
- Nexus Provider / Scene 合同：
  ...
  - 新使用场景进入 Scene。
  - 2.5.0 新增 `/api/v1/intelligence/invoke` 登录态薄接口，复用 Nexus Intelligence Provider / Provider Registry mirror、secure-store credential、审计与 fallback。
  - CoreApp 默认 `tuff-nexus-default` 作为 Nexus app-auth provider adapter，未登录时返回 `NEXUS_AUTH_REQUIRED` 并进入本地 SDK fallback。
  - 2.5.0 不要求完成 Scene runtime orchestration 的全量产品化。
- Workflow / Action 合同：
...
| M0.5 Nexus Invoke | Nexus 登录态 Intelligence invoke + CoreApp Nexus provider adapter | app 登录态可调用 `text.chat`，未登录可 fallback，本地不保存 Nexus token 到普通配置 |
...
| M3.5 TTS Beta | `audio.tts` runtime dispatch、Nexus/OpenAI-compatible adapter、typed `ttsSpeak` API、翻译结果朗读入口 | 翻译结果可朗读，TTS 调用带 trace/audit 与进程内短期缓存，provider unsupported 时有可见失败与 fallback |
...
- 翻译插件 Intelligence-first 路径可返回 traceId；翻译结果可通过 typed `ttsSpeak` / `audio.tts` 朗读，失败时不影响翻译结果展示。
...
- AI 联动失败不阻塞 CoreApp Stable 文本能力；历史 Pilot 相关联动不作为 Stable 阻塞项。
- Nexus Provider / Scene runtime 未完成全量产品化时，2.5.0 Stable 只承载 `/api/v1/intelligence/invoke` 登录态薄接口；registry 保持配置与架构约束，Scene 编排继续作为 2.5.x 后续。
