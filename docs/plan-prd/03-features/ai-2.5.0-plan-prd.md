...
- OmniPanel Selection AI：划词翻译、摘要、改写、解释。
...
- Workflow 模板：剪贴板整理、会议纪要/摘要、文本批处理。
...
| M1 体验设计 | CoreBox AI Ask 与 OmniPanel Selection AI 的交互规格 | 有入口、状态、错误、权限、审计字段定义 |
| M2 安全收口 | BYOK 密钥从普通配置剥离到 secure-store / `authRef` | 明文 key 不再作为稳定存储合同 |
| M3 场景闭环 | CoreBox 问答、OmniPanel 划词、Workflow 模板最小闭环 | 产品验收场景可复现 |
| M4 验收冻结 | 文档、回归、风险与后续 2.5.x backlog 固化 | PRD / TODO / CHANGES / INDEX 同步 |
