...
- 形成稳定的跨插件流转链路，文本/文件/JSON 的端到端成功率 ≥ 95%。
- 关键链路可观测（审计日志 + 失败原因记录），对开发者可自证、可回放。
- Flow SDK 与主进程统一走 typed transport，消除 raw event 直连。

### 1.2 质量约束

- 只允许通过 `FlowEvents` + domain SDK 触发流转，禁止新增 raw channel。
- 权限与授权必须前置校验，且每次流转产生审计记录。
- payload 必须校验大小与类型，超过限制直接拒绝并返回可理解错误。
- 存储必须遵守 Storage Rule：授权与会话落地以 SQLite 为 SoT，JSON 仅作为同步载荷。
- 性能预算：选择面板打开 ≤ 200ms，投递延迟 ≤ 100ms（同机）。

### 1.3 回滚与兼容策略

- 提供 Flow 功能开关（全局禁用或按插件禁用），禁用时 SDK 返回 `FLOW_DISABLED`。
- 失败时必须支持 fallbackAction（如复制到剪贴板），保证用户可继续操作。
- 兼容旧插件：保留 SDK 接口签名，内部可降级为“仅面板选择 + 复制”模式。
