# PRD: DivisionBox 交互容器能力深化 (v1.0)（历史方案 / 分层入口）
...
- 对插件开放完整生命周期事件，形成可预期的 attach/detach 行为。
- DivisionBox 会话具备稳定的状态管理与恢复能力，满足高频使用场景。
- 与 Flow Transfer 联动可验收，并形成最小可用的开发者文档与示例。

### 1.2 质量约束

- 主进程与渲染进程状态必须强一致，状态切换必须可追踪、可回放。
- 所有跨层调用必须通过 typed transport/SDK，不新增 raw event。
- keepAlive 必须有资源上限与 LRU 回收，避免无界占用。
- 性能预算：首帧 ≤ 250ms，缓存恢复 ≤ 120ms。

### 1.3 回滚与兼容策略

- 提供功能开关，允许回退到“非缓存、即时销毁”的安全模式。
- SDK 接口保持兼容，降级时返回可识别的状态码与提示。
- 若 DivisionBox 异常，允许 fallback 到 CoreBox 内嵌视图或直接打开插件 UI。
