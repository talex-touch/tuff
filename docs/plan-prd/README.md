...
- Nexus 插件发布闭环已进入 App 内管理切片：不新增 `/dashboard/plugins` 页面路由，Nexus 继续以 `/dashboard/assets` 承载发布入口；Dashboard plugin API 支持登录态与 `plugin:read/plugin:publish` API key，创建插件可同请求初始化首版，CoreApp Store 新增 `/store/publisher` 发布管理 tab，并通过 `auth:nexus-upload` 由主进程代理 `.tpex` multipart 上传。
...
- Tuff 2.5.0 AI 板块已进入 dev/2.5.0 实现切片：定位仍是桌面 AI 入口收口版本，CoreBox / OmniPanel 是用户可感知主入口；CoreBox AI Ask 最小 Stable 切片已接入 `text.chat`、剪贴板图片 `vision.ocr -> text.chat` 与 Intelligence Session 交接上下文，`touch-intelligence` 会把 CoreBox 轮次绑定到稳定 `handoffSessionId` 供后续恢复/接续；新增登录态 Nexus AI invoke 路径，CoreApp 默认 `tuff-nexus-default` provider 会通过 `/api/v1/intelligence/invoke` 调 Nexus Intelligence Provider / Provider Registry mirror 与 secure-store 凭证，未登录时显式 `NEXUS_AUTH_REQUIRED` 并 fallback 到其它可用 provider；OmniPanel Writing Tools MVP 已新增划词 AI 翻译/摘要/改写/解释/Review、结果预览面板、transient Desktop Context Capsule 与 copy/retry/replace clipboard 二次确认；`touch-translation` 已默认通过 `text.translate` 走 Intelligence 并透传 traceId，翻译结果朗读接入 typed `ttsSpeak` / `audio.tts` Beta 与进程内短期缓存；下个重点继续落地 Workflow `Use Model` 节点、完整 Review Queue、3 个 P0 Workflow 模板与 TTS 队列/播放服务；Stable 只承诺文本 + OCR，Scene runtime 全量编排仍列为 2.5.x 后续。
...
- Windows App Search & Launch Beta 已开始落地：文件索引高级设置新增“本地启动区 / 应用索引管理”，复用 `settingsSdk.appIndex.listEntries/addPath/setEntryEnabled/removeEntry/diagnose/reindex` 管理手动应用条目；支持选择 `.exe/.lnk/.appref-ms`、粘贴 `%ENV%` 路径/UWP shell path/裸 AppID，添加后立即触发关键词重建与诊断；启动链路新增 `protocol` launchKind，并以白名单方式支持 `steam://rungameid/<appid>`。
...
- Steam 最小支持已接入 Windows AppProvider 扫描链路：通过注册表/常见 Steam 根目录、`libraryfolders.vdf` 与 `appmanifest_*.acf` 发现游戏，索引为 `bundleId=steam:<appid>`、`path/launchTarget=steam://rungameid/<appid>`、`launchKind=protocol`，不扫描游戏 exe、不引入游戏平台抽象。
...
- 2.4.10 Windows App 索引与基础 legacy/compat 收口：应用索引管理页、UWP/Store 诊断字段、Steam 最小 provider 与 `protocol` 启动白名单已进入实现态，仍需 Windows 真机 evidence 验收。
...
- Tuff 2.5.0 AI 桌面入口收口：PRD 已锁定版本定位与验收口径；CoreBox AI Ask 已先落最小 Stable 切片，OmniPanel Writing Tools MVP 已进入可验证实现，`touch-translation` 已通过 `text.translate` 走 Intelligence，并接入 typed `ttsSpeak` / `audio.tts` Beta 朗读与进程内短期缓存；继续推进 Workflow `Use Model` 节点、完整 Review Queue 与剪贴板整理/会议纪要/文本批处理模板，不得扩大到全量多模态、Assistant 默认启用或 Nexus Scene runtime 编排。
