...
- 搜索性能 telemetry 上报链路已落地：CoreBox 在最终 `session.end` 上报脱敏后的 `firstResultMs/providerTimings/providerResults/providerStatus`，默认开启并可在设置中手动关闭；登录后仅关联用户 ID，端侧 sanitizer 与 Sentry `beforeSend` 会剔除搜索明文、文件路径、剪贴板内容、邮箱、密钥、prompt/response、request/breadcrumb/stack frame 路径等敏感数据，Nexus Admin Analytics 已展示 Provider Performance；`search:trace:stats -- --output <stats.json>` 可从目标设备日志直接生成 `search-trace-stats/v1`，Windows acceptance template 会给出采样命令；这只代表观测链路完成，真实性能验收仍需目标设备 200 条样本通过 P95/slowRatio 门禁。
...
- Tuff 2.5.0 AI 桌面入口收口：PRD 已锁定版本定位与验收口径；CoreBox AI Ask 已先落最小 Stable 切片，OmniPanel Writing Tools MVP 已进入可验证实现，继续推进 Workflow `Use Model` 节点、完整 Review Queue 与剪贴板整理/会议纪要/文本批处理模板，不得扩大到全量多模态、Assistant 默认启用或 Nexus Scene runtime 编排。
