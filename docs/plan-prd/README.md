...
- 当前工作区口径：2026-06-05 当前 `master` 与 `origin/master` 仍无 ahead 差异（`origin/master...HEAD` 为 `0 0`），但 worktree 仍很脏：314 个 tracked 改动文件、25 个 untracked 文件；dirty 仍主要集中在 TuffEx 按需入口/组件拆分、Nexus 页面与文档适配、CoreApp renderer/i18n/UI 迁移、`intelligence-uikit` 与 TuffEx 脚本/helper 新文件。后续提交必须继续按 related-only 分批，先收 TuffEx/Nexus/renderer/Intelligence UI 已验证大切片，再回 P1-APP-DATA 的 File write/store boundary。
...
- 当前第一批剩余：TuffEx build/audit 与 Nexus typecheck/build 已在 Node 22.16 + pnpm 10.32 环境补证；下一步先补 Nexus/TuffEx visual smoke 的 CDP 截图证据，再把剩余 worktree 按 TuffEx 按需入口与 audit、Nexus 页面/文档适配、CoreApp renderer/i18n/UI 迁移、`intelligence-uikit` 消费侧迁移分批收口；随后回到 File write/store boundary、`scan_progress` / integrity reset durable job history、Browser Bookmarks 官方 `touch-browser-data` 插件 owned runtime source、Quicklinks 官方插件持久 feed + clear/rebuild UI、Everything SDK/CLI 最终策略与 Windows 真机 evidence。
...
4. 先收口当前 dirty worktree 的已完成 UI/SDK 切片：TuffEx build/audit 与 Nexus typecheck/build 已补证，接着补 Nexus/TuffEx visual smoke，再按 TuffEx 按需入口与 audit 脚本、Nexus 页面适配、CoreApp renderer/i18n/UI 迁移、`intelligence-uikit` 消费侧迁移分批处理未跟踪 TuffEx helper 文件。
