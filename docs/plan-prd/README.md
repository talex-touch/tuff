...
- Nexus Data Governance / Analytics：已具备共享治理事件/config、`/api/dashboard/governance/analytics`、Data Governance cockpit、插件私有 analytics、上传/存储/通知/Provider quota 关键指标与 quota risk summary、storage policy enforcement、secure credential reference 与共享 object storage executor；详细历史与文件级变更以 `01-project/CHANGES.md` 为准。当前验收缺口仍是真实浏览器证据、真实凭据/live send、直接 SMTP socket 或托管 relay、Web Push 生产 VAPID/relay、S3/OSS live storage、生产 D1 migration/backfill 与更完整运营大屏。
