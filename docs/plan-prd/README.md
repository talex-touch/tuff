...
- 当前第一批剩余：已推送代码基线与远端同步；CoreApp/TuffEx dialog trusted HTML boundary 与 Widget runtime sandbox evidence 已完成 focused 切片，下一步不再围绕脏代码切片收口，而是按风险顺序推进主路径 UI 语义控件与 `touch-music` 示例插件清理；随后回到 File write/store boundary、`scan_progress` / integrity reset durable job history、Browser Bookmarks 官方 `touch-browser-data` 插件 owned runtime source、Quicklinks 官方插件持久 feed + clear/rebuild UI、Everything SDK/CLI 最终策略与 Windows 真机 evidence。
...
4. 已推送代码基线与远端同步；CoreApp/TuffEx dialog trusted HTML boundary 与 Widget runtime sandbox evidence 已完成 focused 切片，后续优先推进主路径 UI 语义控件与 `touch-music` 示例插件清理。
