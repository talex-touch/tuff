...
- 发布准备：`2.4.10-beta.22` 已补中英文 release notes；`build-and-release` 仍支持 tag push 触发 beta pre-release，`v2.4.10-beta.21` 已存在，下一次新 beta 使用 `v2.4.10-beta.22`。本轮已合入 Widget/CLI 最新修复、兼容性复核记录，并延续 OmniPanel Gate、Utils Package CI 与 macOS beta build 准备结果；完整 `quality:pr` 仍被 `apps/pilot` 既有 lint 债务阻断，不得宣称全仓 PR gate 已绿。
...
- 跨平台兼容与占位实现深度复核已归档：`docs/plan-prd/report/cross-platform-compat-placeholder-deep-review-2026-05-13.md` 确认 CoreApp 平台能力、sync 密文载荷与 transport boundary 当前稳定；剩余 `2.4.11` 风险集中在 Pilot 兼容占位成功响应、CLI token 明文 JSON、插件 provider secret 普通 storage、插件 shell capability 诊断与生产样式大文件 SRP。
...
- 跨平台兼容深度复核（2026-05-13）：2026-05-10 报告中的 Pilot stat 假值、mock payment 默认成功与 touch-image localStorage 历史持久化已收口；CoreApp 平台能力保持显式 degraded/unsupported 合同，生产 raw send 直连未见新增命中，typed migration candidate 保持 `0`，retained raw definition 测试上限为 `265`；剩余 `2.4.11` 重点为 Pilot 兼容占位成功响应、CLI token 明文 JSON、插件 provider secret 普通 storage、插件 shell capability 诊断与 SRP 小切片。
...
- Pilot 兼容占位响应退场：`livechat/random`、prompt detail 与 catch-all 未实现接口必须改为明确 HTTP status、`unavailable + reason` 或迁移目标，避免前端把占位响应当作真实业务数据。
