...
- CLI token 与插件 provider secret storage 收口：文件权限缓解与 `usePluginSecret()` 迁移已推进；CoreApp secure-store 已 hard-cut Electron `safeStorage` / 系统钥匙串 / Credential Locker / libsecret，仅使用本机随机 root 密钥加密；旧 `safe-storage` envelope 不再解密，遗留 secret 清理提示与 packaged runtime build/sync evidence 仍待闭环。
