...
- **当前工作区事实已变化**：截至 2026-06-06，上一条已推送代码基线为 `ea0c2c93c test(search): add preview sdk benchmark command`，`HEAD -> master, origin/master, origin/HEAD` 且 `origin/master...HEAD` 为 `0 0`；本切片仅包含 docs/audit 文档同步，不改运行时代码。此前文档中“314 tracked + 25 untracked dirty worktree”的口径已过期。
- **验证环境口径**：默认 automation shell 仍是 Codex Node `v24.14.0`，且无裸 `pnpm` / `corepack`；但 `/tmp/talex-touch-node22-adhoc` 仍存在临时 ad-hoc signed Node `22.16.0` 和 pnpm `10.32.1` shim。由于本轮只做静态审计与文档同步，未重跑 package build/typecheck。
...
- **当前最值得继续的代码治理**：CoreApp/TuffEx dialog `messageHtml` 的可信 HTML 类型边界、Widget runtime `new Function` sandbox evidence、主路径 `div/span @click` 语义控件收口、`touch-music` 示例插件全量 TuffEx 样式入口与 dev asset 清理、Windows/macOS 真机 evidence。

## UI 适配度与完成度评分

| 模块 | 当前评分 | 判断 |
| --- | ---: | --- |
| CoreApp 桌面主产品 | 7.9 / 10 | 主线适配度正确：CoreBox、Settings、provider/source diagnostics 与 AI 入口都是工作台形态；短板是 dialog HTML boundary、部分旧式 clickable div/span、Widget runtime sandbox evidence 与 Windows/macOS 真机截图。 |
...
| TuffEx 组件体系 | 8.6 / 10 | `base.css`、组件子路径、局部 style、audit 脚本、DatePicker adaptive field 都在往可复用组件库方向走；主要缺口是 dialog trusted HTML API 与少数组件语义/a11y 细节。 |
...
| 架构健壮性 | 8.1 / 10 | 当前已推送代码基线回到同步状态，分批提交纪律已兑现。剩余风险从“混杂 dirty slice”转为“HTML/dynamic execution/platform evidence/示例插件质量长尾”。 |
...
- 下一步可以直接按高信号风险推进：HTML boundary、Widget sandbox evidence、平台真机 evidence、File write/store boundary。
...
- CoreApp legacy dialog：`apps/core-app/src/renderer/src/components/base/dialog/TPopperDialog.vue` 仍直接 `v-html="messageHtml"`。
- TuffEx dialog：`packages/tuffex/packages/components/src/dialog/src/TxPopperDialog.vue` 仍直接 `v-html="messageHtml"`。
- 同类 `TxTouchTip`、`TxBlowDialog`、CoreApp `TouchTip`、`TBlowDialog`、`TDialogMention` 应归为同一小切片。

建议：

- 新增 `TrustedHtml` / `EscapedHtml` 轻量品牌类型或 wrapper helper，让默认业务 API 走纯文本，只有内部明确可信来源能传 HTML。
- 不改变现有 UI 行为，优先给 dialog family 增加类型约束和 focused tests。
...
- 当前问题是 evidence 不够：需要 packaged/dev source 手动验收、sandbox facade regression、依赖 allowlist 和失败路径记录。
...
1. **先做 dialog trusted HTML 小切片**：覆盖 CoreApp legacy dialog + TuffEx dialog family，建立 `TrustedHtml`/默认文本 API 和 focused tests。
2. **补 Widget runtime sandbox evidence**：不急着重写 runtime，先把 sandbox facade、dependency allowlist、local/session storage facade、failure UI 和 packaged/dev source evidence 固化。
3. **做 UI 语义控件 P2 切片**：优先 CoreBoxRender、PluginList add/clear icon、Nexus LanguageToggle、touch-music player controls；mask/backdrop `@click.self` 可暂缓。
4. **清理示例插件质量长尾**：`touch-music` 迁到 TuffEx `base.css` + 子路径组件导入，移除 Vite starter asset；`touch-translation` 更新 storage 注释。
5. **回到 P1-APP-DATA**：docs/audit 文档同步切片收口后，可以继续 File write/store boundary、Browser Bookmarks official plugin lifecycle、Everything productionization、Quicklinks feed/UI evidence。
6. **补平台证据**：Windows App indexing / Everything registry PATH / CoreBox function key / 手动索引完成通知仍需要 Windows 真机 evidence；macOS release-blocking 也需保留独立手工回归记录。
...
- 未执行 package typecheck/build：本轮为静态审计与文档同步，不改运行时代码。
