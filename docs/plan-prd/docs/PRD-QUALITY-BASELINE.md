...
- 当前执行基线：root / CoreApp `2.4.13-beta.3`。历史 release 事实保留在 `../01-project/CHANGES.md` 与对应 Evidence Matrix；本文件只记录仍有效的质量边界。
