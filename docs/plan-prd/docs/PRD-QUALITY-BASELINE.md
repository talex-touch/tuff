...
- 2026-05-14 PR quality gate 调整：`quality:pr` lint 阶段改为 `lint:changed`，PR CI 中按 `GITHUB_BASE_REF` 与 `origin/<base>` 三点 diff 仅 lint PR 修改的 JS/TS/Vue 文件，避免既有全仓 lint 债务阻塞无关 PR；`quality:release` 仍保留全仓 lint，不降低正式 release gate。
- 2026-05-13 beta CI readiness 复核：`2.4.10-beta.22` 准备用于避开已存在的 `v2.4.10-beta.21` tag；Widget/CLI 最新修复与兼容性复核记录已合入，OmniPanel Gate、Utils Package CI 与 macOS beta build 准备结果延续有效，transport boundary retained raw definition 当前扫描上限为 `265`，typed migration candidate 保持 `0`。
