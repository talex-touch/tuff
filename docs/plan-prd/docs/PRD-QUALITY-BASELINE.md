...
- 当前执行基线：root / CoreApp `2.4.13-beta.1`。历史 release 事实保留在 `../01-project/CHANGES.md` 与对应 Evidence Matrix；本文件只记录仍有效的质量边界。
...
- 当前质量状态：公共包发布不再作为独立 Roadmap / blocker / evidence 项；版本变更后以 GitHub 自动发版 workflow 结果为准。R1 Release Integrity 仍被真实发布资产签名材料与生产 signing key 配置阻塞；R2 AI Stable 以 packaged Electron CoreBox 文本/OCR、provider routing 与固定失败路径 evidence 为 Stable 口径；R3 Indexing Runtime 仍需 durable job history、SQLite/FTS durable ownership 与 source-scoped `scan_progress` 迁移设计。旧 compat registry / legacy allowlist / size allowlist 已不在 live tree，不能再作为当前门禁或事实来源引用。vNext 路线见 `../04-implementation/Roadmap-vNext-2026-06-18.md`。
