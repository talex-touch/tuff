...
- 2026-06-06 工作区质量口径：当前 `master` 与 `origin/master` 已同步（`origin/master...HEAD` 为 `0 0`），上一条已推送代码基线为 `ea0c2c93c test(search): add preview sdk benchmark command`。提交仍必须按 related-only 组织；CoreApp/TuffEx dialog trusted HTML boundary 与 Widget runtime sandbox evidence 已进入 focused 证据切片，当前下一批优先级是 UI 语义控件、`touch-music` 示例插件清理、Windows/macOS 真机 evidence 与 Widget 扩展边界长尾，随后回到 File write/store boundary 与 Browser Bookmarks official lifecycle。
