...
- **本地 Git 文档提交钩子不可执行** — pre-commit 触发 `lint-staged` 时从错误的 `/Users/talexdreamsoul/Workspace/talex-touch/node_modules/` 解析模块，报 `Cannot find module .../lint-staged/bin/lint-staged.js`。本次仅在 `git diff --cached --check` 后使用 `HUSKY=0` 提交文档；workspace/依赖解析拓扑修复前，此钩子不能作为提交门禁。
