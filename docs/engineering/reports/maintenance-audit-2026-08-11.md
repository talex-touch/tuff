...
- **本地 Git 文档提交钩子不可执行** — pre-commit 触发 `lint-staged` 时从错误的 `/Users/talexdreamsoul/Workspace/talex-touch/node_modules/` 解析模块，报 `Cannot find module .../lint-staged/bin/lint-staged.js`。本次仅在 `git diff --cached --check` 后使用 `HUSKY=0` 提交文档；workspace/依赖解析拓扑修复前，此钩子不能作为提交门禁。跟踪：[#1564](https://github.com/talex-touch/tuff/issues/1564)。
...
- **明确功能/验收缺口不能靠任务状态掩盖** — BaseAnchor 共享 drip/bead 引擎及视觉/交互验收未完成；plugin-folder action 尚未证明能打开已安装插件根目录并给出用户可见失败反馈；发布日志仍需先恢复 `actionlint` 再采集桌面/窄窗截图。它们必须各自具备实现与运行验收，而非只保留任务标题。BaseAnchor 由 [#1565](https://github.com/talex-touch/tuff/issues/1565) 单独跟踪。
