...
- **未归属的工具输出仍留在工作区。** `.dsh-plugin-hub-adapter-staging/`、`.dsh-plugin-hub-install/`、`.dsh-plugin-hub-config-dump.yml`、`.dsh-plugin-hub-root.html` 未跟踪且不属于本审计提交。其生成者应建立任务与验证边界；无价值时由所有者删除，审计不得吞入或清理它们。[#1785](https://github.com/talex-touch/tuff/issues/1785) 负责该未归属输出边界。
