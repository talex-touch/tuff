...
- **全库文档质量门仍为红色。** `scripts/docs/verify-docs.mjs` 本轮报出两条 `DOC-TASK-CHILDREN`：`07-27-optimize-core-utility-plugins` 引用的 `08-22-autopaste-plugin-beta-e2e` 与 `08-05-search-audit-remediation` 引用的 `08-22-08-22-broaden-corebox-recommendations` 尚未成为可解析 child。二者均由 [#309](https://github.com/talex-touch/tuff/issues/309) 统一收敛；在 child task 真正受控或父项移除失效引用前，路线图不能当作绿色发布证据。
