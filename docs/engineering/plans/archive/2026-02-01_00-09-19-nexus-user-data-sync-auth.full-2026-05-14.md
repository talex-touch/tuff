...
### 本地 oplog 写入与加解密链路
- 写入顺序：业务数据 -> 生成 DEK -> payload 加密 -> item 落盘 -> oplog 记录。
- oplog 仅保存元数据与 op_hash，不保存明文 payload。

### 同步调度器接入点
- 触发点：oplog 写入后触发 push；应用启动/恢复触发 handshake + pull。
- 调度：使用 PollingService 作为定时对账，配合 WS 实时提醒。

### 迁移步骤与回滚
1. 备份旧本地数据与配置。
2. 将可同步数据导入 items/oplog，并补写 device_id。
3. 校验解密读写通过后切换到新链路。
4. 回滚：恢复旧数据目录并禁用同步开关。
...
### 单元测试
- 加解密：DEK/MK/DK 生成与轮换、明文元数据最小化校验。
- 冲突与 LWW：同一 item 多设备写入的排序与合并逻辑。

### 集成测试
- 断网恢复：push/pull 断线重连后 cursor 连续性。
- 限额场景：存储/对象/单条/设备超限阻断与恢复。
