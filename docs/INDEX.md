...
- 当前工作区口径：2026-06-05 当前 `master` 与 `origin/master` 仍无 ahead 差异（`origin/master...HEAD` 为 `0 0`），但 worktree 仍很脏：314 个 tracked 改动文件、25 个 untracked 文件；当前剩余 dirty 主要集中在 TuffEx 按需入口/组件拆分、Nexus 页面与文档适配、CoreApp renderer/i18n/UI 迁移、`intelligence-uikit` 消费侧迁移与 TuffEx helper/script 新文件。后续提交按 related-only 分批，先收这些 UI/SDK 大切片，再回 File write/store boundary。
...
- 当前索引 goal：P1-APP-DATA 继续统一 Search Provider / Indexed Source / Indexing Runtime SDK 抽象；已完成 provider SDK、插件 provider 权限 gate、Settings enable/order、source-to-provider link、File progress ETA、Browser Bookmarks consent skeleton 与 Quicklinks linked provider enablement。下一批先完成当前 TuffEx/Nexus/renderer worktree 收口，再按 File write/store boundary、Browser Bookmarks 官方插件 lifecycle、Everything、Quicklinks feed/UI evidence、Browser History/System Settings/Obsidian/VSCode 推进。
