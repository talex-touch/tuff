...
- **CoreBox AI Ask 2.5.0 Stable 切片（2026-05-10）**：`touch-intelligence` 已作为首个最小可见实现切片接入 `text.chat` 与 `vision.ocr`；CoreBox 文本问题直接 Chat，剪贴板图片在空文本或显式 `ai` 提问时先 OCR 再回答，并补齐 pending/ready/error 状态、权限拒绝/provider 不可用/quota/模型不支持/OCR 空结果提示、`caller/entry/featureId/requestId/inputKinds/capabilityId/sessionId/handoffSessionId` 审计 metadata、可恢复 Intelligence Session 交接上下文与复制回答 `clipboard.write` 权限检查。
- **Tuff 2.5.0 AI Plan PRD、Nexus Invoke 与 OmniPanel Writing Tools（2026-05-14）**：AI 桌面入口收口 PRD 已从 Scope Lock 进入 dev/2.5.0 实现；CoreApp 默认 `tuff-nexus-default` provider 通过登录态 `/api/v1/intelligence/invoke` 自动调用 Nexus Intelligence Provider / Provider Registry mirror 与 secure-store credential，未登录时显式 `NEXUS_AUTH_REQUIRED` 并 fallback 到其它可用 provider；Nexus invoke 成功调用且 `usage.totalTokens > 0` 会按 `totalTokens` 扣 AI credits 并写入 `credit_ledger`，credits 不足返回 `402 CREDITS_EXCEEDED`，模型倍率与 dynamic `pricingRef` 留给 Provider Registry Phase 4；CoreApp 设置页与 Intelligence 首页通过 `/api/credits/summary` 展示个人剩余、已用、总额与团队池剩余；OmniPanel Writing Tools MVP 已接入划词 AI 翻译/摘要/改写/解释/Review、结果预览、transient Desktop Context Capsule、copy/retry/replace clipboard 二次确认与 Nexus invoke smoke；`touch-translation` 已默认通过 `text.translate` 走 Intelligence 并透传 traceId，翻译结果朗读已接入 typed `ttsSpeak` / `audio.tts` Beta 与进程内短期缓存；下个版本继续落地 Workflow `Use Model` 节点、完整 Review Queue、剪贴板整理/会议纪要/文本批处理模板与 TTS 队列/播放服务；Stable 只承诺文本 + OCR，Scene runtime 全量编排仍属 2.5.x 后续。
...
| `docs/INDEX.md` | 已同步到 2026-05-14 | 仅维护导航与高价值快照 |
