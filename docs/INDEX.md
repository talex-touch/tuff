...
- Nexus 数据治理方向：已具备 governance event/config foundation、`/api/dashboard/governance/analytics`、Data Governance cockpit、插件私有 analytics、上传/存储/通知/Provider quota 关键指标与 quota risk summary、storage policy enforcement、secure credential reference 与共享 object storage executor；详细历史与文件级变更以 `docs/plan-prd/01-project/CHANGES.md` 为准。当前缺口保留为真实浏览器证据、真实凭据/live send、直接 SMTP socket 或托管 relay、Web Push 生产 VAPID/relay、S3/OSS live storage、生产 D1 migration/backfill 与更完整运营大屏。
- Nexus 上传可靠性诊断：Data Governance upload health 已展示 failed/stuck problem attempts 的脱敏 hash 摘要，包含状态、resource type、surface、storage channel/provider、content type、statusCode、reason、duration/age，不暴露原始 attemptId 或 resourceId。
