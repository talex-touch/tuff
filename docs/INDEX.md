...
- Nexus 数据治理方向：共享 governance event/config foundation、`/api/dashboard/governance/analytics` 聚合 API、search/visit telemetry bridge 与后台 analytics cockpit 已覆盖脱敏用户注册/增长、插件下载/安装/调用、插件 hot leaderboard（hotScore、增长率、action quantity）、搜索 trend/heatmap、filter/provider latency/results/status、result category、geo/timezone、本地搜索 hour/day、上下文 app/source、入口/触发方式、偏好模式、session bucket、搜索涉及插件与插件分类、上传 lifecycle started/completed/failed、attemptId/duration/statusCode 脱敏元数据、stuck attempts/stuckRate（仅统计超过 15 分钟仍无 completed/failed 的 started-only attempt）、duration、surface、storage provider、资源上传失败原因/资源类型/content type/存储渠道/大小统计，且上传 lifecycle 治理写入已有超时保护，不会因治理库卡住阻塞真实上传主链；图片读写删/发布资产/签名读写/插件包 publish/re-edit upload lifecycle、插件图标手动上传/包内提取 upload lifecycle 与读写删 storage telemetry、存储渠道策略评估与操作前 enforcement（maxBytes 约束写入、trafficBytes 约束读取、operation 约束写/读/删）、local/R2/S3/OSS policy catalog 与 Data Governance profile 选择/默认 limits/config、encrypted `secure://storage/*` storage credential 绑定、memory/R2/S3-compatible/OSS 共享 object storage executor、通知渠道策略、插件审核通知投递计划/审计、多 Resend/SMTP/Feishu provider 实例配置语义、`secure://notifications/*` D1 加密凭据绑定、通知投递 planned/sent/skipped/failed 健康指标与 sentRate、per-provider delivery health/latest failure reason、浏览器 push subscription 注册/删除 analytics、Provider quota、provider token/request 消耗与 model/provider type 分布、Provider quota request/token 利用率与 blocked/warning 状态；单插件 analytics API 已返回下载/安装/调用 trend、unique actors、geo/channel/action/version/artifact/package-size breakdown，以及评论状态、已通过平均评分和评分分布，Dashboard assets 插件详情抽屉已对 owner/admin 展示这些私有分析指标与评论质量。Release asset/signature 与 plugin package storage usage 已分离真实 object key 与脱敏 governance resource id，插件包按稳定 `plugin:{id}:version:{channel}:{version}` 聚合，插件图标按稳定 `plugin:{id}:icon` 聚合，避免以原始上传文件名或随机 `.tpex` key 聚合治理事件；通知默认仍是 plan-only，配置 `mode: "send"` 时 browser adapter 会写入按用户隔离的持久 inbox，SMTP adapter 可通过 HTTPS relay 使用 `secure://notifications/*` SMTP 凭据发送，Web Push adapter 可通过 secure relay 使用运行时订阅发送，并提供 `/dashboard/notifications` 用户侧通知中心、浏览器原生通知权限/测试入口、浏览器端 Web Push 注册/注销入口、`/api/dashboard/notifications/inbox` 查询与标记已读 API、`/api/dashboard/notifications/push-subscriptions` 订阅管理 API、`/api/dashboard/notifications/channels/test` 指定渠道 dry-run/按配置 send 测试 API；Data Governance 页面已接入通知渠道测试面板，可直接选择保存的渠道查看 delivery status/reason/provider/adapter。Resend/webhook/Feishu/Lark/SMTP/Web Push relay 仍要求 secure credentials 与运行时收件人或订阅后发送，直接 SMTP socket、Web Push 生产 VAPID 配置与真实跨浏览器可见证据仍未完成；S3/OSS 已接 dedicated object storage executor，但仍缺 live storage evidence 与生产凭据/迁移回填。后续继续补真实浏览器证据、真实凭据/live send 证据、直接 SMTP socket 或托管 relay live evidence、Web Push subscription live evidence、S3/OSS live storage evidence、生产 D1 migration/backfill 和更完整运营大屏。
- Nexus 存储分析与告警：`/api/dashboard/governance/analytics` 已返回 storage channel/provider/resource/action usage 与 daily trend，Data Governance 可查看 stored bytes、traffic bytes、operations、writes/reads/deletes；`/api/dashboard/storage/policies` 已返回 storage alert queue，Data Governance 可按 stored bytes、traffic bytes、operations 展示 blocked/warning 用量、限制、利用率和原因；`/api/dashboard/storage/alerts/notify` 可把当前告警按通知渠道 dry-run 或按配置发送。
