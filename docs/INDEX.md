...
- 2026-03 新增：Pilot 启动性能与 CI 链路收口：`dev` 默认轻量模式、UnoCSS dev safelist 降载、代码块重渲染器异步加载，冷启动从分钟级降到秒级；新增 `pilot-ci.yml`（`lint/typecheck/test/build + static-dist + 1Panel webhook`）支撑 GitHub→1Panel 自动部署（`docs/plan-prd/01-project/CHANGES.md`、`docs/plan-prd/TODO.md`）。
