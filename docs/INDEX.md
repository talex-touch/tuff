...
- Nexus 数据治理方向：已具备 governance event/config foundation、`/api/dashboard/governance/analytics`、Data Governance cockpit、匿名 app visit hotspot/heatmap/trend、匿名 search reliability summary/trend、local hour/weekday/time-slot search preference、telemetry-driven visit hotspot 与 search selection quality buckets、用户 signup growth amount/rate trend、插件私有 analytics/conversion/conversion trend/review status trend、upload status/retry trend、notification delivery trend、notification channel health/risk、multi-provider email adapters、plugin moderation developer notification routing、Dashboard 侧栏未读 badge、存储/Provider usage trend 与 quota risk/remaining budget summary、provider token/model usage breakdown、storage policy summary/risk list 与 remaining/overage budgets、operations dashboard summary、storage policy enforcement、secure credential reference 与共享 object storage executor；详细历史与文件级变更以 `docs/plan-prd/01-project/CHANGES.md` 为准。当前缺口保留为真实浏览器证据、真实凭据/live send、直接 SMTP socket 或托管 relay、Web Push 生产 VAPID/relay、S3/OSS live storage、生产 D1 migration/backfill 与更深运营大屏。
- Nexus 上传可靠性诊断：Data Governance upload health 已展示 failed/stuck problem attempts 的脱敏 hash 摘要，包含状态、resource type、surface、storage channel/provider、content type、statusCode、reason、duration/age、retryable、retry count、max retries 与 next retry delay，不暴露原始 attemptId 或 resourceId；upload analytics 也聚合 retryable / scheduled / exhausted 失败、retry disposition 与 retry trend；外部 S3/OSS 与 R2 object write 已有 bounded retry/backoff，最终失败会把 retry metadata 写入 upload governance。
- Nexus 存储分析与告警：`/api/dashboard/governance/analytics` 已返回 storage channel/provider/resource/action usage、daily trend、policySummary 与 policyRisks，Data Governance 可查看 stored bytes、traffic bytes、operations、writes/reads/deletes、策略 active/blocked/warning/alerts 与峰值利用率，并展示 storage policy remaining/overage budgets；`/api/dashboard/storage/policies` 已返回 storage alert queue，Data Governance 可按 stored bytes、traffic bytes、operations 展示 blocked/warning 用量、限制、利用率、剩余额度、alert overage 和原因；`/api/dashboard/storage/alerts/notify` 可把当前告警按通知渠道 dry-run 或按配置发送。
