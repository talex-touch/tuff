...
> 更新时间：2026-06-18
> 口径：当前工作区事实 + `docs/plan-prd/04-implementation/Roadmap-vNext-2026-06-18.md` + `docs/plan-prd/TODO.md` + `docs/plan-prd/04-implementation/Current-Execution-Plan-2026-06-17.md` + `CHANGES.md`。
...
- 当前稳定基线：`2.4.10`。
- 当前代码版本：root / CoreApp 均为 `2.4.12-beta.8`。
- 本轮实时核对：当前分支 `master`，当前 `HEAD=351c289e2`，本地相对 `origin/master` 为 `0 1`。
...
- 公共包发布不再作为独立 Roadmap / blocker / evidence 项；后续只跟踪版本变更 push 到 GitHub 后的自动发版 workflow 结果。

## 3. 当前主线

`Roadmap vNext` 主线是先清理旧 blocker 口径，再收 release integrity、AI Stable 与 Indexing Runtime：

1. R0：移除公共包发布与平台验证旧 blocker 口径。
2. R1：收口 Nexus release asset `sha256`、`signatureUrl`、signature endpoint 与 manifest/download matrix。
3. R2：补 AI 2.5.0 Stable CoreBox 文本 + 显式 OCR、provider routing 与固定失败路径 evidence。
4. R3：把 File write/store、SQLite/FTS、`scan_progress` 与 integrity reset 收敛到 runtime task/store。
5. R4-R9：QuickOps、Plugin Trust Boundary、UI/TuffEx、Nexus Governance、i18n/Catalog 与 AI 后续能力按小切片推进。

`2.5.0` 口径保持克制：

- Stable 只承诺 CoreBox 文本 + 显式 OCR、provider routing 与固定失败路径。
- OmniPanel Writing Tools 为 MVP/Beta；Workflow / Review Queue / Skills / Automation 保持 Beta。
...
- 当前工作区很脏，约 90 个 dirty/untracked entries；不能声明当前工作区 `quality:release` 已通过。
...
- Everything 仍缺 SDK/CLI 最终策略、registry PATH 探测、Windows 运行性能/evidence 与 fail-closed diagnostics 验收。
- `2.5.0` AI 仍缺 packaged Electron CoreBox 文本/OCR success/failure evidence、Nexus invoke 未登录/provider unavailable/quota/model unsupported/permission denied 可见错误、Local/Ollama routing trace 与 provider metadata chips；OmniPanel/Workflow/Agent tool execution evidence 保持 Beta 队列。

## 6. 下一步推进顺序

1. R0 docs cleanup：入口文档、GOAL 与 roadmap tracking 移除旧公共包发布和平台验证 blocker 口径，保留 `git diff --check` 作为文档验证。
2. R1 Release Integrity：补 Nexus assets `sha256` / `signatureUrl` / signature endpoint / download matrix / manifest 对齐。
3. R2 AI 2.5.0 evidence：只补 CoreBox 文本 + 显式 OCR packaged Electron success/failure、Nexus invoke failure、Local/Ollama routing 与 provider/model metadata chips，不提前承诺 OmniPanel/Workflow/Agent Beta/Experimental 能力。
4. R3 File write/store boundary：优先迁移 SQLite/FTS 真实写入、scan worker、index flush trace、`scan_progress` 与 integrity reset durable history。
5. R4-R7：QuickOps 产品化、Plugin permission sweep、Secret backend、Widget runtime、UI/TuffEx 与 Nexus Governance 按 related-only 小切片推进。
6. Browser Bookmarks / Quicklinks / Everything：Browser Bookmarks 补平台 evidence 与 durable runtime 长尾；Quicklinks 接官方持久 feed 和 clear/rebuild UI；Everything 收 SDK/CLI 策略、registry PATH 与 Windows evidence。
7. Docs sync：每个行为/接口/架构切片至少同步 `README.md`、`TODO.md`、`CHANGES.md`、`docs/INDEX.md` 之一；目标或质量门禁变化同步 Roadmap 与 Quality Baseline。
...
- 不执行 `git commit`、`git push`、`git reset`、tag/release 创建或生产 API/DB 操作，除非用户明确要求。
- 不回退、清理或覆盖无关 dirty/untracked 文件。
- 不把 focused tests 写成生产完成证据。
- 不把 owner 已完成的平台验证重新写成 Roadmap 待办、平台后续或 blocker。
