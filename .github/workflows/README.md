...
- **`ai-review.yml`** - Opt-in AI pull-request review
  - Runs on `pull_request_target` but checks out only the trusted base commit and never installs or executes pull-request head code
  - Uses `contents: read`, `pull-requests: read`, and `issues: write`; repository secrets are exposed only to the trusted `scripts/ci/ai-review.mjs`
  - Accepts checked Chinese or English PR-template requests, skips drafts, missing provider credentials, and author associations outside the configured allowlist
  - Cancels superseded runs per pull request so only the latest event can create or update the bot review comment

- **`release-drafter.yml`** - Release draft automation
  - Updates the release draft on `main` / `master` pushes and merged PRs
  - The final GitHub Release body is generated by `build-and-release.yml`; release-drafter remains a draft signal, not the publish-time source of truth
