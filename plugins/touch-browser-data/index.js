...
function resolveEnabledSourceIds() {
  ...
  if (typeof isProviderEnabled !== 'function')
    return []
...
function fromChromiumTimestamp(value) {
  try {
    ...
    if (timestamp <= CHROMIUM_EPOCH_MICROS)
      return 0
  ...
  for (const definition of definitions) {
    ...
    for (const file of files) {
      ...
      if (result.reason) {
        failedProfileCount += 1
        if (diagnostic)
          diagnostic.lastError = result.reason
      }
      else {
        readableProfileCount += 1
      }
