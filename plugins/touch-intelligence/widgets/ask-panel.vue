...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    function handleRetry() {
      if (status.value !== 'error' || isBusy.value)
        return
      emit('host-action', {
        actionId: 'retry',
        payload: buildHostActionPayload({
    ...
    function handleOpenIntelligenceSettings() {
      if (!canOpenIntelligenceSettings.value || isBusy.value)
        return
      emit('host-action', { actionId: 'open-intelligence-settings' })
    }

    function handleOpenPluginPermissions() {
      if (!canOpenPluginPermissions.value || isBusy.value)
        return
