const assert = require('node:assert/strict')
const childProcess = require('node:child_process')
const fs = require('node:fs')
const Module = require('node:module')
const path = require('node:path')
const process = require('node:process')
const test = require('node:test')

const permissionState = {
  async check() {
    return false
  },
  async request() {
    return false
  },
}

globalThis.logger = {
  warn() {},
}
globalThis.permission = permissionState

let execFileCalls = 0
let spawnCalls = 0
const originalExecFile = childProcess.execFile
const originalSpawn = childProcess.spawn
const originalPlatform = Object.getOwnPropertyDescriptor(process, 'platform')

childProcess.execFile = () => {
  execFileCalls += 1
  throw new Error('execFile must remain blocked')
}
childProcess.spawn = () => {
  spawnCalls += 1
  throw new Error('spawn must remain blocked')
}

function loadPluginModule(filename) {
  const source = fs.readFileSync(filename, 'utf8')
  const mod = new Module(filename)
  mod.filename = filename
  mod.paths = Module._nodeModulePaths(path.dirname(filename))
  mod._compile(source, filename)
  return mod.exports
}

const { onItemAction } = loadPluginModule(path.join(__dirname, 'index.js'))

childProcess.execFile = originalExecFile
childProcess.spawn = originalSpawn

function setRuntimePlatform(platform) {
  Object.defineProperty(process, 'platform', {
    configurable: true,
    enumerable: true,
    value: platform,
  })
}

function createActionItem(platform = process.platform) {
  return {
    meta: {
      defaultAction: 'window-manager',
      actionId: 'launch',
      payload: {
        platform,
        appName: 'Calculator',
      },
    },
  }
}

test.beforeEach(() => {
  setRuntimePlatform('darwin')
  permissionState.check = async () => false
  permissionState.request = async () => false
  execFileCalls = 0
  spawnCalls = 0
})

test.after(() => {
  Object.defineProperty(process, 'platform', originalPlatform)
})

test('blocks forged payload platform before permission and shell execution', async () => {
  let permissionChecks = 0
  permissionState.check = async () => {
    permissionChecks += 1
    return true
  }

  const result = await onItemAction(createActionItem('win32'))

  assert.equal(result.status, 'blocked')
  assert.equal(result.reason, 'platform-mismatch')
  assert.equal(permissionChecks, 0)
  assert.equal(execFileCalls, 0)
  assert.equal(spawnCalls, 0)
})

test('blocks unsupported runtime platform before permission and shell execution', async () => {
  setRuntimePlatform('linux')
  let permissionChecks = 0
  permissionState.check = async () => {
    permissionChecks += 1
    return true
  }

  const result = await onItemAction(createActionItem('linux'))

  assert.equal(result.status, 'blocked')
  assert.equal(result.reason, 'platform:linux')
  assert.equal(permissionChecks, 0)
  assert.equal(execFileCalls, 0)
  assert.equal(spawnCalls, 0)
})

test('blocks before shell execution when permission sdk is unavailable', async () => {
  permissionState.check = undefined
  permissionState.request = undefined

  const result = await onItemAction(createActionItem())

  assert.equal(result.status, 'blocked')
  assert.equal(result.reason, 'permission-sdk-unavailable')
  assert.equal(execFileCalls, 0)
  assert.equal(spawnCalls, 0)
})

test('blocks before shell execution when permission is denied', async () => {
  const result = await onItemAction(createActionItem())

  assert.equal(result.status, 'blocked')
  assert.equal(result.reason, 'permission-denied')
  assert.equal(execFileCalls, 0)
  assert.equal(spawnCalls, 0)
})

test('blocks before shell execution when permission check fails', async () => {
  permissionState.check = async () => {
    throw new Error('permission check failed')
  }

  const result = await onItemAction(createActionItem())

  assert.equal(result.status, 'blocked')
  assert.equal(result.reason, 'permission-request-failed')
  assert.equal(execFileCalls, 0)
  assert.equal(spawnCalls, 0)
})

test('blocks before shell execution when permission request fails', async () => {
  permissionState.request = async () => {
    throw new Error('permission request failed')
  }

  const result = await onItemAction(createActionItem())

  assert.equal(result.status, 'blocked')
  assert.equal(result.reason, 'permission-request-failed')
  assert.equal(execFileCalls, 0)
  assert.equal(spawnCalls, 0)
})
