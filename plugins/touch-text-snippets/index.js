...
const PLUGIN_NAME = 'touch-text-snippets'
...
async function ensureSnippetsFile() {
  try {
    ...
    logger?.warn?.('[touch-text-snippets] Failed to ensure snippets file', error)
  }
}
...
const pluginLifecycle = {
      ...
      if (featureId === 'text-snippets.search') {
        ...
        if (matched.length === 0) {
          items.push(buildInfoItem({
            ...
            featureId,
            title: '暂无匹配片段',
            subtitle: '在 snippets.json 中添加文本片段',
          }))
          ...
          matched.slice(0, 20).forEach((snippet) => {
            ...
            items.push(buildCopyItem({
              ...
              featureId,
              title: snippet.title || '未命名文本片段',
              subtitle: `${snippet.content?.slice(0, 60) || ''}${tags}`,
  ...
  async onItemAction(item) {
    try {
      ...
      logger?.error?.('[touch-text-snippets] Failed to handle action', error)
    }
  },
}
