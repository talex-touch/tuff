...
const aiRequested
  = /(?:☑|\[[xX]\])\s*(?:需要触发 AI 对此 PR 的自动分析|Request AI assistance for PR analysis)/.test(
    prBody,
  )
if (!aiRequested) {
...
const maxTokens = env.AI_REVIEW_MAX_OUTPUT_TOKENS
  ? Number.parseInt(env.AI_REVIEW_MAX_OUTPUT_TOKENS, 10)
  : undefined

const focusMatch = prBody.match(/期望的分析重点：([^\n]+)/)
const requestedFocus = focusMatch
  ? focusMatch[1].replace(/<!--.*?-->/g, '').trim()
  : ''
...
const files = await github.listPaginatedJson(
  (page, perPage) =>
    `/repos/${owner}/${repo}/pulls/${pr.number}/files?per_page=${perPage}&page=${page}`,
)
...
const patchSections = []
let accumulated = 0
const patchBudget = Number.parseInt(
  env.AI_REVIEW_PATCH_CHARACTER_LIMIT ?? '12000',
  10,
)
    ...
    messages: [
      {
        role: 'system',
        content:
          '你是一名严谨的高级软件工程师，擅长代码评审与风险分析。请在提供建议时保持专业、简洁。',
      },
      {
        role: 'user',
        content: prompt,
...
const comments = await github.listPaginatedJson(
  (page, perPage) =>
    `/repos/${owner}/${repo}/issues/${pr.number}/comments?per_page=${perPage}&page=${page}`,
)
const existing = comments.find(
  comment =>
    comment.user?.login === 'github-actions[bot]'
    && comment.body?.includes('### 🤖 AI 评审结果'),
)
