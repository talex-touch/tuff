...
export function trackedFiles(repoRoot) {
  return execFileSync('git', ['ls-files', '-z'], { cwd: repoRoot, encoding: 'buffer' })
    .toString().split('\0').filter(Boolean).map(POSIX).sort()
}
...
function diagnostic(ruleId, file, point, message) { return { ruleId, file, line: point?.line ?? 0, column: point?.column ?? 0, message } }
function resolveRelative(source, url) {
  const raw = url.split(/[?#]/, 1)[0]
  if (!raw || raw.startsWith('#') || raw.startsWith('/') || /^[a-z][a-z0-9+.-]*:/i.test(raw) || raw.startsWith('//')) return null
  let decoded
  try { decoded = decodeURIComponent(raw) } catch { return { error: 'invalid URL encoding' } }
  ...
  for (const file of scope.productDocs) {
    ...
    const visit = node => {
      if (node.type === 'link' || node.type === 'image') {
        ...
        if (result?.error) diagnostics.push(diagnostic('DOC-LINK-INVALID', file, node.position?.start, `${node.url}: ${result.error}`))
        else if (result && !scope.tracked.has(result.target) && !scope.tracked.has(`${result.target}/README.md`) && !scope.tracked.has(`${result.target}/index.md`) && !scope.files.some(tracked => tracked.startsWith(`${result.target}/`))) diagnostics.push(diagnostic('DOC-LINK-UNTRACKED', file, node.position?.start, `${node.url}: ${result.target} is not tracked`))
      }
...
export function checkReleaseNotes(repoRoot) {
  ...
  if (root !== core) return [diagnostic('DOC-RELEASE-VERSION', 'package.json', null, `root ${root} does not match CoreApp ${core}`)]
...
function allowed(file, ruleId) { return ALLOWLIST.get(file)?.has(ruleId) ?? false }

export function checkTasks(repoRoot, scope) {
  const diagnostics = []
  for (const file of scope.activeTasks) {
    let task
    try { task = JSON.parse(fs.readFileSync(path.join(repoRoot, file), 'utf8')) } catch { diagnostics.push(diagnostic('DOC-TASK-JSON', file, null, 'invalid JSON')); continue }
    if (task.status === 'completed') diagnostics.push(diagnostic('DOC-TASK-ACTIVE-COMPLETED', file, null, 'completed task is active'))
    for (const key of ['assignee']) if (!task[key]) diagnostics.push(diagnostic('DOC-TASK-META', file, null, `missing ${key}`))
    for (const key of ['nextAction', 'blocker', 'evidence']) if (!task.meta?.[key]) diagnostics.push(diagnostic('DOC-TASK-META', file, null, `missing meta.${key}`))
  }
...
export function renderDiagnostics(diagnostics, cap = DEFAULT_CAP) {
  if (!diagnostics.length) return 'docs:verify passed\n'
