...
### D 部分(新索引文件进推荐)—— 已完成(2026-09-05)

**Q6 的原始设问是错的。** PRD 假设「需要准入规则筛目录」,但
`indexingRootPolicy.resolveFileSearchRoots()` 只保留 `permissionState === 'granted'` 的根 ——
**用户早就逐个授权过哪些目录重要**。再加一层目录设置就是第二套 path policy。

真正要回答的是「哪种事件配得上一个推荐位」,而这个有干净答案:

- **单闸门就够**:`files.ctime` 存的是文件系统 birthtime(`file-provider.ts:3669`),
  重扫旧目录不会让内容变新,全量索引因此**天然产出零候选**,不需要 watch/scan 透传。
  app 需要第二道闸门只是因为自更新会刷新 bundle birthtime;编辑文件只改 mtime。
- **准入是排除表不是白名单**:白名单的失败模式是静默丢掉用户真正在用的格式,
  排除表的失败模式只是多一条无聊推荐。
- **归属:宿主写死**。用户设置会和索引根冲突;插件声明等于重开 C3 刚关掉的滥用面。

实现:`getRecentlyCreatedFiles`(SQL 下推 + LIMIT)、`file-recommendation-admission.ts`
(29 条测试)、`getNewlyAddedFileItems` 维度(4 个名额)、`newly-added` source 四处同步、
文件提交的缓存失效**按分钟节流**(此前完全不失效正是因为会打穿缓存)。
...
实机验证:昨天的 4 张截图带 `新文件` badge 出现在推荐中,与新装应用按分数交错。
...
- ~~**Q6**~~ 已决(2026-09-05):宿主写死。设问本身是错的 —— 目录范围早已由用户授权的索引根决定,再加一层就是第二套 path policy。见上方 D 部分。
