...
The request’s reservation ledger ID is persisted before dispatch. A successful normalized result is written to the pinned private R2/memory result object, then the exact charge and `settled` state are persisted, then the remainder is released against the original ledger’s team/month. `credits_released_at` is written only after that idempotent release; CAS losers accept only an identical persisted terminal result. Same-key POST and GET recovery run before live Provider/pricing gates and cannot redispatch.
...
When VoiceDock has no configured realtime `audio.asr` binding but has the exact built-in `nexus-audio-transcribe` model through a ready `audio.stt` route, Voice Session uses the final-only buffer. The 10 MiB cap covers five minutes of 16 kHz mono PCM16; the post-capture request gets 150 seconds inside a 180-second IPC envelope, while realtime retains its 30-second connection budget and fresh retry IDs. Buffered recovery reuses its UUID. Capture-start user ID and Nexus origin are frozen; a change before upload/delivery rejects and purges the audio, while same-user token refresh remains valid.
