...
`qwen-audio-3.0-asr-flash` is not the WebSocket realtime model. The Nexus DashScope adapter sends the captured WAV as a `data:audio/wav;base64,...` value to `POST /api/v1/services/aigc/multimodal-generation/generation` with `X-DashScope-SSE: disable`. The provider model and endpoint stay server-owned; the CoreApp request contains only WAV bytes and its normal app-authenticated route.

The adapter accepts only the exact configured Qwen Audio Flash model, normalizes `output.text` (with the documented nested sentence fallback), and takes positive `usage.duration` when present, otherwise the server-parsed WAV duration. Provider response details and credentials are discarded after normalization. Provider rejection is pre-acceptance; a transport outcome that is uncertain after dispatch is terminally failed rather than retried.

The existing request row is still created and credits are reserved before the provider call. A successful synchronous result is written to a private result object under the request-owned namespace, then settled from `reserved` and returned with the transcript. The result object is owner-bound and expires with the request handoff TTL; it is readable only through the authenticated same-request status path and never appears in D1 or ledger projections. This gives same-key retries a durable, no-second-dispatch recovery path without persisting plaintext transcript in the database.

When a local VoiceDock has no configured realtime `audio.asr` binding but has a ready `audio.stt` Nexus route, the main Voice Session uses a bounded buffered adapter: it emits capture readiness/levels, sends the complete WAV at stop, and emits one final event followed by end. It does not emit synthetic partials or silently replace an explicitly configured but broken realtime binding.
