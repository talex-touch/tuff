...
- The existing request/credit boundary remains authoritative. CoreApp retains one UUID idempotency key across buffered recovery. Nexus requires a durable private R2 result before persisting the final charge and `settled` state, then releases the unused hold against the original reservation ledger’s team and month. Interrupted settlement/release resumes from same-key replay without another Provider call; settled release still runs when the transcript has expired.
- The shared Voice Session exposes this route as final-only buffered input only when no realtime `audio.asr` binding exists. It must not fabricate partial events or claim realtime latency for this non-realtime model. Its 10 MiB PCM bound covers the advertised five-minute 16 kHz mono PCM16 capture; its separate 150-second post-capture deadline covers Nexus’s 120-second Provider deadline plus controlled response/settlement overhead. Expired private result objects are deleted in bounded, durably advancing cleanup pages.
- A new Qwen request accepts only canonical 16 kHz mono PCM16, applies an atomic per-user submission limit before a chunk-count/byte-bounded body read, and checks the Base64 ceiling before storage or credit reservation. The synchronous route consumes the request body directly and never writes a redundant source-audio handoff object.
- Buffered capture freezes the signed-in user and Nexus origin before recording. A user/origin change before upload or final delivery aborts without invoking STT, persisting source audio, offering recovery, or writing late text; same-user token refresh remains valid. Retry-buffer mutations are capture-owner fenced, and concurrent recovery calls share one in-flight Provider/delivery operation.
- Expiry maintenance reconciles reserved result evidence and every settled-but-unreleased charge before deleting private objects. Successful credit release and result deletion each have durable markers; legacy in-flight rows resolve and persist their original reservation ledger before settlement, while old already-settled rows are migration-marked as complete. Pre-acceptance released tombstones expire after the idempotency window.
