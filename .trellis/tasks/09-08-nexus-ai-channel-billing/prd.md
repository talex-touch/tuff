...
- The existing request/credit boundary remains authoritative. CoreApp retains one UUID idempotency key across buffered recovery. Nexus stores a normalized private result before persisting the final charge and `settled` state, then releases the unused hold idempotently against the original reservation ledger’s team and month. Interrupted settlement/release resumes from same-key replay without another Provider call; settled release still runs when the transcript has expired.
...
- Buffered capture freezes the signed-in user and Nexus origin before recording. A user/origin change before upload or final delivery aborts without invoking STT, persisting source audio, offering recovery, or writing late text; same-user token refresh remains valid.
