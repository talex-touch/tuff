...
- [ ] AC1 `ssh deploy@10.0.3.14 -p 2222` 被识别为 SSH 端点；详情区出现 用户 / 主机 / 端口 三行，各自点击只复制该字段。
- [ ] AC2 `ssh-ed25519 AAAAC3Nza… user@laptop` 被识别为 SSH 公钥；密钥类型和注释分行显示；**不掩码**。
- [ ] AC3 主机 IP 默认掩码，点「显示」展开；复制得到的是完整 IP 而非掩码文本。
- [ ] AC4 含 IP 的普通记录 `retentionProtected` 为 `false`，按类别策略正常过期；含 SSH 私钥的记录仍为 `true`。**必须有负控制**：把 IP 接回 secret 保留期通道会让这条红。
- [ ] AC5 裸 `8080` / `123456` 不产生端口；`localhost:8080` 与 `-p 8080` 产生端口。
- [ ] AC6 R6 的歧义规则有测试覆盖，正反两向各至少一例。
- [ ] AC7 `files` 记录里的 `~/.ssh/id_ed25519` 被标为 SSH 相关。
- [ ] AC8 关掉 IP 掩码设置后，IP 明文显示，且识别与拆行不受影响。
- [ ] AC9 utils / core-app clipboard / 插件三套测试与 typecheck、lint 全绿。
- [ ] AC10 新增的每条判定规则都做过注入验证（拆掉规则本身会让对应用例变红）。
