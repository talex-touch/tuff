...
- [ ] 完成 `07-27-isolate-plugin-prelude-297`：utilityProcess 默认启用；typed capability RPC；callbacks/cancel/timeout/crash/restart/generation/resource limits；移除生产 main-process fallback。
- [ ] 运行全部官方插件构建、加载与关键能力回归。

## Wave 4 — 隐私生命周期与总收口

- [ ] 与用户逐项确认 #301 的数据类别默认保留期、卸载/撤权策略、备份恢复和远程处理提示。
- [ ] 实现 clipboard、OCR/search cache、plugin data、secrets、logs/telemetry 的 usage/export/clear/delete controls 与合成 canary 测试。
- [ ] 更新 #302 checklist，执行全量相关 lint/typecheck/test/build 和最终 GitHub open issue 查询。
