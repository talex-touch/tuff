...
```ts
preempts: {
  // hint 必须抢占 hint —— 这正是 PRD R2 的原始诉求（指针从 A 移到 B，A 立即关）。
  // 初稿这里误写成 []，会让整个任务的核心场景失效。
  hint:   ['hint'],
  menu:   ['hint', 'menu'],      // 打开菜单会清掉提示，也会关掉兄弟菜单
  dialog: ['hint', 'menu'],      // dialog 之间的叠放归 DialogManager 管，不在此处
}
suppress: {
...
#### 实施结果：上面这条判据**没有达标**（2026-08-15）

```
base-anchor-motion.ts   1088 行 → 1205 行   (+117)
dist/es/…-motion.js    26.4 KiB → 28.6 KiB  (+2.2)
```

原因是我只做了**解析器**的查表化（`phaseTable` / `resolveCloseType` / `resolveExitGeometry`），
**没有**动那 421 行的 `animateOpen` / `animateClose` 镜像 —— 而那 39% 才是真正的重复。
解析器原本只是约 50 行三元表达式，换成表 + 三个带文档的辅助函数后净增长，属预期之外但方向明确的误判：
我把「解析器去重」和「动画器去重」当成了同一件事。

`audit:size` 仍**在预算内**（绿），所以不阻塞。但判据是我自己立的，记在这里而不是悄悄放过。

**未做动画器合并的理由**（不是遗忘，是权衡）：那 421 行是刚刚按参考视频逐帧调出来的动效，
且当时树上还压着 796 行他人在途改动；jsdom 无布局，单测无法捕捉动效回归，
合并出的细微时序偏差没有任何自动化手段能兜住。风险与本任务的目标不成比例。

**后续**：动画器合并应作为**独立任务**，前置条件是视觉回归手段（录制比对或 CDP 逐帧采样），
而不是塞进本任务顺手做。
