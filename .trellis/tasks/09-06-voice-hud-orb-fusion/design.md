...
## 20. 空态背景：三版 ASCII 全废，改用 canvas 画波（2026-09-08）

ASCII 那条路走死了，值得记下**为什么**，因为三版的错法各不相同：

| 版本 | 做法 | 读起来像 |
| --- | --- | --- |
| v1 | 9 行 `/\|<>~+=*`，65% 墨 | 乱码（字符角度太多，眼睛安顿不下来）；而且每行为循环复制一份，密度一高重复就被抓到 |
| v2 | 短簇 + 长间隔，20% 墨，遮罩 52%→94% | **什么都看不见**（遮罩到 94% 才不透明，只剩最外圈） |
| v3 | 30% 墨，透明度 0.5 | 灰尘、划痕、印刷瑕疵 |

三次我都在调**密度和透明度**，而真正的分界线是别的：

> **连续的笔画读作「流」，断开的点簇读作「脏」。**

v2/v3 为了「安静」把连续性拆成了簇，那一步就注定后面怎么调都是屑——再调透明度只是在决定「屑有多明显」。

### 20.1 canvas 的实现约束

四条正弦叠加，每条不同频率/振幅/相位/漂移方向，`lineCap: round`，用元素自身的 `color`（`--shell-primary`）描边——所以它跟着主题走，而不是写死一个颜色。

- **包络**：`1 - exp(-((x-0.5)·2.6)²)`，中间压平。文字站在静止的空气里，而不是压在波上——和之前那个径向遮罩是同一个意图，但做在几何里而不是叠一层遮罩。
- **DPR**：backing store 用**元素自己的 `getBoundingClientRect()` × dpr**，不是窗口尺寸——canvas 在一张有内边距的卡片里，它的宽度和视口无关。配 `ResizeObserver`。
- **30fps**：这是背景纹理，不是给人盯着看的动画。
- **reduced-motion**：只画一帧。形状是要点，漂移才是可以拒绝的那部分。
- **卸载**：`onBeforeUnmount` 里 `stopWave()`。watcher 只在 canvas 随分支消失时触发，**整页离开不走那条路**，而一个悬着的 rAF 会活得比拥有它的组件久。这条有负控制（删掉立刻转红）。

### 20.2 一条不能声称的覆盖

`drawWave` 里有 `if (!context) return`（jsdom 的 `getContext('2d')` 返回 null）。但**测试跑完之前一帧都没执行**，所以那条分支从未被走到——它是防御性代码，没有守卫。用例名字已经改成只说它真正验证的事（卸载时取消动画帧），不冒领这份覆盖。
