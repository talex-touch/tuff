...
空出来的地方放 ASCII 音流：9 行字符横向漂移，径向遮罩把中间挖空，所以字在静止的空气里、周围在流动。它必须**保持是装饰**：这屏本来就没有数据，任何长得像波形的东西都是无中生有画了一条。所以 `aria-hidden`，而且**用固定序列生成而不是 `Math.random`**——每次挂载都一样，图案里不编码「你什么时候打开的这一页」。有测试钉着：换成随机立刻转红。
