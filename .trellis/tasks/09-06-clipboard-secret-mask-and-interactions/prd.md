...
- [ ] AC1 一条 OpenAI key（`sk-` + 48 位）记录下，列表标题、`title` 属性、预览区、更多信息四处均无完整值；单测断言渲染后的 DOM 文本不包含原始 key。
- [ ] AC2 打开预览区显示开关后预览区与洞察区同时出现完整值；切到另一条记录再切回来，开关已复位为隐藏。
- [ ] AC3 私钥记录没有显示开关，且 DOM 里不含 `-----BEGIN` 之后的任何正文片段。
- [ ] AC4 密钥记录的「更多信息」摘要不含"字符拆分"，展开后也没有字符网格。
- [ ] AC5 在非输入区域按下鼠标横跨整个窗口拖动，不产生跨区域选区；预览区内部仍可正常选中文本。
- [ ] AC6 Cmd/Ctrl + ←/→ 在分类条上循环切换，并触发对应的历史查询；焦点在输入框时不触发。
- [ ] AC7 图片记录在默认窗口尺寸下预览区无滚动条，主题色带竖排显示在图片一侧。
- [ ] AC8 链接记录按 Cmd/Ctrl+Enter 调用 `AppEvents.system.openExternal`（单测断言 transport 收到该事件与 URL），不再走 `window.open`。
- [ ] AC9 文件记录按 Cmd/Ctrl+Enter 调用 `AppEvents.system.showInFolder` 并带上第一个文件路径；文件行的眼睛按钮走同一条路径且带该行自己的路径。
- [ ] AC10 图片记录按 Cmd/Ctrl+Enter 打开大图浮层，Esc 关闭；浮层打开期间上下键不再改选中项。
- [ ] AC11 文本/密钥记录按 Cmd/Ctrl+Enter 仍是复制，footer 按钮文案与实际动作一致。
- [ ] AC12 `pnpm --filter @talex-touch/clipboard-history-plugin test` 与 `typecheck` 全绿；`packages/utils` 的 typecheck 不因新增 SDK 方法变红。
