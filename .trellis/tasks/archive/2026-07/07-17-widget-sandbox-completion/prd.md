...
- [x] Direct clipboard, host history/location, host `postMessage`, worker, and DOM navigation attempts do not reach the host capability.
- [x] Widget-local history and messaging retain useful in-widget semantics without crossing widget boundaries.
- [x] Sensitive calls fail closed after quota exhaustion and produce bounded audit evidence without recording payloads.
- [x] Dynamic `eval`, `Function`, dynamic import, constructor-chain escape markers, and WebAssembly compilation are rejected before widget evaluation.
- [x] Sandbox evidence names every injected global, browser policy, quota policy, audit policy, and same-realm limitation.
- [x] Main calculation and PreviewSDK advanced expressions accept only allowlisted arithmetic/functions/constants and execute without generic mathjs evaluation.
- [x] Focused widget and expression verification covers allowed paths, denied paths, isolation, quota, audit retention, and cleanup.
