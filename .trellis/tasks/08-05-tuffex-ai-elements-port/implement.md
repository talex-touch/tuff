...
- [ ] types.ts：AiSourceItem/AiSourcesPart/AiSuggestion/AiChainStep；AiMessagePart 联合扩员
- [ ] sources/ 组件 + 单测；TxAiMessage 增 sources 分派分支 + 分派测试（既有 ai-elements 测试全绿）
...
- [ ] tool-confirmation/：三险级、remember、approve/deny + 单测
- [ ] suggestion-chips/：滚动 + select + 单测
- 验证：vitest 过滤两目录

### S3 TxContextIndicator + TxChainOfThought
- [ ] context-indicator/：环 + 阈值色 + formatter + 单测
- [ ] chain-of-thought/：步骤时间线 + 流式 active + 折叠 + 单测
- 验证：vitest 过滤两目录

### S4 注册与全量
- [ ] components.ts ×5（字母序）；各 index withInstall
