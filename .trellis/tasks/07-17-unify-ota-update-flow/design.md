...
- macOS 在 Apple native trust 仍为 `waived` 时继续使用自有 `.app` 替换脚本，但必须保留风险标记和 detached signature 门禁。
...
macOS native trust 处于 `waived` 时，诊断和更新详情必须保留该风险，不显示 Developer ID/Gatekeeper pass。
