...
1. [ ] Run symbol/reference and package-manifest inventory for `tuffcli`, legacy unplugin CLI bin, duplicate exporter/security/compress utilities and all `/vite` consumers.
2. [ ] Compare canonical and legacy exporter behavior; migrate only still-required behavior into `tuff-cli-core` with shared contract tests.
3. [ ] Route `tuff build`/`tuff builder` exclusively through CLI core and keep `unplugin-export-plugin/vite` as the Vite integration boundary.
4. [ ] Migrate official plugin scripts, package dependencies, documentation and tests to `tuff` and canonical builder.
5. [ ] Remove duplicate exporter/security/compress code and obsolete exports from unplugin package.
6. [ ] Remove unplugin CLI bin/forwarder, `tuffcli` bin, default command fallback and compatibility environment labels.
7. [ ] Update `package.json`, tsup configs, package files lists and `pnpm-lock.yaml`; preserve acyclic dependency direction.
8. [ ] Build and pack CLI/unplugin packages; inspect tarballs for one `tuff` bin, expected `/vite` exports and no legacy implementation.
9. [ ] Clean-install packed packages into a fixture plugin and run validate, builder, dry-run publish and Vite dev/build smoke.
10. [ ] Run final repository reference audit; no aliases/re-exports remain.
