...
- macOS 默认语音键由 Command 切换为 Fn：单独短按切换、长按说话；Fn 与其他按键组合不触发语音。启用语音手势时消费单键 Fn 的系统默认表情/Globe 动作，退出或停用后恢复，不改系统全局偏好。
- 录音句柄未就绪时松开必须在句柄到达后 finalize，而不是 cancel；旧会话回调不得结束新会话；错误提示不得被紧随的 stream end 抹掉。
- 增加截图所示语音洞察：口述字数、估算节省时间、平均速度、总时长、活跃/连续天数、年度热力图、分享与报告。
- 洞察从本功能启用后按真实成功会话记账，不补造历史；只保存本地聚合数字，不保存录音、文字或目标应用。节省时间明确使用估算基线。个人学习仍不在本轮范围，报告不得伪造个性化分数。
