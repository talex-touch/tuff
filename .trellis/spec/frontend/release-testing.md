...
- Release workflow shell steps must extract `rollbackCompatible` with value-preserving `jq -r`, not truthiness-validating `jq -e`: `false` is the valid default and must not terminate a `set -e` sync job.
