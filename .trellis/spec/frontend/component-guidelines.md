...
## TuffEx Suite Taxonomy (concepts / base / pro / ai / data / flow)
...
- `apps/nexus/scripts/recategorize-component-docs.py` — `TAXONOMY` is the single source of truth for `category` frontmatter (23 categories; suites: concepts = Foundations, base = BaseSuite/Basic/Form/Layout/Navigation/Data/Feedback/Status, pro = ProSuite/Advanced/Effects/Primitives, ai = AiSuite/AiChat/AiAgent/AiReasoning/AiContext, data = DataSuite/Charts/Visualization, flow = FlowSuite/Flow). Rerun with `--apply`; it errors on docs missing from the table.
