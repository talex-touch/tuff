...
6. `pnpm -C packages/tuffex build` before any downstream typecheck (exports resolve to `dist/`), then `pnpm -C apps/nexus typecheck` (wrapper, not `:raw`) and `pnpm -C apps/nexus exec vitest run` — CI blocks on the latter, and the gate scripts do not cover what it checks.
