...
node node_modules/vitest/vitest.mjs run test/docs   # content guards; parses every docs page (~10 s)
```
...
- **`check:icon-collections` checks the collection, not the name.** `i-carbon-clipboard` passes it and renders nothing — carbon has no such icon. Check names against the installed set:
  ...
  ```
- **A self-closing component tag used to swallow everything after it; the content pipeline now closes it first** (2026-09-26). `<TuffDocSourceLink />` on its own line is raw HTML, and rehype-raw re-parses it with parse5, which ignores `/>` on anything but a void element. The tag stayed open, every later block in its parent became the link's child, and the link has no slot: 80 component pages lost their trailing notes and 24 whole sections (使用场景 / 相关组件 / 自定义); prism-glow moved its link to the end of the file to dodge it. `app/utils/remark-close-component-tags.ts`, registered in `content.build.markdown.remarkPlugins`, rewrites every self-closing PascalCase tag in an `html` node to an explicit pair, so `<Name />` is safe anywhere in the prose. Fenced code, inline code and `code: |` props never become `html` nodes and stay as written. Lowercase tags are not touched: `<i class="…" />` or `<my-el />` still opens an element. `test/docs/self-closing-component-tags.test.ts` parses every docs page through the configured plugins and fails when a heading ends up nested or a source link holds content.
  - It moves where an element ends, never which element a tag becomes. MDC's `html` handler kebab-cases only the tag a raw HTML node opens with, and only the first occurrence of its name, so that tag is written as a kebab-case pair; `<Name></Name>` there would come out as `<name-kebab></Name>` and stay open. Later tags keep their spelling, since parse5 lowercases both halves alike. Kebab-casing all of them was tried first and 500ed `progress-bar`: its upload demo's `:::TuffDemoWrapper` line does not parse (escaped quotes in `description`), so its `code: |` block is raw HTML, and its `<TxProgressBar :format="p => …" />` lines turned from inert `<txprogressbar>` elements into live components that threw `props.format is not a function`.
  - It is a remark plugin, not a `content:file:beforeParse` hook. @nuxt/content 3.15 caches every parse in `.data/content/contents.sqlite` under a hash of `content.build.markdown` plus the file, and hashes a function by its own source text only. A hook never changes that key, so it never reaches a page that is already cached — on a dev server that has run before, every page. The entry passes `options.revision`, a hash of the plugin file, so an edit anywhere in it re-parses all pages on the next start (the dev log shows `(0 cached, 613 parsed)`). `remark-mermaid` has no such fingerprint: after changing its helpers, `pnpm -C apps/nexus content:reset` and restart.
...
- **The docs dev server lags on `.mdc` edits.** A page can serve the previous parse for a minute or more; every probe returns `false` with no error page, which reads exactly like "my section never rendered". Re-poll before concluding anything. `/api/docs/page` sits behind a nitro stale-while-revalidate cache (`maxAge` 300 s, `staleMaxAge` 3600 s): the first request after expiry answers with the old body and refreshes it in the background, which is how 6 of 80 fixed pages still read as broken on 2026-09-26. For a deterministic read, delete `.nuxt/cache/nitro/functions/docs-page/<path>/<locale>/body.json` first.
...
- **Frame-driven SVG geometry is verified from sampled frames, not screenshots** (2026-09-26, `TxFusionSurface`). A screenshot takes ~1.7 s and `Animation.setPlaybackRate` cannot slow rAF, so a 600 ms sprout-and-pinch is over before the first capture. In one `page.evaluate`, start a rAF loop that records the element's `path.getAttribute('d')` (plus any inline `transform` / `opacity` / `filter` it writes), trigger the interaction, stop after the motion settles, return the samples. Then write each distinct `d` into a tiny standalone SVG under `/tmp`, rasterise with `magick -density 120 f.svg f.png`, and `magick montage` them into one sheet to read. Take real screenshots only for the settled states (material, stroke, shadow, both themes). The same sheet format is what the geometry tests' authors used, so shapes can be compared frame for frame.
