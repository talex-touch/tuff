...
- `auth.token` has one persistent path: CoreApp's local-secret secure store. Historical payloads may be a plain non-refreshable access token; new payloads are exact version-1 JSON bundles containing the access token, refresh token, and access-token expiry. The renderer never reads, writes, diagnoses, or configures this path; `auth.requiresReauthenticationOnNextStartup` is a main-owned, non-sensitive fail-closed marker rather than a user setting or protection override.

### Contracts

- The deployed backend is `local-secret`: a locally generated random secret in `config/local-secret.v1.key` derives AES-256-GCM protection for the `auth.token` credential envelope in `secure-store.json`.
- This is not OS Keychain, Credential Locker, libsecret, Electron `safeStorage`, a hardware key, or an OS root key. The local secret and access/refresh credentials are never portable or ordinary-exportable.
...
- Before each protected auth-credential write, `AuthModule` durably persists `auth.requiresReauthenticationOnNextStartup: true`. It writes no new credential bundle if this marker cannot be persisted; only a successful encrypted write followed by durable marker clearing permits future restoration. A marker-write, secure-store creation/write, envelope validation/decryption, bundle validation, read, health-check, or marker-clear failure leaves the current authenticated process memory-only and never uses an ordinary settings/JSON/log/sync fallback.
- At cold start, a true marker prevents any credential read or restore. Main best-effort deletes `auth.token` from the protected store and clears the marker only after that deletion succeeds; a failed cleanup preserves the marker. Logout, rejected refresh credentials, and auth-unauthorized cleanup clear the in-memory access/refresh pair, persist the marker before protected deletion, and clear it only after that deletion succeeds. Main diagnostics use stable state/reason projections and do not include token values, local-secret material, paths, or native errors.
- Nexus access tokens are fixed at 24 hours. Device-bound refresh tokens are valid for 30 days under short authorization and 180 days under long authorization. Refresh tokens cannot authenticate business APIs; CoreApp refreshes single-flight before access expiry and once after a `401`, retries the original request once, and keeps legacy plain access-token payloads non-refreshable.

### Tests Required

- Auth-module tests cover removal of every legacy override field, healthy protected cold-start restore for both a historical plain access token and the exact v1 access/refresh bundle, malformed bundle rejection, marker persistence before credential writes, a failed write plus incomplete protected cleanup followed by a blocked restart, marker-write/marker-clear failures, single-flight refresh, one-time `401` retry, rejected-refresh cleanup, and logout deletion with marker retention on deletion failure.
- `docs/engineering/sensitive-data-inventory.json` records the exact local-secret AES-256-GCM backend, non-portability, v1 access/refresh bundle, and memory-only failure lifecycle; `corepack pnpm privacy:inventory:verify` must pass.
