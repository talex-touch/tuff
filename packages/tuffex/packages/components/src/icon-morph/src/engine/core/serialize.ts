...
export function serialize(
  ...
  for (let k = 0; k < subs.length; k++) {
    ...
    if (!o) continue
    const n = o.length / 2
    d += `M${fmt(o[0] ?? 0)} ${fmt(o[1] ?? 0)}`
    for (let i = 1; i < n; i++) d += `L${fmt(o[2 * i] ?? 0)} ${fmt(o[2 * i + 1] ?? 0)}`
    if (closed?.[k]) d += 'Z'
  }
  return d
}
...
export function cubicsToPathD(paths: readonly CubicPath[]): string {
  ...
  for (const { pts, closed } of paths) {
    d += `M${fmtCanon(pts[0] ?? 0)} ${fmtCanon(pts[1] ?? 0)}`
    for (let i = 2; i < pts.length; i += 6) {
      d += `C${fmtCanon(pts[i] ?? 0)} ${fmtCanon(pts[i + 1] ?? 0)} ${fmtCanon(
        pts[i + 2] ?? 0,
      )} ${fmtCanon(pts[i + 3] ?? 0)} ${fmtCanon(pts[i + 4] ?? 0)} ${fmtCanon(pts[i + 5] ?? 0)}`
    }
    if (closed) d += 'Z'
  }
  return d
