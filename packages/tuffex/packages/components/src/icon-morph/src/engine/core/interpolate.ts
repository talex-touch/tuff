...
export function interpPolar(plan: MorphPlan, t: number, out: Float64Array[]): void {
  for (let k = 0; k < plan.items.length; k++) {
    ...
    let cy: number
    if (it.block) {
      const [ox = 0, oy = 0] = it.block.off
      const [dx = 0, dy = 0] = it.block.drift
    ...
    for (let i = 0; i < n; i++) {
      ...
      const bTx = it.bT[2 * i] ?? 0
      const bTy = it.bT[2 * i + 1] ?? 0
      const px = aCx + (bTx - aCx) * t
      const py = aCy + (bTy - aCy) * t
...
export function interpLinear(plan: MorphPlan, t: number, out: Float64Array[]): void {
  for (let k = 0; k < plan.items.length; k++) {
    ...
    for (let i = 0; i < n; i++) {
      const aX = it.a[2 * i] ?? 0
      const aY = it.a[2 * i + 1] ?? 0
      const bOX = it.bO[2 * i] ?? 0
      const bOY = it.bO[2 * i + 1] ?? 0
      o[2 * i] = aX + (bOX - aX) * t
      o[2 * i + 1] = aY + (bOY - aY) * t
    }
  }
