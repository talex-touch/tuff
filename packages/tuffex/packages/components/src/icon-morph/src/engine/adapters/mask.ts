/* Output format adapter: a CSS-mask element → the driver's write contract
   ...
   (flipping buffers per write — see the function doc for why). */
 ...
 *  so a real HTMLElement's `.style` satisfies it. */
 ...
 *  the back buffer and re-points `mask-image` at it. */
export function maskTarget(el: MaskEl, opts: MaskTargetOptions = {}): MaskPathEl {
  ...
  const layers: [ReturnType<typeof makeLayer>, ReturnType<typeof makeLayer>] = [makeLayer(), makeLayer()]
  let front = 0
  const s = el.style
  ...
  return {
    setAttribute(name, d): void {
      if (name !== 'd') return
      front = 1 - front
      const layer = layers[front] ?? layers[0]
