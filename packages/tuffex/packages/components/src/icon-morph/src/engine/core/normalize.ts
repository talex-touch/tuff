...
function builder(x0: number, y0: number): BuilderOps {
  ...
  const finish = (closed: boolean): CubicPath | null => {
    if (closed) line(pts[0] ?? 0, pts[1] ?? 0) // explicit closing segment if needed
...
function polyPath(nums: number[], closed: boolean): CubicPath | null {
  ...
  const [, line, , , finish] = builder(nums[0] ?? 0, nums[1] ?? 0)
  for (let i = 2; i + 1 < nums.length; i += 2) line(nums[i] ?? 0, nums[i + 1] ?? 0)
