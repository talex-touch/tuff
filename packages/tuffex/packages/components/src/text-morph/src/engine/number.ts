// Ported from torph/src/lib/text-morph/utils/number.ts
...
// tuffex lint style, the `Morph*` type prefix, and strict-TS hardening.

import type { MorphSegment, MorphSegmentKind } from './types'
import { lcsIndices } from './lcs'

export type NumberSegment = MorphSegment & { kind: MorphSegmentKind }
...
export function numericSkeleton(word: string): string {
  ...
  for (const char of word) {
    if (!isDigit(char) && !CORE_SEPARATORS.includes(char))
      out += char
  }
  return out
}
...
export function isNumericWord(word: string): boolean {
  ...
  if (start >= end)
    return false
  if (!isDigit(word[start]!) || !isDigit(word[end - 1]!))
    return false
...
export function classifyKind(char: string): MorphSegmentKind {
...
export function decimalSeparator(locale: string): string {
  ...
  let separator = '.'
  try {
    separator = new Intl.NumberFormat(locale)
      .formatToParts(1.1)
...
export function segmentNumber(
  ...
  for (const [, oldIdx] of matches) usedIds.add(prevSegments[oldIdx]!.id)
...
function cursorMatch(
  ...
  const pair = (ni: number, oi: number) => matches.set(newKept[ni]!, oldKept[oi]!)

  let keptCursor = 0
  while (keptCursor < newKept.length && newKept[keptCursor]! < cursor) keptCursor++

  const lenDiff = newKept.length - oldKept.length

  if (lenDiff > 0) {
    const editStart = keptCursor - lenDiff
    for (let i = 0; i < editStart && i < oldKept.length; i++) pair(i, i)
    ...
    for (let i = 0; i < keptCursor && i < newKept.length; i++) pair(i, i)
    for (let i = keptCursor; i < newKept.length; i++) {
      const oldIdx = i - lenDiff
      if (oldIdx >= 0 && oldIdx < oldKept.length)
        pair(i, oldIdx)
  ...
  for (let k = 1; k <= oldSeps.length && k <= newSeps.length; k++) {
    ...
    if (oldChars[oldIdx] === newChars[newIdx])
      matches.set(newIdx, oldIdx)
  }
...
function keptIndices(chars: string[], decimalChar: string): number[] {
  ...
  for (let i = 0; i < chars.length; i++) {
    if (!isGrouping(chars[i]!, decimalChar))
      indices.push(i)
  }
  return indices
}
...
function findPivot(chars: string[], start: number, end: number, decimalChar: string): number {
  for (let i = end - 1; i >= start; i--) {
    if (chars[i] === decimalChar)
      return i
  }
  return end
}
...
function placeMatch(
  ...
  if (oldDigits > 0 && newDigits > 0 && Math.abs(oldDigits - newDigits) >= MAGNITUDE_JUMP)
    return matches
