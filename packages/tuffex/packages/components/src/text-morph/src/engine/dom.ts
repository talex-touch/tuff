// Ported from torph/src/lib/utils/dom.ts (https://github.com/lochie/torph).
// MIT License © lochie. Kept intentionally close to upstream so its fixes stay
// diffable; deviations are limited to tuffex lint style, the `Morph*` prefix,
// and routing `getAnimations()` through a guard so a jsdom test env degrades.
...
import { getAnimationsOf, layoutSize, parseTranslate } from './metrics'
...
export function detachFromFlow(container: HTMLElement, elements: HTMLElement[]) {
  ...
  for (const child of elements) {
    ...
    getAnimationsOf(child).forEach(a => a.cancel())
    snapshots.set(child, { left: x + tx, top: y + ty, width, height, opacity })
  }
...
export function splitWordSpans(element: HTMLElement, splits: Map<string, MorphSegment[]>) {
  if (splits.size === 0)
    return
 ...
 */
function syncSlot(element: HTMLElement, segment: MorphSegment) {
  if (!segment.kind) {
