// Ported from torph/src/lib/text-morph/utils/diff.ts
...
export interface DiffResult {
  ...
  splits: Map<string, MorphSegment[]>
}
...
function splitIfWhole(
  oldGroup: { word: string, segments: MorphSegment[] },
  splits: Map<string, MorphSegment[]>,
): MorphSegment[] {
...
function asNumberSegments(segments: MorphSegment[]): NumberSegment[] {
  return segments.map(seg => ({ ...seg, kind: seg.kind ?? classifyKind(seg.string) }))
}
...
export function diffSegments(
  ...
  const digitsInvolved = numbersOn && (hasDigit(newText) || oldWords.some(g => hasDigit(g.word)))
  ...
  const [oldLcsIdx, newLcsIdx] = lcsIndices(oldWordStrings.map(token), newWordStrings.map(token))
  const oldMatchedSet = new Set(oldLcsIdx)
  const newMatchedSet = new Set(newLcsIdx)

  const newToOldWord = new Map<number, number>()
  for (let k = 0; k < newLcsIdx.length; k++) newToOldWord.set(newLcsIdx[k]!, oldLcsIdx[k]!)

  let oldUnmatched = oldWordStrings.map((_, i) => i).filter(i => !oldMatchedSet.has(i))
  let newUnmatched = newWordStrings.map((_, i) => i).filter(i => !newMatchedSet.has(i))
  ...
  for (const plan of plans) {
    ...
    if (plan.mode !== 'reuse' && oldGroup.segments.length === 1) {
      ...
      const wordSeg = oldGroup.segments[0]!
      for (let i = 0; i < oldGroup.word.length; i++) alloc.reserve(`${wordSeg.id}:${i}`)
    }
    else {
  ...
  const splits = new Map<string, MorphSegment[]>()
  let charOffset = 0
  ...
  for (let ni = 0; ni < newWordStrings.length; ni++) {
    ...
    if (plan.mode === 'reuse') {
      ...
      const newCharToOldSeg = new Map<number, MorphSegment>()
      ...
      for (let ci = 0; ci < newChars.length; ci++) {
        if (newCharToOldSeg.has(ci)) {
          ...
          segments.push({ id: alloc.take(`${newWord}~${ci}`), string: newChars[ci]! })
        }
      }
    }
