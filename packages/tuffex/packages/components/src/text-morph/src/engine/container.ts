// Ported from torph/src/lib/utils/animate.ts (https://github.com/lochie/torph).
...
export function slopeAt(easing: MorphEasingFn, t: number): number {
...
): { curve: MorphEasingFn, k: number } {
  // `Math.max(0, NaN)` is NaN and `NaN <= 0` is false, so the guard has to be positive.
  const k = Math.max(0, Math.min(CARRY_MAX, normalisedVelocity) - slopeAt(base, 0))
  if (!(k > 0))
    return { curve: base, k: 0 }

  const bump = Math.max(3, Math.ceil(k / (Math.E * CARRY_OVERSHOOT)) - 1)
  return {
    curve: t => (t >= 1 ? 1 : base(t) + k * t * (1 - t) ** bump),
    k,
  }
}

/** Sampled fine enough that a segment is shorter than a frame at 120Hz. */
export function sampleEasing(curve: MorphEasingFn, duration: number): string {
...
function animateAxis(
  ...
  const run = (
    ...
    const anim = animateElement(
      element,
      [{ [property]: `${axisFrom}px` }, { [property]: `${axisTo}px` }],
      { duration, easing, fill: 'both' },
    )
    if (anim && seek !== undefined)
      anim.currentTime = seek
    return { anim, from: axisFrom, to: axisTo, easing, curve, startedAt: now() - (seek ?? 0) }
  }
   ...
   */
  if (previous && previous.elapsed !== null && Math.abs(previous.to - to) < SAME_TARGET)
...
function axisState(axis: Axis, duration: number): AxisState {
  ...
  const gone = !axis.anim || axis.anim.currentTime === null
  ...
  return {
    ...
    curve: axis.curve,
    elapsed,
    velocity: elapsed === null || !axis.curve
      ? 0
      : ((axis.to - axis.from) * slopeAt(axis.curve, elapsed / duration)) / duration,
  }
}
...
export function transitionContainerSize(
  ...
  const width = animateAxis(element, 'width', oldWidth, newWidth, previous?.width, duration, ease, base)
  const height = animateAxis(element, 'height', oldHeight, newHeight, previous?.height, duration, ease, base)
