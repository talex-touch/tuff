...
export interface ChartTooltipProps {
  ...
  /**
   * Extra line rendered below the row list (kumo `TimeseriesChart.tooltipFooter`).
   * The `+N more` row stays its own line above it.
   */
  footer?: string
  /**
   * Clamping boundary, mirroring ECharts' `confine`:
   * - `'clipping-ancestors'` clamps inside the intersection of the viewport and
   *   every ancestor whose computed overflow is not `visible`.
   * - an element (or list of elements) clamps inside those boxes.
   * @default 'clipping-ancestors'
   */
  boundary?: 'clipping-ancestors' | Element | Element[]
}
