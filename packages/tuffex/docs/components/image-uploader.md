...
An image picker with inline previews and remove controls. ImageUploader manages the selection lifecycle — picking files, generating previews, and emitting structured file objects.
...
Bind `v-model` to a file array. Users click the add tile to open the native file picker.
...
- Object URLs created by the component are revoked when an item is removed or when the component unmounts.
- `disabled` disables adding and removing images.
