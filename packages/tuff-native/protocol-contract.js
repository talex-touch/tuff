...
const IDENTIFIER_PATTERN = /^[A-Z0-9][\w.:-]{0,127}$/i
...
function validateAttachmentDescriptors(value) {
  if (!Array.isArray(value))
    fail('ATTACHMENT_MISMATCH', 'Invalid attachment descriptors')
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (ids.has(id))
      fail('ATTACHMENT_MISMATCH', 'Attachment ids must be unique')
    ids.add(id)
    ...
    if (descriptor.mediaType !== undefined)
      requireString(descriptor.mediaType, 'attachment media type', 128)
    if (descriptor.purpose !== undefined)
      requireIdentifier(descriptor.purpose, 'attachment purpose')
  }
  return value
}
...
function validateError(value) {
  ...
  if (!ERROR_CATEGORIES.has(error.category))
    fail('INVALID_ENVELOPE', 'Invalid error category')
...
function validateMeta(value) {
  ...
  if (meta.engine !== undefined)
    requireIdentifier(meta.engine, 'engine')
  if (meta.degraded !== undefined)
    requireBoolean(meta.degraded, 'degraded state')
...
function validateCapability(value) {
  ...
  if (capability.engine !== undefined)
    requireIdentifier(capability.engine, 'capability engine')
  if (!CAPABILITY_STATES.has(capability.state))
    fail('INVALID_ENVELOPE', 'Invalid capability state')
  if (capability.reason !== undefined)
    requireIdentifier(capability.reason, 'capability reason')
  if (!Array.isArray(capability.features))
    fail('INVALID_ENVELOPE', 'Invalid capability features')
  ...
  if (!Array.isArray(capability.operations))
    fail('INVALID_ENVELOPE', 'Invalid capability operations')
  ...
  for (const rawOperation of capability.operations) {
    ...
    if (operations.has(name))
      fail('INVALID_ENVELOPE', 'Operation names must be unique')
    operations.add(name)
    if (!OPERATION_MODES.has(operation.mode))
      fail('INVALID_ENVELOPE', 'Invalid operation mode')
    if (!CANCELLATION_MODES.has(operation.cancellation))
      fail('INVALID_ENVELOPE', 'Invalid cancellation mode')
...
function validateClientHello(control) {
  ...
  if (!Array.isArray(control.requestedFeatures))
    fail('INVALID_ENVELOPE', 'Invalid requested features')
...
function validateServerHello(control) {
  ...
  if (!Array.isArray(control.carrierFeatures))
    fail('INVALID_ENVELOPE', 'Invalid carrier features')
  ...
  if (!Array.isArray(control.capabilities))
    fail('INVALID_ENVELOPE', 'Invalid capabilities')
...
function validateRequest(control) {
  ...
  if (!Object.hasOwn(control, 'payload'))
    fail('INVALID_ENVELOPE', 'Missing request payload')
  ...
  if (control.payload && isRecord(control.payload) && Object.hasOwn(control.payload, 'initialWindow')) {
    ...
    if (Object.hasOwn(control.payload, 'streamId'))
      requireIdentifier(control.payload.streamId, 'stream id')
  }
}
...
function validateResponse(control) {
  ...
  if (control.ok) {
    if (!Object.hasOwn(control, 'payload'))
      fail('INVALID_ENVELOPE', 'Missing response payload')
    validateJsonValue(control.payload, 'response payload')
    return
  }
  if (control.attachments.length !== 0)
    fail('ATTACHMENT_MISMATCH', 'Error responses cannot carry attachments')
...
function validateStreamFrame(control) {
  ...
  if (control.kind === 'stream_data') {
    if (!Object.hasOwn(control, 'payload'))
      fail('INVALID_ENVELOPE', 'Missing stream payload')
    validateJsonValue(control.payload, 'stream payload')
    if (control.meta !== undefined)
      validateJsonValue(control.meta, 'stream metadata')
    return
  }
  if (control.kind === 'stream_end') {
    if (control.payload !== undefined)
      validateJsonValue(control.payload, 'stream terminal payload')
    return
  }
  if (control.attachments.length !== 0)
    fail('ATTACHMENT_MISMATCH', 'Stream errors cannot carry attachments')
  validateError(control.error)
}
...
function validateCancel(control) {
  ...
  if (!TARGET_TYPES.has(target.type))
    fail('INVALID_ENVELOPE', 'Invalid cancel target')
  requireIdentifier(target.id, 'cancel target id')
  if (!CANCEL_REASONS.has(control.reason))
    fail('INVALID_ENVELOPE', 'Invalid cancel reason')
}
...
function decodeControl(encoded) {
  if (typeof encoded !== 'string')
    fail('MALFORMED_CONTROL', 'Native control must be a string')
  if (Buffer.byteLength(encoded, 'utf8') > MAX_CONTROL_BYTES) {
    fail('MALFORMED_CONTROL', 'Native control exceeds the byte limit')
  }
  let control
...
function validatePacket(encoded, attachments) {
  ...
  if (!Array.isArray(attachments))
    fail('ATTACHMENT_MISMATCH', 'Native attachments must be an array')
