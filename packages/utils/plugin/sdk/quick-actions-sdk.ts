...
export type QuickActionNativeShareTarget =
  | NativeShareTarget
  | "system-share"
  | (string & {});
...
export interface QuickActionsSDK {
   ...
   */
  getNativeShareTargets: (
    payloadType?: FlowPayloadType,
  ) => Promise<FlowTargetInfo[]>;
...
function resolveItemTitle(item: TuffItem, fallback = "Tuff item"): string {
  const title =
    item.render?.basic?.title ?? item.meta?.web?.title ?? item.id ?? fallback;
...
function resolveItemSubtitle(item: TuffItem): string | undefined {
  const subtitle =
    item.render?.basic?.subtitle ?? item.render?.basic?.description;
...
function defaultNativeShareTargetOrder(
  payloadType?: FlowPayloadType,
): QuickActionNativeShareTarget[] {
...
function targetMatchesPreference(
  ...
  return (
    ...
    target.fullId === preferredId ||
    target.fullId === `native.${preferredId}`
  );
}
...
export function createQuickActionsSDK(
      ...
      try {
        ...
        if (result && typeof (result as Promise<void>).catch === "function") {
          void (result as Promise<void>).catch((error) => {
            console.error(
              "[QuickActionsSDK] onActionExecute handler error",
              error,
            );
          });
        }
  ...
  const dispose = () => {
    ...
    if (registeredActionIds.size > 0) {
      void transport
        .send(MetaOverlayEvents.action.unregister, { pluginId })
        .catch((error) => {
          console.error(
            "[QuickActionsSDK] Failed to unregister all actions during dispose",
            error,
          );
        });
    }
  ...
  const createSharePayloadFromItem = (
    ...
    const text =
      options.text ?? [title, subtitle, url].filter(Boolean).join("\n");
    return {
      type: "text",
      data: text,
      context: {
  ...
  const resolveNativeShareTarget = async (
    ...
    const targets = (
      await api.getNativeShareTargets(options.payloadType)
    ).filter((target) => target.isEnabled !== false);
    ...
    for (const preference of preferredTargets) {
      const matched = targets.find((target) =>
        targetMatchesPreference(target, preference),
      );
      if (matched) {
        return matched;
      }
  ...
  const api: QuickActionsSDK = {
    registerAction(action: TuffQuickAction): () => void {
      ...
      void transport
        .send(MetaOverlayEvents.action.register, {
          pluginId,
          action: {
            ...action,
            handler: pluginId,
      ...
      return () => {
        ...
        void transport
          .send(MetaOverlayEvents.action.unregister, {
            pluginId,
            actionId: action.id,
          })
          .catch((error) => {
            console.error(
              "[QuickActionsSDK] Failed to unregister action",
              error,
            );
          });
      };
    ...
    unregisterAll(): void {
      ...
      void transport
        ...
        .catch((error) => {
          console.error(
            "[QuickActionsSDK] Failed to unregister all actions",
            error,
          );
        });
    },
    ...
    async getNativeShareTargets(
      ...
      if (!response?.success) {
        throw new Error(
          response?.error?.message || "Failed to get native share targets",
        );
      }

      return (response.data || []).filter(
        (target) => target.isNativeShare === true,
      );
    },
