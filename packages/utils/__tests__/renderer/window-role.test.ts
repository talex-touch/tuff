...
describe("window-role", () => {
  it("parses known core type from argv", () => {
    const role = parseWindowArgs([
      "--touch-type=core-box",
      "--core-type=omni-panel",
    ]);
    expect(role).toEqual({
      touchType: "core-box",
      coreType: "omni-panel",
  ...
  it("round-trips the region selector role to its dedicated renderer mode", () => {
    const role = {
      touchType: "assistant" as const,
      assistantType: "region-selector" as const,
    };

    expect(parseWindowArgs(buildWindowArgs(role))).toMatchObject(role);
    expect(resolveRendererWindowMode(role)).toBe("AssistantRegionSelector");
  });
