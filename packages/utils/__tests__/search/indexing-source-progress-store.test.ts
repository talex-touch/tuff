...
function createStore(
  ...
  const loadCompletedPaths = vi.fn(
    async () => new Set(options.completedPaths ?? []),
  );
  const deleteCompletedPaths = vi.fn(async () => undefined);
  const ensureReadyForUpsert = vi.fn(
    options.ensureReadyForUpsert ?? (async () => true),
  );
  const upsertCompletedPaths = vi.fn(
    options.upsertCompletedPaths ?? (async () => undefined),
  );
...
describe("IndexedSourceProgressStoreService", () => {
  ...
  it("marks all roots pending when the completed path store is unavailable", async () => {
    ...
    await expect(
      service.summarizeRoots(["/a", "/b"], new Set(), {
  ...
  it("deduplicates watched roots before summarizing pending state", async () => {
    ...
    await expect(
      service.summarizeRoots(["/a", "/a", "/b", "/b"]),
    ).resolves.toEqual({
      totalRoots: 1,
      pendingRoots: 1,
    });
  ...
  it("skips upsert when no paths are provided", async () => {
    const { ensureReadyForUpsert, service, upsertCompletedPaths } =
      createStore();
