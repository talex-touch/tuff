...
import { IndexedWriteDeleteExecutorService } from "../../search";

describe("indexing-write-delete-executor-service", () => {
  it("normalizes paths, deletes existing records, removes indexed artifacts, and logs count", async () => {
    const existing = [
      { id: 1, path: "/tmp/a.txt" },
      { id: 2, path: "/tmp/b.txt" },
    ];
    ...
    expect(removeIndexedArtifacts).toHaveBeenCalledWith([
      "/tmp/a.txt",
      "/tmp/b.txt",
    ]);
    expect(logDebug).toHaveBeenCalledWith("test delete completed", {
      removed: 2,
    });
  ...
  it("skips delete and artifact removal when no existing records match", async () => {
    const deleteRecords = vi.fn();
    const removeIndexedArtifacts = vi.fn();
    const service = new IndexedWriteDeleteExecutorService({
    ...
    expect(removeIndexedArtifacts).not.toHaveBeenCalled();
  });
  ...
  it("can delete already resolved records without looking them up again", async () => {
    ...
    expect(deleteRecords).toHaveBeenCalledWith(existing);
    expect(removeIndexedArtifacts).toHaveBeenCalledWith(["/tmp/c.txt"]);
  });
});
