...
describe('indexing watch root policy', () => {
  ...
  it('ignores roots rejected by the watch path normalizer', () => {
    const rootSet = resolveIndexedWatchRootSet({
      ...
      normalizePath: (rawPath) => (rawPath.trim() === '/ignored' ? '' : rawPath.trim().toLowerCase())
    })

    expect(rootSet.paths).toEqual(['/Users/demo/Documents', '/Users/demo/Desktop'])
    expect(rootSet.normalizedPaths).toEqual(['/users/demo/documents', '/users/demo/desktop'])
  })
  ...
  it('does not treat empty normalized paths or roots as owned', () => {
    expect(
      isIndexedWatchPathOwned({
        ...
        normalizePath: (rawPath) => (rawPath === '/ignored' ? '' : rawPath.toLowerCase()),
        pathSeparator: '/'
      })
    ).toBe(false)
  })
