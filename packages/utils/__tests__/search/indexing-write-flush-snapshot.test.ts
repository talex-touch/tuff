...
describe("IndexedWriteFlushSnapshotService", () => {
  ...
  it("isolates returned flush snapshots from cache mutation", () => {
    ...
    const counters = firstSnapshot!.metadata!.counters as {
      withContent: number;
    };
  ...
  it("extracts flush results from error-like objects only", () => {
    ...
    expect(getIndexedWriteFlushResultFromError({ flushResult })).toBe(
      flushResult,
    );
    expect(
      getIndexedWriteFlushResultFromError({ flushResult: null }),
    ).toBeNull();
