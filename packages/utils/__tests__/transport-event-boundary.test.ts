...
function collectRawEventDefinitions(
  relativePath: string,
  source: string,
): RawEventDefinition[] {
  const rawEventPattern =
    /defineRawEvent(?:<[^]*?>)?\(\s*(?:['"`]([^'"`$]+)['"`])?/g;
...
function collectRawSendViolations(
  relativePath: string,
  source: string,
): RawSendViolation[] {
  const rawSendPattern =
    /\b(?:send|sendSync|invoke)\(\s*['"`]([^'"`$]+:[^'"`$]+)['"`]/g;
...
describe("transport event boundary", () => {
  ...
  it("keeps download migration push events in the typed event registry", () => {
    expect(DownloadEvents.migration.progress.toEventName()).toBe(
      "download:migration:progress",
    );
    expect(DownloadEvents.migration.result.toEventName()).toBe(
