...
function cloneIndexingSnapshotFallback(
  value: unknown,
  ...
  if (ArrayBuffer.isView(value)) {
    ...
    const buffer = cloneIndexingSnapshotFallback(
      view.buffer,
      seen,
    ) as ArrayBuffer;
  ...
  for (const [key, item] of Object.entries(value as Record<string, unknown>)) {
    (clone as Record<string, unknown>)[key] = cloneIndexingSnapshotFallback(
      item,
      seen,
    );
  }
...
function cloneIndexingSnapshotError(
  error: Error,
  seen: WeakMap<object, unknown>,
): Error {
  ...
  if ("cause" in error) {
    (clone as Error & { cause?: unknown }).cause =
      cloneIndexingSnapshotFallback(error.cause, seen);
  }
