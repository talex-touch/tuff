...
export function resolveIndexedSourceRecoveryRecommendation(
  ...
  const permissionRecommendation =
    resolvePermissionRecoveryRecommendation(source);
  ...
  const latestTaskRecommendation =
    resolveLatestTaskRecoveryRecommendation(source);
...
function resolvePermissionRecoveryRecommendation(
  ...
  if (
    ...
    return {
      ...
      priority: "high",
      reason:
        source.health.reason ?? `permission:${source.health.permissionState}`,
    };
  }
...
function resolveRunGateRecoveryRecommendation(
  ...
  if (
    entry.lastBlockedReason === "already-running" ||
    entry.lastBlockedReason === "debounced"
  ) {
    return {
      action: "wait",
      priority: "low",
...
function resolveLatestTaskRecoveryRecommendation(
  ...
  if (
    skippedReason?.includes("permission") ||
    skippedReason?.includes("root-permission")
  ) {
    return {
      action: "grant-permission",
      priority: "high",
...
function isRunningProgressStatus(status: IndexedSourceProgressStatus): boolean {
  return (
    status === "running" || status === "estimated" || status === "stabilizing"
  );
}
...
function isWaitSkippedReason(reason: string | null): reason is string {
  return (
    reason?.startsWith("retry-window:") === true ||
    isDebounceSkippedReason(reason)
  );
}
...
function getLatestTask(
  ...
  return recentTasks
    ...
    .reduce<
      IndexedSourceTaskHistoryEntry | undefined
    >((latest, task) => (!latest || task.completedAt > latest.completedAt ? task : latest), undefined);
}
...
function getLatestRunGateEntry(
  ...
  return entries
    ...
    .reduce<
      NonNullable<IndexedSourceDiagnostics["taskRunGate"]>[number] | undefined
    >((latest, entry) => (!latest || (entry.lastBlockedAt ?? 0) > (latest.lastBlockedAt ?? 0) ? entry : latest), undefined);
}
