...
export function resolveIndexedSourceMaintenanceActions(
  ...
  return actions.map((action) =>
    resolveIndexedSourceMaintenanceAction(source, action),
  );
}
...
function resolveResetMaintenanceAction(
  ...
  if (
    canReset &&
    source.lifecycleIssues?.includes("reset-capability-missing-handler")
  ) {
    return {
      action: "reset",
      enabled: false,
  ...
  if (
    canClear &&
    source.lifecycleIssues?.includes("clear-capability-missing-handler")
  ) {
    return {
      action: "reset",
      enabled: false,
