...
export type IndexedWriteFlushSnapshotStatus =
  | "idle"
  | "flushed"
  | "not-ready"
  | "failed";
...
export type IndexedWriteFlushResultSnapshotInput<
  TStatus extends string = string,
> = Omit<IndexedWriteFlushSnapshotBase, "checkedAt" | "status"> & {
  status: TStatus;
};

export interface IndexedWriteFlushIdleSnapshotInput<
...
export type IndexedWriteFlushIdleSnapshotDraft<
  TReason extends string = string,
> = Omit<
...
function cloneFlushSnapshot<TSnapshot extends IndexedWriteFlushSnapshotBase>(
  ...
  return {
    ...
    metadata: snapshot.metadata
      ? cloneFlushSnapshotMetadata(snapshot.metadata)
      : undefined,
  };
}

function cloneFlushSnapshotMetadata(
  metadata: Record<string, unknown>,
...
export function buildIndexedWriteFlushResultSnapshot<
  TSnapshot extends IndexedWriteFlushSnapshotBase =
    IndexedWriteFlushSnapshotBase,
>(
  result: IndexedWriteFlushResultSnapshotInput<TSnapshot["status"]>,
): Omit<TSnapshot, "checkedAt"> {
  return {
    ...
    metadata: result.metadata
      ? cloneFlushSnapshotMetadata(result.metadata)
      : undefined,
    durationMs: normalizeOptionalSnapshotNumber(result.durationMs),
  } as Omit<TSnapshot, "checkedAt">;
}

export function buildIndexedWriteFlushIdleSnapshot<
  TReason extends string = string,
>(
...
export interface IndexedWriteFlushFailureSnapshotInput<
  TResult extends Partial<IndexedWriteFlushSnapshotBase> =
    Partial<IndexedWriteFlushSnapshotBase>,
> {
  error: unknown;
...
export interface IndexedWriteFlushFailureRetryMetadataInput<
  TReason extends string = string,
> {
  delayMs: number;
  retryReason: TReason;
...
export function buildIndexedWriteFlushFailureRetryMetadata<
  TReason extends string = string,
>(
...
>(
  input: IndexedWriteFlushFailureSnapshotInput<TResult>,
): Omit<TSnapshot, "checkedAt"> {
  const flushResult =
    input.flushResult ??
    getIndexedWriteFlushResultFromError<TResult>(input.error);
  const stringifyError =
    input.stringifyError ?? defaultIndexedWriteFlushErrorStringifier;

  return {
    status: "failed",
    entries: normalizeSnapshotCount(flushResult?.entries),
    pending: normalizeSnapshotCount(flushResult?.pending ?? input.pendingSize),
    inflight: normalizeSnapshotCount(
      flushResult?.inflight ?? input.inflightSize,
    ),
