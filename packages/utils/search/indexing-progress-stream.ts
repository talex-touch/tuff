...
>({
  ...
  const normalizedNow = Math.max(
    normalizedLastEmitAt,
    normalizeTimestamp(now, normalizedLastEmitAt),
  );
  ...
  if (
    Math.abs(normalizedNext.current - normalizedPrevious.current) >=
    normalizedConfig.currentStep
  ) {
    return true;
  }
...
function normalizePayload<TPayload extends IndexingProgressStreamPayload>(
  payload: TPayload,
): TPayload {
  return {
    ...payload,
