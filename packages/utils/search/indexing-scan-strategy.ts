...
export function resolveIndexedScanStrategy(
  input: IndexedScanStrategyInput,
): IndexedScanStrategy {
  const normalizePath = input.normalizePath ?? ((path: string) => path);
  const completedPathKeys = new Set(
    Array.from(input.completedScanPaths, (completedPath) =>
      normalizePath(completedPath),
    ).filter((completedPath) => completedPath.trim().length > 0),
  );
  const isCompleted = (watchPath: string): boolean =>
    completedPathKeys.has(normalizePath(watchPath));
  const watchPaths = uniqueIndexedScanStrategyPaths(
    input.watchPaths,
    normalizePath,
  );
