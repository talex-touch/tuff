...
export interface IndexingProgressEstimatorOptions<
...
export class IndexingProgressEstimatorService<TStage extends string = string> {
  ...
  constructor(options: IndexingProgressEstimatorOptions<TStage> = {}) {
    ...
    this.minProgressRatioForEta = normalizeNonNegativeNumber(
      options.minProgressRatioForEta,
      DEFAULT_MIN_PROGRESS_RATIO_FOR_ETA,
    );
    ...
    this.maxEtaMs = normalizeNonNegativeNumber(
      options.maxEtaMs,
      DEFAULT_MAX_ETA_MS,
    );
    this.minElapsedMsForFallbackEta = normalizeNonNegativeNumber(
      options.minElapsedMsForFallbackEta,
      DEFAULT_MIN_ELAPSED_MS_FOR_FALLBACK_ETA,
    );
    this.fallbackEtaSafetyMultiplier = normalizeNonNegativeNumber(
  ...
  update(rawInput: IndexingProgressSample<TStage>): IndexingProgressEstimate {
    ...
    const noProgressMs =
      this.lastProgressAt > 0 ? input.now - this.lastProgressAt : 0;

    if (input.total > 0 && remainingItems === 0) {
      this.lastEstimate = {
        estimatedRemainingMs: 0,
        averageItemsPerSecond:
          this.smoothedItemsPerSecond || this.getElapsedItemsPerSecond(input),
        status: "estimated",
        speedSampleCount: this.speedSampleCount,
        estimateBasis:
          this.smoothedItemsPerSecond > 0 ? "stage-speed" : "elapsed-progress",
      };
      return this.lastEstimate;
    }
    ...
    const basis: IndexingProgressEstimateBasis = canUseStageSpeed
      ? "stage-speed"
      : "elapsed-progress";
    const safetyMultiplier = canUseStageSpeed
      ? 1
  ...
  private getElapsedItemsPerSecond(
    input: IndexingProgressSample<TStage>,
  ): number {
    const elapsedMs = input.now - this.stageStartedAt;
