...
export function resolveIndexedScanEligibility(
  ...
  const watchPathKeys = new Set(
    watchPaths.map((watchPath) => normalizePath(watchPath)),
  );
  ...
  const newPaths = watchPaths.filter(
    (watchPath) => !completedMap.has(normalizePath(watchPath)),
  );
  const stalePaths =
    input.intervalMs <= 0
      ? watchPaths.filter((watchPath) =>
          completedMap.has(normalizePath(watchPath)),
        )
      : watchPaths.filter((watchPath) => {
