...
export function resolveIndexedSourceTaskRetryDecision(
  ...
  const baseDelayMs = normalizeDelay(options.baseDelayMs, DEFAULT_BASE_DELAY_MS)
  const maxDelayMs = normalizeDelay(options.maxDelayMs, DEFAULT_MAX_DELAY_MS)
  const failureWindowMs = normalizeDelay(options.failureWindowMs, DEFAULT_FAILURE_WINDOW_MS)
  const failures = collectRecentFailures(recentTasks, kind, now, failureWindowMs)
  ...
  const retryDelayMs = Math.min(maxDelayMs, baseDelayMs * 2 ** Math.max(0, failedAttempts - 1))
