...
export class IndexedSourceTaskRunGate {
  ...
  canStart(
    sourceId: string,
    kind: IndexedSourceTaskHistoryKind,
  ): IndexedSourceTaskRunGateDecision {
  ...
  complete(
    ...
    const existingLastCompletedAt = normalizeFiniteTimestamp(
  ...
  isRunning(sourceId: string, kind: IndexedSourceTaskHistoryKind): boolean {
    return (
      this.entries.get(this.getKey(sourceId, kind))?.runningSince !== undefined
    );
  }
  ...
  getSnapshot(generatedAt = this.now()): IndexedSourceTaskRunGateSnapshot {
    ...
    return {
      ...
      runningEntries: entries.filter(
        (entry) => entry.runningSince !== undefined,
      ).length,
  ...
  private recordBlocked(
    ...
    updates: Pick<
      IndexedSourceTaskRunGateEntry,
      "runningSince" | "lastCompletedAt" | "nextAllowedAt"
    >,
  ): void {
    const key = this.getKey(sourceId, kind);
    this.entries.set(key, {
