...
export type IndexedWriteReconciliationUpsertRecord =
...
export interface IndexedWriteReconciliationDiffResult<
  TDisk extends IndexedWriteReconciliationDiskPayload =
    IndexedWriteReconciliationDiskPayload,
> {
  filesToAdd: TDisk[];
...
> {
  ...
  constructor(
    options: IndexedWritePlanServiceOptions<TIncoming, TExisting, TUpdate>,
  ) {
    ...
    this.toUpdateRecord =
      ...
      ((incoming, existing) =>
        this.defaultUpdateRecord(incoming, existing) as TUpdate);
  }
  ...
  plan(input: {
    ...
    const existingMap = new Map(
      input.existingRows.map((row) => [row.path, row]),
    );
    ...
    const manualAccepted = input.records.reduce((count, record) => {
      return (
        count + (input.manualPaths.has(this.normalizePath(record.path)) ? 1 : 0)
      );
    }, 0);
...
export function buildIndexedWriteManualContext<
  TEntry extends readonly [unknown, IndexedWriteManualEntryPayload],
>(
  entries: TEntry[],
...
export function toIndexedWriteDate(
...
export function toIndexedWriteTimestamp(
  value: Date | number | string | null | undefined,
): number | null {
  if (value == null) {
    return null;
...
>(
  ...
  if (reconciliationPaths.length > 0) {
    for (const [filePath, dbFile] of dbMap.entries()) {
      if (
        matchesIndexedWriteReconciliationPath(reconciliationPaths, filePath)
      ) {
...
export function mapIndexedWriteWorkerFilePayload(
  ...
  const fallbackTimestamp = toIndexedWriteTimestamp(
...
function matchesIndexedWriteReconciliationPath(
  paths: string[],
  targetPath: string,
): boolean {
