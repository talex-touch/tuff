...
export interface IndexedSourceGroupedEvidenceResult<
...
export interface IndexedSourceGroupedEvidenceInput<
  TKey extends string = string,
> {
  sourceId: string;
  keys: readonly TKey[];
...
export class IndexedSourceGroupedEvidenceService {
  build<TKey extends string>(
    input: IndexedSourceGroupedEvidenceInput<TKey>,
  ): IndexedSourceEvidence[] {
    const checkedAt = normalizeEvidenceTimestamp(input.checkedAt);
    const bySourceId = new Map<
      TKey,
      IndexedSourceGroupedEvidenceResult<TKey>
    >();
    ...
    return input.keys.map((key) => {
      ...
      const itemCount = normalizeEvidenceCount(
        override?.itemCount ?? result?.itemCount,
      );
      ...
      return {
        ...
        status:
          override?.status ?? this.resolveStatus(result?.error, itemCount),
        itemCount,
        lastCheckedAt: checkedAt,
        reason:
          override?.reason ??
          result?.error ??
          (itemCount === 0 ? input.emptyReason?.(key) : undefined),
        metadata: {
          ...(input.metadata ?? {}),
