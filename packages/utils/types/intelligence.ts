...
export type WorkflowReviewQueueItemStatus = 'pending' | 'copied' | 'clipboard_replaced' | 'dismissed' | 'failed'
...
export interface IntelligenceStreamOptions<T = unknown> {
  ...
  onPartEvent?: (partEvent: IntelligencePartEvent, event: IntelligenceStreamEvent<T>) => void
...
/**
 * Authoritative server receipt for Nexus-owned batch transcription.
 * Clients display these values as returned; they must not calculate prices locally.
 */
export interface IntelligenceSTTBilling {
  requestId: string
...
export interface IntelligenceSTTResult {
  /** Transcribed text. */
  text: string
  /** Confidence score (0-1), when reported by the provider. */
  confidence?: number
  /** Authoritative server billing receipt, when supplied by the provider. */
...
/** Server-owned product alias for the built-in Nexus batch WAV transcription route. */
export const NEXUS_AUDIO_TRANSCRIBE_MODEL = 'nexus-audio-transcribe' as const
...
export type ContextContinuationReason =
  | 'archived-session-continuation'
  | 'expired-session-continuation'
  | 'idle-session-continuation'
  | 'continuation-session-missing'

export type ContextContinuationStatus = 'included' | 'excluded' | 'unavailable'

/** Metadata-only description of an explicit continuation across a session boundary. */
export interface ContextContinuationSummary {
...
export interface CompressionSnapshotMetadata {
  privacyLevel?: ContextPrivacyLevel
  factState?: 'confirmed' | 'inferred' | 'user-rejected'
  confidence?: number
  memoryIds?: string[]
  checkpointId?: string
}
...
export interface CompressionSnapshotDraft {
  goal?: string
  currentState?: string
  decisions?: string[]
  constraints?: string[]
...
export interface CreateCompressionSnapshotInput {
  sessionId: string
  expectedSessionUpdatedAt: number
  snapshot: CompressionSnapshotDraft
}

export type CompressionSnapshotDegradedReason = 'cas-conflict'

export type CreateCompressionSnapshotResult =
  | {
      status: 'created'
  ...
  | {
      status: 'degraded'
      degradedReason: CompressionSnapshotDegradedReason
      sessionUpdatedAt: number
    }

export interface ListCompressionSnapshotsInput {
  sessionId: string
  limit?: number
}

export interface ListCompressionSnapshotsResult {
  snapshots: CompressionSnapshot[]
}
...
export interface MemoryItem {
  ...
  sourceSessionId?: string
  sourceTurnId?: string
  /** Previous memory atomically superseded by this item, when applicable. */
  replacesMemoryId?: string
  privacyLevel: ContextPrivacyLevel
  /** Positive lifetime in milliseconds from the latest saved `updatedAt`. */
  ttl?: number
  enabled: boolean
  createdAt: number
  updatedAt: number
...
export type MemoryListStatus = 'all' | 'enabled' | 'disabled'
...
export interface EvaluateMemoryResult {
  ...
  /** Host-issued fingerprint binding the normalized evaluated payload. */
  fingerprint?: string
}
...
export interface PrepareContextTurnInput {
  ...
  /** Force a fresh session boundary even when the input is not an explicit reset phrase. */
...
export interface PrepareContextTurnResult {
  ...
  continuation?: ContextContinuationSummary
}

export type IntelligenceContextMode = 'new' | 'continue' | 'stateless'

export interface IntelligenceContextExecutionIntent {
  mode: IntelligenceContextMode
  /** Session ownership boundary used by the host context store. */
  owner?: ContextSession['owner']
  sessionId?: string
...
/** Host-owned request that never accepts caller-supplied history as trusted context. */
export interface IntelligenceContextExecutionRequest {
  capabilityId: string
  input: string
  payload: IntelligenceChatPayload
  options?: IntelligenceInvokeOptions
  context: IntelligenceContextExecutionIntent
}

/** Metadata-only context summary safe to return to a plugin or renderer. */
export interface IntelligenceContextExecutionSummary {
  mode: IntelligenceContextMode
  scope: ContextScope
  sessionId?: string
  turnId?: string
  packageId?: string
  traceId?: string
  checkpoint?: Pick<ContextCheckpoint, 'id' | 'type' | 'reason'>
  continuation?: ContextContinuationSummary
  itemCount: number
  tokenBudget: number
  tokenEstimate: number
  sourceTypes: ContextPackage['items'][number]['sourceType'][]
  retrievalItemCount: number
  citationCount: number
  degradedReason?: string
}

export interface IntelligenceContextExecutionResult<T = unknown> {
  invocation: IntelligenceInvokeResult<T>
  context: IntelligenceContextExecutionSummary
}

export type IntelligenceContextStreamEvent<T = unknown> = IntelligenceStreamEvent<T> & {
  context?: IntelligenceContextExecutionSummary
}

export interface MemoryReplacementInput {
  type: MemoryItem['type']
  scope: MemoryItem['scope']
  content: string
  summary?: string
  ...
  sourceTurnId?: string
  privacyLevel?: ContextPrivacyLevel
  /** Positive lifetime in milliseconds; saving the item restarts the lifetime. */
  ttl?: number
  enabled?: boolean
}

export interface MemoryUpsertInput extends MemoryReplacementInput {
  id?: string
}
...
export const DEFAULT_CAPABILITIES: Record<string, IntelligenceCapabilityConfig> = {
  ...
  'vision.ocr': {
    ...
    promptTemplate: '你是 OCR 助手，识别图片所有文本，生成 keywords 数组（5 个以内）辅助搜索。',
  ...
  'image.translate.e2e': {
    ...
    providers: [{ providerId: TUFF_NEXUS_PROVIDER_ID, priority: 1, enabled: false }],
  },
  'text.translate': {
    id: 'text.translate',
  ...
  'text.rewrite': {
    id: 'text.rewrite',
    label: '改写 / Rewrite',
    description: '按指定风格、语气或受众改写文本',
    promptTemplate: '请按 {{style}} 风格、{{tone}} 语气改写以下文本，并尽量保留原意。',
    providers: [
      { providerId: TUFF_NEXUS_PROVIDER_ID, priority: 1, enabled: false },
      { providerId: 'deepseek-default', priority: 2, enabled: true },
      { providerId: 'openai-default', priority: 3, enabled: false },
  ...
  'text.grammar': {
    id: 'text.grammar',
    label: '语法检查 / Grammar Check',
    description: '检查并修正拼写、语法、标点和表达问题',
    promptTemplate: '检查以下文本的拼写、语法、标点和表达问题，返回修正文本与问题列表。',
    providers: [
  ...
  'text.classify': {
    id: 'text.classify',
    label: '文本分类 / Text Classification',
    description: '将文本归入指定分类，支持单标签或多标签',
    promptTemplate:
      '根据给定分类对文本进行分类，返回 JSON：{predictions: [{category: string, confidence: number}], explanation: string}',
    providers: [
      { providerId: 'deepseek-default', priority: 1, enabled: true },
      { providerId: 'openai-default', priority: 2, enabled: false },
      { providerId: 'anthropic-default', priority: 3, enabled: false },
  ...
  'code.refactor': {
    id: 'code.refactor',
    label: '代码重构 / Code Refactor',
    description: '在保持行为不变的前提下重构代码',
    promptTemplate: '重构以下代码以提升可读性、可维护性或性能，并说明关键变化。',
    providers: [
      {
        providerId: 'deepseek-default',
        models: ['deepseek-coder'],
  ...
  'code.debug': {
    id: 'code.debug',
    label: '代码调试 / Code Debugging',
    description: '分析错误、定位根因并给出修复建议',
    promptTemplate: '分析以下代码和错误信息，返回根因、修复代码与预防建议。',
    providers: [
      {
        providerId: 'deepseek-default',
        models: ['deepseek-coder'],
  ...
  'audio.asr': {
    id: 'audio.asr',
    label: '实时语音识别 / Realtime ASR',
    description: '将麦克风实时语音转换为文本',
    providers: [],
  },
  'audio.transcribe': {
    id: 'audio.transcribe',
  ...
  'rag.query': {
    id: 'rag.query',
    label: 'RAG 问答 / RAG Query',
    description: '结合检索上下文回答问题',
    providers: [],
  },
  'search.semantic': {
    id: 'search.semantic',
    label: '语义搜索 / Semantic Search',
    description: '按语义相似度检索文档或片段',
    providers: [],
  },
  'search.rerank': {
    id: 'search.rerank',
    label: '搜索重排 / Search Rerank',
    description: '按查询相关性重排候选文档',
    providers: [],
  },
  'workflow.execute': {
    id: 'workflow.execute',
    label: '工作流执行 / Workflow Execution',
    description: '执行多步骤提示词、工具或 Agent 工作流',
    providers: [],
  },
  'agent.run': {
    id: 'agent.run',
    label: 'Agent 执行 / Agent Run',
    description: '运行具备工具访问能力的自主 AI Agent',
    providers: [],
  },
}
