...
export function normalizePortStreamMessage<TChunk>(
  ...
  const record = raw as StreamMessage<TChunk> &
    TransportPortEnvelope<StreamMessage<TChunk>>;
  ...
  if (rawType === "data") {
    const chunk =
      (record as { chunk?: TChunk }).chunk ??
      (record as { payload?: { chunk?: TChunk } }).payload?.chunk;
