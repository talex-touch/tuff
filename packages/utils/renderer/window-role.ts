...
export function isKnownTouchType(
  value: string | undefined,
): value is TouchType {
  return (
    typeof value === "string" &&
    (TOUCH_TYPES as readonly string[]).includes(value)
  );
}
...
export function isKnownCoreType(value: string | undefined): value is CoreType {
  return (
    typeof value === "string" &&
    (CORE_TYPES as readonly string[]).includes(value)
  );
}

export function isKnownAssistantType(
  value: string | undefined,
): value is AssistantType {
  return (
    typeof value === "string" &&
    (ASSISTANT_TYPES as readonly string[]).includes(value)
  );
}
...
export function parseWindowArgs(argv: string[]): WindowRole {
  ...
  const touchType = isKnownTouchType(argMap["touch-type"])
    ? argMap["touch-type"]
    : undefined;
  const coreType = isKnownCoreType(argMap["core-type"])
    ? argMap["core-type"]
    : undefined;
...
export function resolveRendererWindowMode(
  role: WindowRole,
): RendererWindowMode {
  if (role.metaOverlay) {
    return "MetaOverlay";
  ...
  if (role.touchType === "assistant") {
    ...
    if (role.assistantType === "region-selector") {
      return "AssistantRegionSelector";
    }
    return "Assistant";
  }
